...
# the temporal validation tail. The budget fraction covers up to two fits per
# component: the gated pre-tail fit and the recency refit on the full span.
...
def _train_member(X_t, Y_t, M_t, X_v, Y_v, M_v, X_f, Y_f, M_f, delta, comp_w,
    ...
    residuals) on PURE VALIDITY masks — the continuous recency mask ramp is
    reverted (measured -0.03 in gen011_00); recency emphasis lives only in
    the verified fine-tune below. Validates and early-stops on
    component-balanced masked MSE, the quantity the mean-R^2 metric measures.

    EMA weight averaging with warmup (this parent's variant of the verified
    gen008_03 scheme): alongside the raw SGD iterates we maintain an
    exponential moving average of the parameters whose decay ramps
    (1+s)/(10+s) -> 0.999, so early steps do not anchor the average at the
    random init. Each epoch BOTH the raw and the EMA weights are validated
    and the better one is checkpointed; because the raw net remains a
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        # the better one is checkpointed and drives the LR scheduler.
        net.eval()
    ...
    # Recency fine-tune (verified +0.005): the validation tail is the most
    # recent data — closest in time to the held-out test cycles — and was
    # excluded from training above. Briefly fine-tune the selected model on
    # the FULL dataset at low LR, sweeping oldest -> newest within each pass
    # so the freshest samples shape the final weights.
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # Build all tensors ONCE and share them across ensemble members. All
    # masks are pure 0/1 validity — the continuous recency mask ramp of the
    # parent is reverted (measured regression); recency handling lives only
    # in the fine-tune and the GBM refit, its two verified forms.
