...
# Budgeted cosine LR (parent, gen016_01): each MLP member anneals its
# learning rate from _LR_MAX to _LR_MIN following a cosine over the elapsed
# wall-clock fraction of its own training slice, so every member finishes in
# a converged low-LR regime exactly when its slice expires regardless of how
# many epochs that turns out to be.
_LR_MAX = 1e-3
_LR_MIN = 2e-5
...
# component on the temporal validation tail.
# THE ONE NEW CHANGE: the boosting length is no longer a blind constant.
# The gate fit runs to _GBM_MAX_ITER (raised 100 -> 160) and staged_predict
# scores every intermediate stage on the untouched tail; the argmin stage n*
# decides inclusion AND sets max_iter for the full-span recency refit. The
# budget fraction grows to cover the longer fits; the MLP slices already
# rescale to whatever wall clock remains.
_GBM_BUDGET_FRAC = 0.34
_GBM_MAX_ROWS = 500_000
_GBM_MAX_ITER = 160
_GBM_TAIL_EVAL_CAP = 150_000  # cap on tail rows used for staged scoring
...
def _train_member(X_t, Y_t, M_t, X_v, Y_v, M_v, X_f, Y_f, M_f, delta, comp_w,
    ...
    Budgeted cosine LR (parent): before each epoch the learning rate is set
    to a cosine interpolation between _LR_MAX and _LR_MIN driven by the
    fraction of this member's wall-clock slice already spent, so the member
    is annealed to a low LR exactly as its slice expires regardless of how
    many epochs that turns out to be — and the schedule adapts automatically
    if the GBM slice leaves this member more or less time.
...
def _make_gbm(seed, max_iter=_GBM_MAX_ITER):
...
def _staged_tail_mse(gbm, X_tail, y_tail):
    """MSE on the tail at EVERY boosting stage of a fitted HistGBR.

    staged_predict builds the raw predictions incrementally — each stage adds
    only that iteration's trees — so the total cost is on the order of ONE
    full predict over the tail, not n_stages predicts. Returns a float64
    array of per-stage MSEs, length n_iter_.
    """
    mses = []
    for pred in gbm.staged_predict(X_tail):
        d = pred.astype(np.float64) - y_tail
        mses.append(float(np.mean(d * d)))
    return np.asarray(mses, dtype=np.float64)


def _predict_at_stage(gbm, X, stage):
    """Prediction of a fitted HistGBR truncated to `stage` iterations
    (1-indexed), via the same incremental staged_predict walk."""
    pred = None
    for i, pred in enumerate(gbm.staged_predict(X), start=1):
        if i == stage:
            return pred
    return pred  # stage beyond n_iter_: last stage (should not happen)
...
def _fit_gbm_members(X_raw, X_test_raw, Ys, mask, train_idx, val_idx, seed,
    ...
    TAIL-SELECTED BOOSTING LENGTH (the one new change): the previous fixed
    max_iter=100 at lr 0.08 on 500k rows was never validated in either
    direction — likely short of convergence, and nothing would catch it if
    it overfit. Now the gating fit runs to _GBM_MAX_ITER (160) and
    staged_predict scores every intermediate stage on the untouched temporal
    tail (cost ~ one full predict). The argmin stage n* (i) decides
    inclusion — the component joins only if its BEST stage beats the
    zero-residual predictor (ridge alone), so a failed family still dilutes
    nothing — and (ii) becomes max_iter for the verified full-span recency
    refit (same recency window, shifted forward to include the tail, the
    rows closest in time to the test cycles). If the clock is tight the gate
    model itself predicts test truncated at n*, so the fallback is never
    worse than the old fixed-length model when 100 overshot and is stronger
    when it undershot.
    ...
    for c in range(2):
        ...
        if not vm.any():
            ...
            continue
        vv = val_idx[vm]
        # Evenly strided cap keeps the staged scoring cheap while still
        # covering the whole tail span.
        if len(vv) > _GBM_TAIL_EVAL_CAP:
            step = len(vv) // _GBM_TAIL_EVAL_CAP + 1
            vv = vv[::step]
        X_tail = X_raw[vv]
        y_tail = Ys[vv, c].astype(np.float64)
        stage_mse = _staged_tail_mse(gbm, X_tail, y_tail)
        n_star = int(np.argmin(stage_mse)) + 1
        mse_zero = float(np.mean(y_tail ** 2))
        if float(stage_mse[n_star - 1]) < mse_zero:
            included[c] = True
            # RECENCY REFIT at the tail-selected length: identical
            # hyperparameters except max_iter=n_star, on the most recent
            # _GBM_MAX_ROWS valid rows of the FULL span — rows are in cycle
            # (time) order, so this is the data closest to the test cycles.
            # Only attempted when the clock allows; otherwise the gate model
            # truncated at n_star predicts test.
            if time.time() < deadline:
                full_rows = np.flatnonzero(mask[:, c] > 0)[-_GBM_MAX_ROWS:]
                gbm_r = _make_gbm(seed + 31 * c + 7, max_iter=n_star)
                gbm_r.fit(X_raw[full_rows], Ys[full_rows, c])
                pred_test[:, c] = gbm_r.predict(X_test_raw)
                del gbm_r
            else:
                pred_test[:, c] = _predict_at_stage(gbm, X_test_raw, n_star)
        del gbm
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # GBM MEMBERS: a tree-family residual model per component, gated and
    # length-selected on the temporal tail, then recency-refit on the full
    # span at the selected length (see _fit_gbm_members), capped to a fixed
    # slice of the wall clock so the MLP ensemble keeps the majority.
