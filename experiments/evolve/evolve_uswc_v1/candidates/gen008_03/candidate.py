...
def _train_member(X_t, Y_t, M_t, X_v, Y_v, M_v, X_f, Y_f, M_f, delta, comp_w,
    ...
    EMA WEIGHT AVERAGING (the one new change): alongside the raw SGD
    iterates we maintain an exponential moving average of the weights
    (timm-style warmup decay ramping to 0.999, i.e. a ~4-epoch averaging
    horizon at this batch count). Averaged iterates cancel SGD noise and sit
    in flatter regions of the loss surface — precisely the solutions that
    survive the temporal train->test distribution shift this task scores on.
    Each epoch BOTH the raw and the EMA weights are validated and the better
    one is checkpointed, so selection is a strict superset of the previous
    scheme and can only match or improve the in-sample choice. The recency
