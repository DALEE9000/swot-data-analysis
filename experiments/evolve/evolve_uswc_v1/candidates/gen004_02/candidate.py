...
# Known layout for this run: 9 base features x 9 stencil cells, feature-major
# (feature f occupies columns f*9:(f+1)*9). Order:
...
def _augment(X):
    """Append physics-derived nonlinear features (stencil-ordering-invariant).

    Cross-shore (u) flow on the USWC is largely wind-stress driven (Ekman /
    upwelling), and stress is quadratic in the wind: tau ~ |W| W. Frontal and
    shear activity (SSHA/SST/velocity variability inside the stencil) marks
    ageostrophic motion. All stats are NaN-aware over each feature's 9 stencil
    cells, so they are invariant to the (unknown) cell ordering and robust to
    swath-edge padding. Falls back to the raw matrix if the layout differs.
    """
    if X.shape[1] != _NCELL * _NBASE:
        return X.astype(np.float32)
    X = X.astype(np.float32)

    def blk(f):
        return X[:, f * _NCELL:(f + 1) * _NCELL]

    with warnings.catch_warnings():
        warnings.simplefilter("ignore")  # all-NaN stencil rows -> NaN, handled later
        fmean = {f: np.nanmean(blk(f), axis=1) for f in (2, 3, 4, 5, 6, 7)}
        fstd = {f: np.nanstd(blk(f), axis=1) for f in (1, 2, 3, 8)}

    Ug, Vg = fmean[2], fmean[3]
    Uga, Vga = fmean[4], fmean[5]
    Wu, Wv = fmean[6], fmean[7]
    Wspd = np.sqrt(Wu ** 2 + Wv ** 2)
    new = np.stack([
        Wspd,                       # wind speed
        Wspd * Wu,                  # quadratic-drag wind stress, zonal
        Wspd * Wv,                  # quadratic-drag wind stress, meridional
        np.sqrt(Ug ** 2 + Vg ** 2),   # geostrophic current speed
        np.sqrt(Uga ** 2 + Vga ** 2),  # ageostrophic current speed
        fstd[1],                    # SSHA stencil variability (frontal proxy)
        fstd[8],                    # SST stencil variability (frontal proxy)
        fstd[2],                    # ugos stencil variability (shear proxy)
        fstd[3],                    # vgos stencil variability (shear proxy)
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # PHYSICS-DERIVED FEATURES: append nonlinear Ekman/frontal predictors
    # (wind stress, current speeds, stencil variability) to train and test.
    Xa_train = _augment(X_train)
    Xa_test = _augment(X_test)

    # Per-column standardization ignoring NaNs; NaN -> 0 (the feature mean).
    mean = np.nan_to_num(np.nanmean(Xa_train, axis=0), nan=0.0).astype(np.float32)
    scale = np.nanstd(Xa_train, axis=0)
    scale = np.where(np.isfinite(scale) & (scale > 1e-6), scale, 1.0).astype(np.float32)

    Xs = _standardize(Xa_train, mean, scale)
    del Xa_train
    ...
    # nonlinear/ageostrophic RESIDUAL. The augmented columns let it also use
    # wind stress and frontal-activity terms linearly.
    ...
    Xp = _standardize(Xa_test, mean, scale)
    del Xa_test
