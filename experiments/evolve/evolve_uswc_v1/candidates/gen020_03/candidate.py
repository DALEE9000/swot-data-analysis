...
# TAIL-STACKED COMBINE (verified in gen019_01): per component, a
# non-negative least squares fit on the temporal validation tail learns the
# member weights from LEAK-FREE tail predictions (pre-fine-tune MLP
# checkpoints, gated pre-tail GBM fits); the fitted weights are shrunk
# _STACK_UNIFORM_W of the way back toward the uniform average, guarded on
# finiteness / weight sum / tail size, and applied to the recency-refit
# test predictions. Any guard failure falls back to the exact equal-weight
...
# RECENCY-RIDGE DRIFT MEMBER (verified in gen020_02): a second closed-form
# ridge is fit on only the most recent _RIDGE_REC_ROWS pre-tail rows; the
# DIFFERENCE between its prediction and the full ridge's is a
# deterministic estimate of coefficient/intercept drift (seasonal wind
# regimes, mean-flow drift), entering the tail-stacked combine as one
# extra NNLS column whose uniform-prior weight is ZERO — the temporal tail
# alone decides, per component, how much drift correction to apply, and
# every guard-failure path reduces exactly to the parent's combine. For
# the test prediction the drift ridge is recency-refit on the last
# _RIDGE_REC_ROWS rows of the FULL span (the verified gate/refit
# protocol). Cost is two chunked closed-form solves, seconds of CPU.
_RIDGE_REC_ROWS = 400_000
_RIDGE_REC_MIN = 100_000


def _augment_physics(X):
    """Append quadratic stress-like features, current-speed features,
    wind-current coupling features, stencil-spread features, and stencil
    validity fractions.
    ...
    WIND-CURRENT COUPLING (THE ONE NEW CHANGE): per stencil cell append
    |W|*ug and |W|*vg. Wind speed sets mixed-layer depth and vertical
    momentum mixing, which gates how strongly the surface geostrophic
    current imprints on the subsurface: deep-mixing (windy) regimes couple
    the layers, calm stratified regimes decouple them. The stress block is
    wind-only and the current block current-only, so no existing feature
    spans this interaction — the closed-form ridge (and both drift ridges)
    could not previously represent wind-modulated attenuation of the
    geostrophic transfer at all, and the MLP/GBM had to synthesize the
    product themselves. 18 extra columns: 153 -> 171.
    ...
    return np.concatenate(
        [X,
         ...
         (wspd * ug).astype(np.float32),
         (wspd * vg).astype(np.float32),
         spread,
         valid_frac], axis=1)
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # |W|*v10), geostrophic current-speed (|Ug|, |Ug|*ug, |Ug|*vg), and
    # wind-current coupling (|W|*ug, |W|*vg) blocks per stencil cell, plus
    # per-base-feature stencil spread (nanstd over the 9 cells) and stencil
    # validity fractions: 81 -> 171 columns, all before any statistics are
    # computed, so the ridge, the MLP, and the GBM all see quadratic wind
    # forcing, speed-dependent transfer, mixing-gated surface->subsurface
    # coupling, local front-strength regime information, and explicit
    ...
    # the full training set. With the stress, current-speed, wind-current
    # coupling, stencil-spread, and validity-fraction features appended, the
    # linear fit can represent the Ekman response to wind stress,
    # speed-dependent attenuation/rotation of the geostrophic current,
    # mixing-modulated surface->subsurface coupling, a front-strength-
    # modulated offset, and a per-feature correction for partially observed
    # (mean-imputed) rows; the nonlinear members then only have to learn the
    # remaining RESIDUAL.
    ...
    # RECENCY-RIDGE DRIFT MEMBER (verified in gen020_02), gate side: fit a
    # second ridge on only the most recent _RIDGE_REC_ROWS PRE-TAIL rows.
    # Its tail prediction minus the full ridge's tail prediction is a
    # deterministic, leak-free estimate of how much the linear transfer has
    # drifted toward test time; it becomes one extra column in the NNLS
    # stack (uniform-prior weight zero, so every fallback path is exactly
    # the parent's combine). The matching test-set column is built later
    # from a ridge recency-refit on the full span.
    rd_val = None
    rd_test = None
    rec_rows = min(_RIDGE_REC_ROWS, len(train_idx))
    if rec_rows >= _RIDGE_REC_MIN:
        rec_idx = train_idx[-rec_rows:]
        B_rec, b_rec = _fit_ridge(Xs[rec_idx], Y_raw[rec_idx], mask[rec_idx],
                                  alpha=10.0)
        dB_gate = (B_rec.astype(np.float64) - B.astype(np.float64))
        db_gate = (b_rec.astype(np.float64) - b_int.astype(np.float64))
        rd_val = Xs[val_idx].astype(np.float64) @ dB_gate + db_gate
    ...
    # RECENCY-RIDGE DRIFT MEMBER, refit side (verified gate/refit protocol):
    # refit the drift ridge on the last rec_rows rows of the FULL span —
    # rows are in cycle order, so this window ends at the data nearest the
    # test cycles — and build the drift column for the test set.
    if rd_val is not None:
        full_rec_idx = np.arange(n)[-rec_rows:]
        B_rec2, b_rec2 = _fit_ridge(Xs[full_rec_idx], Y_raw[full_rec_idx],
                                    mask[full_rec_idx], alpha=10.0)
        rd_test = (Xp.astype(np.float64)
                   @ (B_rec2.astype(np.float64) - B.astype(np.float64))
                   + (b_rec2.astype(np.float64) - b_int.astype(np.float64)))
        if not (np.all(np.isfinite(rd_val)) and np.all(np.isfinite(rd_test))):
            rd_val = None
            rd_test = None
    ...
    # TAIL-STACKED COMBINE (verified in gen019_01), with the drift column
    # (verified in gen020_02): per component, learn non-negative member
    # weights on the temporal validation tail from the LEAK-FREE tail
    # predictions (pre-fine-tune MLP checkpoints, gated pre-tail GBM fits,
    # and the pre-tail recency-ridge drift), shrink them _STACK_UNIFORM_W
    # of the way back toward the uniform prior, and apply them to the
    # recency-refit test predictions. The drift column's uniform-prior
    # weight is ZERO, so under the shrinkage it enters at half its NNLS
    # weight and every guard failure (tail too small, non-finite,
    # pathological weight sum, NNLS exception) reproduces the parent's
    # combine exactly.
    out = lin_test.copy()
    for c in range(2):
        ...
        if k == 0:
            continue
        # Uniform prior: equal weight over the verified members, ZERO on the
        # drift column — the prior IS the parent's combine.
        w = np.full(k, 1.0 / k, dtype=np.float64)
        if rd_val is not None and rd_test is not None:
            cols_test.append(rd_test[:, c])
            cols_val.append(rd_val[:, c])
            w = np.concatenate([w, [0.0]])
        kk = len(cols_test)
        vm = mask[val_idx, c] > 0
        if kk >= 2 and int(vm.sum()) >= _STACK_MIN_ROWS:
