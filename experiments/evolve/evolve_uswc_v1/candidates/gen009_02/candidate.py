...
def _augment_physics(X):
    ...
    missingness indicators so mean-imputed swath-edge rows stop
    masquerading as quiescent interior rows.
...
class _EMA:
    ...
    THE ONE NEW CHANGE this generation. SGD iterates on noisy, heavy-tailed
    residual targets bounce around the loss basin; the running average of the
    trajectory sits in a flatter, lower-variance point that generalizes
    better under temporal shift. Bias-corrected warmup (min(decay,
    (1+t)/(10+t))) keeps the average from lagging the fast early descent.
    All selection/validation happens on these averaged weights, so the model
    we pick is exactly the model we deploy.
    """
...
def _train_member(X_t, Y_t, M_t, X_v, Y_v, M_v, X_f, Y_f, M_f, delta, comp_w,
    ...
    residuals); maintains an EMA of the weights, and validates/early-stops
    the EMA weights on component-balanced masked MSE, the quantity the
    mean-R^2 metric measures. Returns the net in eval mode."""
    ...
    # EMA window ~1000 steps (~5 epochs at ~200 steps/epoch): long enough to
    # average away SGD noise, short enough to track LR-schedule improvements.
    ...
    def _val_loss_of(state):
        # Evaluate a weight snapshot without losing the live SGD weights.
        ...
        with torch.no_grad():
            ...
            for i in range(0, len(X_v), bs):
                losses.append(_masked_mse(net(X_v[i:i + bs]), Y_v[i:i + bs],
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        # Validation / checkpoint selection on the EMA weights with
        # component-balanced masked MSE: the score averages per-component
        # R^2, so selection must weigh u and v equally in sigma units — and
        # since the deployed model is the EMA, the EMA is what we select on.
        val_loss = _val_loss_of(ema.shadow)
    ...
    # shape the final weights. A SHORTER-window EMA (0.995, ~200 steps) runs
    # over the fine-tune so the final weights are a noise-damped average that
    # still emphasizes the newest blocks of the sweep.
    ft_opt = torch.optim.AdamW(net.parameters(), lr=1e-4, weight_decay=1e-4)
    ft_epochs = min(2, max(0, max_epochs - best_epoch))
    ft_ema = _EMA(net, decay=0.995)
    net.train()
    ...
    # Deploy the fine-tune EMA (equals the selected weights if no fine-tune
    # steps ran, since the warmup-corrected average starts at those weights).
    if ft_ema.steps > 0:
        net.load_state_dict(ft_ema.shadow)
    net.eval()
    return net
