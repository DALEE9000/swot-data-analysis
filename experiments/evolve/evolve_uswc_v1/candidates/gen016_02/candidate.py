...
# PREDICTION-TIME RECENCY RIDGE (the one change vs the verified 0.3502
# stack). The parent (gen015_03) let a recency-blended ridge redefine the
# residual targets and deployed a post-gate REFIT the tail never scored —
# both departures from the verified program, and it lost 0.022. Here the
# recency hypothesis is retained but made strictly additive and strictly
# validated: the residual stack (targets, Huber deltas, component weights,
# GBM gating, MLP training) is computed from the full-span ridge exactly as
# in gen014_02; a second ridge fit on the most recent _RIDGE_RECENT_ROWS
# pre-tail rows enters ONLY as a per-component prediction-time replacement
# of the linear term, using exactly the blend weight scored on the
# untouched temporal tail (no refit), and only if that blend beats the
# full-span ridge on the tail by at least _RIDGE_MIN_GAIN relative MSE —
# a materiality gate so noise-level wins cannot flip the deployment. With
# the gate closed the program is byte-equivalent to gen014_02.
_RIDGE_RECENT_ROWS = 400_000
_RIDGE_ALPHAS = (0.25, 0.5, 0.75, 1.0)
_RIDGE_MIN_GAIN = 0.01
...
def _fit_ridge(Xs, Ys, mask, alpha=10.0, rows=None):
    ...
    of the signal with no SGD noise. If `rows` is given, the fit is restricted
    to that row subset (used for the recency ridge); otherwise all rows.
    """
    d = Xs.shape[1]
    ...
    for c in range(2):
        if rows is None:
            idx = np.flatnonzero(mask[:, c] > 0)
        else:
            idx = rows[mask[rows, c] > 0]
        if len(idx) == 0:
            continue
        ybar = float(Ys[idx, c].mean())
...
def _fit_gbm_members(X_raw, X_test_raw, Ys, mask, train_idx, val_idx, seed,
    ...
    pre-tail model when the clock is tight, so this can only add the
    verified recency benefit, never lose the gated prediction.
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # TEMPORAL validation split (computed up front — the recency ridge gate
    # needs it): rows are flattened in cycle (time) order, and the harness's
    # test split is later cycles. Hold out the last 10% of rows as a
    # contiguous tail so ridge gating / early stopping / LR scheduling /
    # GBM gating all select for forward-in-time generalization instead of
    # memorization.
    n = len(Xs)
    ...
    B_full, b_full = _fit_ridge(Xs, Y_raw, mask, alpha=10.0)

    # PREDICTION-TIME RECENCY RIDGE (the one change): a second ridge is fit
    # on the most recent _RIDGE_RECENT_ROWS rows of the PRE-tail span; per
    # component, blends of the full-span and recent ridge are scored on the
    # untouched temporal tail, and a blend is deployed ONLY if it beats the
    # full-span ridge there by >= _RIDGE_MIN_GAIN relative MSE. Critically,
    # the deployed object is EXACTLY the blend the tail scored — no post-
    # gate refit — and it replaces only the linear term of the final test
    # prediction (B_dep/b_dep below). The residual targets, Huber deltas,
    # component weights, GBM gating, and MLP training all use the full-span
    # ridge, byte-identical to the verified 0.3502 program, so a drifting
    # recent-window fit can never destabilize the nonlinear stack.
    B_dep, b_dep = B_full.copy(), b_full.copy()
    rec_gate = train_idx[-_RIDGE_RECENT_ROWS:]
    if len(rec_gate) >= 50_000 and len(val_idx) > 0:
        B_rg, b_rg = _fit_ridge(Xs, Y_raw, mask, alpha=10.0, rows=rec_gate)
        Xv = Xs[val_idx]
        P_full = (Xv @ B_full + b_full).astype(np.float64)
        P_rec = (Xv @ B_rg + b_rg).astype(np.float64)
        for c in range(2):
            vm = mask[val_idx, c] > 0
            if not vm.any():
                continue
            yv = Y_raw[val_idx, c][vm].astype(np.float64)
            pf, pr = P_full[vm, c], P_rec[vm, c]
            mse_full = float(np.mean((pf - yv) ** 2))
            best_mse, best_a = mse_full, 0.0
            for a in _RIDGE_ALPHAS:
                mse = float(np.mean(((1.0 - a) * pf + a * pr - yv) ** 2))
                if mse < best_mse:
                    best_mse, best_a = mse, a
            # Materiality gate: deploy only a clearly-better blend, and
            # deploy the exact validated coefficients (no refit).
            if best_a > 0.0 and best_mse < (1.0 - _RIDGE_MIN_GAIN) * mse_full:
                a32 = np.float32(best_a)
                B_dep[:, c] = (1.0 - a32) * B_full[:, c] + a32 * B_rg[:, c]
                b_dep[c] = (1.0 - a32) * b_full[c] + a32 * b_rg[c]
        del B_rg, b_rg, Xv, P_full, P_rec

    # Residual targets from the FULL-SPAN ridge (identical to gen014_02):
    # every nonlinear member trains against these regardless of the
    # deployed linear term above.
    lin_train = Xs @ B_full + b_full
    ...
    # Deployed linear term: the tail-validated (possibly blended) ridge.
    lin_test = (Xp @ B_dep + b_dep).astype(np.float64)
    ...
    # of the deployed (tail-validated) ridge baseline.
    out = lin_test.copy()
    for c in range(2):
