...
def _augment_physics(X):
    ...
    wind-current interaction features, stencil-spread features, and stencil
    validity fractions.
    ...
    WIND-CURRENT INTERACTION (the one new change): per stencil cell, the dot
    product (u10*ug + v10*vg) and cross product (u10*vg - v10*ug) of the wind
    and the geostrophic current. The existing quadratic blocks are
    self-interactions of wind or current ALONE; no linear combination of them
    expresses their mutual alignment. On the US West Coast that alignment is
    the classic regime variable — wind blowing with/against/across the
    alongshore current controls upwelling-favorable Ekman transport and
    surface-layer shear, and hence how the surface geostrophic velocity maps
    to subsurface velocity. Bilinear, per-cell, invariant to the stencil
    layout, and NaN cells degrade to column-mean imputation as usual.
    ...
    fraction of finite cells across its 9 stencil cells, so downstream models
    can tell genuinely mean-valued cells from imputed swath-edge cells.
    ...
    # Wind-current alignment: dot (along-current wind forcing) and cross
    # (cross-current wind forcing / relative spin) per stencil cell.
    wc_dot = u10 * ug + v10 * vg
    wc_crs = u10 * vg - v10 * ug
    ...
    return np.concatenate(
        [X,
         ...
         wc_dot.astype(np.float32),
         wc_crs.astype(np.float32),
         spread,
         valid_frac], axis=1)
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # wind-current interaction (W.Ug dot and cross products) blocks per
    # stencil cell, plus per-base-feature stencil spread (nanstd over the 9
    # cells) and stencil validity fractions: 81 -> 171 columns, all before
    # any statistics are computed, so the ridge and the MLP both see quadratic
    # wind forcing, speed-dependent transfer, wind-current alignment, local
    ...
    # interaction, stencil-spread, and validity-fraction features appended,
    # the linear fit can represent the Ekman response to wind stress,
    # speed-dependent attenuation/rotation of the geostrophic current, an
    # alignment-modulated wind-current coupling, a front-strength-modulated
