...
# GBM ensemble member (verified +0.012 in gen014_01, recency refit verified
# +0.005 in gen014_02): wall-clock fraction reserved for the tree models and
# the recency-capped row count they train on. Trees are fit on RAW features
# with NaNs intact (native missing-value handling) and are gated per
# component on the temporal validation tail. The budget fraction covers up
# to two fits per component: the gated pre-tail fit and the recency refit.
...
def _train_member(X_t, Y_t, M_t, X_v, Y_v, M_v, X_f, Y_f, M_f, delta, comp_w,
    ...
    Returns (net, tail_pred): the net (loaded with the best-found weights,
    then recency fine-tuned) in eval mode, and this member's PRE-fine-tune
    predictions on the validation tail (n_val, 2) — a leakage-free record of
    the selected checkpoint's skill on data it never trained on, used by the
    tail-gated stacking step in the caller."""
    ...
    # PRE-FINE-TUNE TAIL SNAPSHOT: record the selected checkpoint's
    # predictions on the validation tail BEFORE the recency fine-tune touches
    # tail rows. These are honest out-of-sample tail predictions, safe to use
    # later for fitting/gating stacking weights without leakage.
    net.eval()
    tail_chunks = []
    with torch.no_grad():
        for i in range(0, len(X_v), bs):
            tail_chunks.append(net(X_v[i:i + bs]).cpu().numpy())
    tail_pred = (np.concatenate(tail_chunks, axis=0)
                 if tail_chunks else np.zeros((0, 2), dtype=np.float32))
...
def _fit_gbm_members(X_raw, X_test_raw, Ys, mask, train_idx, val_idx, seed,
    ...
    Gate-then-refit (verified in gen014_02): the gating fit uses the most
    recent _GBM_MAX_ROWS valid rows of the PRE-validation span, so the
    untouched temporal tail can gate inclusion — a component's GBM joins the
    residual average only if it beats the zero-residual predictor (ridge
    alone) on that tail, so a failed family dilutes nothing. Per component
    that PASSES the gate, the GBM is refit with identical hyperparameters on
    the most recent _GBM_MAX_ROWS valid rows of the FULL span — the same
    window size, shifted forward to include the tail — and the refit model
    predicts the test set. A deadline check before the refit falls back to
    the gated pre-tail model when the clock is tight.

    Also returns the PRE-TAIL gating model's predictions on the full
    validation tail, val_pred (n_val, 2): honest out-of-sample tail
    predictions used by the caller's tail-gated stacking step.

    Returns (pred_test (n_test, 2) float64, val_pred (n_val, 2) float64,
    included (2,) bool).
    """
    pred_test = np.zeros((len(X_test_raw), 2), dtype=np.float64)
    val_pred = np.zeros((len(val_idx), 2), dtype=np.float64)
    ...
    for c in range(2):
        ...
        if not vm.any():
            del gbm
            continue
        pv_all = gbm.predict(X_raw[val_idx])
        y_val = Ys[val_idx, c].astype(np.float64)[vm]
        mse_gbm = float(np.mean((pv_all[vm] - y_val) ** 2))
        ...
        if mse_gbm < mse_zero:
            ...
            if time.time() < deadline:
                ...
                del gbm_r
        del gbm
    return pred_test, val_pred, included


def _stack_weights(mlp_tail_avg, gbm_tail, y_tail, valid, n_members):
    """Tail-gated stacking weights for (MLP-average, GBM) — the ONE new step.

    The equal-weight combine gives the GBM weight 1/(n_members+1), but the
    GBM is the most decorrelated (and repeatedly the most impactful) member,
    so the loss-optimal weight is usually different. Fit a 2-weight least
    squares on the OLDER half of the temporal tail using only pre-tail model
    predictions (no leakage: MLP tail preds are snapshotted before the
    recency fine-tune, GBM tail preds come from the pre-tail gating fit),
    then GATE: accept the fitted weights only if they beat the equal-weight
    combo on the untouched NEWER half of the tail. Worst case is exactly the
    verified equal-weight behavior; the failed gen014_00 calibration had no
    such holdout gate and rescaled the full prediction — this reweights only
    the residual members, and only when forward-in-time evidence agrees.

    Returns (w_mlp, w_gbm) as floats.
    """
    n_val = len(y_tail)
    w_eq = np.array([n_members / (n_members + 1.0), 1.0 / (n_members + 1.0)])
    half = n_val // 2
    pos = np.arange(n_val)
    selA = valid & (pos < half)    # older half: fit
    selB = valid & (pos >= half)   # newer half: gate
    if selA.sum() < 1000 or selB.sum() < 1000:
        return float(w_eq[0]), float(w_eq[1])
    P = np.stack([mlp_tail_avg[selA], gbm_tail[selA]], axis=1).astype(np.float64)
    yA = y_tail[selA].astype(np.float64)
    G = P.T @ P
    G += (1e-3 * np.trace(G) / 2.0 + 1e-12) * np.eye(2)
    try:
        wf = np.linalg.solve(G, P.T @ yA)
    except np.linalg.LinAlgError:
        return float(w_eq[0]), float(w_eq[1])
    wf = np.clip(wf, 0.0, 1.5)

    def _mse_b(w):
        pb = w[0] * mlp_tail_avg[selB] + w[1] * gbm_tail[selB]
        return float(np.mean((pb - y_tail[selB].astype(np.float64)) ** 2))

    if _mse_b(wf) < _mse_b(w_eq):
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # MLP ensemble keeps the lion's share. The gating model's tail
    # predictions are kept for the stacking step below.
    gbm_deadline = t0 + _GBM_BUDGET_FRAC * time_budget_s
    gbm_pred_test, gbm_val_pred, gbm_included = _fit_gbm_members(
    ...
    tail_sum = np.zeros((n_val, 2), dtype=np.float64)
    n_members = 0
    ...
    for m, frac in enumerate(slice_ends):
        ...
        if m > 0 and remaining < 0.08 * remaining_total:
            break
        net, tail_pred = _train_member(
            X_t, Y_t, M_t, X_v, Y_v, M_v, X_f, Y_f, M_f, delta,
            comp_w, Xs.shape[1], seed + 1000 * m, device, max_epochs, t_end)
        with torch.no_grad():
            for i in range(0, len(Xp), 65536):
                xb = torch.from_numpy(Xp[i:i + 65536]).to(device)
                pred_sum[i:i + 65536] += net(xb).cpu().numpy()
        tail_sum += tail_pred.astype(np.float64)
        n_members += 1
        del net
        if device.type == "cuda":
            torch.cuda.empty_cache()

    # COMBINE: per component, weight the MLP-member average against the gated
    # GBM using tail-gated stacking weights (equal-member weights — the exact
    # verified gen014_02 combine — unless the fitted weights win on the
    # untouched newer half of the tail), all on top of the exact ridge
    # baseline. Components without a gated GBM keep the plain MLP average.
    out = lin_test.copy()
    for c in range(2):
        if n_members == 0:
            if gbm_included[c]:
            ...
            continue
        mlp_avg_test = pred_sum[:, c] / n_members
        if not gbm_included[c]:
            out[:, c] += mlp_avg_test
            continue
        valid_tail = mask[val_idx, c] > 0
        w_mlp, w_gbm = _stack_weights(
            tail_sum[:, c] / n_members, gbm_val_pred[:, c],
            Ys[val_idx, c], valid_tail, n_members)
        out[:, c] += w_mlp * mlp_avg_test + w_gbm * gbm_pred_test[:, c]
