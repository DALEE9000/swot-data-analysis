...
def _augment_physics(X):
    ...
    missingness indicators for the mean-imputed downstream models.
    NAN-AWARE STENCIL MEAN (the one new change): per base feature, the
    nanmean across its 9 stencil cells. After standardization NaN cells are
    imputed to 0 (the column mean), so any linear combination of a partially
    observed stencil is attenuated by n_valid/9 — the imputed average is the
    true local mean shrunk toward zero exactly at swath edges. The validity
    fraction says HOW MUCH attenuation happened; the nanmean supplies the
    UNattenuated local estimate itself, the ratio of those two quantities
    and therefore not linearly recoverable by the ridge. Fully valid rows
    see a redundant (harmless) column; edge rows recover full-strength
    signal precisely where the validity-fraction gain showed the headroom
    lives. Ordering-invariant, computed before standardization, ~free.
    Guarded to the known 81-column layout; any other width passes through
    unchanged.
    """
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # explicit data-quality indicators, and de-attenuated local estimates
    # for partially observed (mean-imputed) swath-edge rows.
    ...
    # and — via the nanmean columns — an imputation-attenuation-free local
    # signal at swath edges; the MLP ensemble then only has to learn the
