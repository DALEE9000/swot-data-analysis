...
# TAIL-FITTED COMBINE (the one new change): shrinkage factor pulling the
# tail-fitted per-component combination weights toward the parent's
# equal-weight combine. 0.0 would reproduce the parent exactly; 1.0 would
# trust the tail fit fully. 0.5 keeps the parent as a strong prior while
# letting ~100k tail rows correct the blind 3/4-MLP, 1/4-GBM split.
_COMBINE_SHRINK = 0.5
_COMBINE_MIN_ROWS = 1000
_COMBINE_W_MAX = 1.5
...
def _train_member(X_t, Y_t, M_t, X_v, Y_v, M_v, X_f, Y_f, M_f, delta, comp_w,
    ...
    then recency fine-tuned) in eval mode, plus the checkpoint-selected
    model's predictions on the validation tail as a float64 (n_val, 2)
    array. val_pred is computed from the SELECTED weights BEFORE the
    recency fine-tune touches the tail, so it is an honest out-of-sample
    view of this member on the tail — the input the tail-fitted combine
    needs."""
    ...
    # Honest tail predictions from the selected checkpoint, captured BEFORE
    # the recency fine-tune trains on the tail. One extra validation pass —
    # negligible wall-clock. These feed the tail-fitted combine.
    net.eval()
    val_pred = np.zeros((len(X_v), 2), dtype=np.float64)
    with torch.no_grad():
        for i in range(0, len(X_v), bs):
            val_pred[i:i + bs] = net(X_v[i:i + bs]).cpu().numpy()
...
def _fit_gbm_members(X_raw, X_test_raw, Ys, mask, train_idx, val_idx, seed,
    ...
    Returns (pred_test (n_test, 2) float64, pred_val (n_val, 2) float64,
    included (2,) bool). pred_val holds the GATING fit's predictions on the
    masked validation-tail rows (zero elsewhere) — an honest out-of-sample
    view of the tree family on the tail, consumed by the tail-fitted
    combine.
    """
    pred_test = np.zeros((len(X_test_raw), 2), dtype=np.float64)
    pred_val = np.zeros((len(val_idx), 2), dtype=np.float64)
    ...
    for c in range(2):
        ...
        if mse_gbm < mse_zero:
            ...
            if time.time() < deadline:
                ...
                del gbm_r
        del gbm
    return pred_test, pred_val, included
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    gbm_pred_test, gbm_pred_val, gbm_included = _fit_gbm_members(
    ...
    mlp_val_sum = np.zeros((n_val, 2), dtype=np.float64)
    n_members = 0
    ...
    for m, frac in enumerate(slice_ends):
        ...
        if m > 0 and remaining < 0.08 * remaining_total:
            break
        net, val_pred = _train_member(X_t, Y_t, M_t, X_v, Y_v, M_v, X_f, Y_f,
                                      M_f, delta, comp_w, Xs.shape[1],
                                      seed + 1000 * m, device, max_epochs,
                                      t_end)
        mlp_val_sum += val_pred
        with torch.no_grad():
    ...
    # TAIL-FITTED COMBINE (the one new change): per component, fit the two
    # combination weights [w_mlp, w_gbm] on the temporal validation tail by
    # ridge-regularized least squares against the post-ridge residual
    # targets, using honest out-of-sample tail predictions (checkpoint MLPs
    # before the recency fine-tune; the gating GBM fit). The parent's blind
    # equal-weight combine — n_members/(n_members+1) on the MLP average,
    # 1/(n_members+1) on the GBM — is kept as a strong prior: the fitted
    # weights are clipped to [0, _COMBINE_W_MAX] and shrunk _COMBINE_SHRINK
    # of the way from the equal weights toward the fit. Any degeneracy (too
    # few valid tail rows, singular system, non-finite solution, zero
    # members) falls back to the parent's exact equal-weight behavior.
    Ys_val = Ys[val_idx]
    mask_val = mask[val_idx]
    out = lin_test.copy()
    for c in range(2):
        nm = max(1, n_members)
        mlp_avg_test = pred_sum[:, c] / nm
        mlp_avg_val = mlp_val_sum[:, c] / nm
        denom = max(1, n_members + (1 if gbm_included[c] else 0))
        if gbm_included[c]:
            w_eq = np.array([n_members / denom, 1.0 / denom], dtype=np.float64)
            cols_val = np.stack([mlp_avg_val, gbm_pred_val[:, c]], axis=1)
            cols_test = np.stack([mlp_avg_test, gbm_pred_test[:, c]], axis=1)
        else:
            w_eq = np.array([n_members / denom], dtype=np.float64)
            cols_val = mlp_avg_val[:, None]
            cols_test = mlp_avg_test[:, None]
        w = w_eq
        vm = mask_val[:, c] > 0
        if n_members > 0 and int(vm.sum()) >= _COMBINE_MIN_ROWS:
            A = cols_val[vm]
            y = Ys_val[vm, c].astype(np.float64)
            G = A.T @ A + 1e-6 * np.eye(A.shape[1])
            try:
                w_fit = np.linalg.solve(G, A.T @ y)
                w_fit = np.clip(w_fit, 0.0, _COMBINE_W_MAX)
                if np.all(np.isfinite(w_fit)):
                    w = (1.0 - _COMBINE_SHRINK) * w_eq + _COMBINE_SHRINK * w_fit
