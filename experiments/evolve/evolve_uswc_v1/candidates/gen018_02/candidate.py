...
# ARCHITECTURE-DIVERSE ENSEMBLE (THE ONE NEW CHANGE): the decorrelated GBM
# family is the biggest verified gain in this lineage (+0.012 in gen014_01)
# — evidence that what the ensemble lacks is error DECORRELATION, not more
# copies of the same model. Yet the MLP members are identical clones
# differing only by seed. Member 0 keeps the exact verified architecture so
# the ensemble always contains one full-strength known-good model; member 1
# is wider-and-shallower (a smoother, lower-frequency fit of the residual),
# member 2 deeper-and-narrower (a more compositional fit). Different
# depths/widths induce genuinely different inductive biases, decorrelating
# member errors far more than seed noise does, at essentially unchanged
# parameter count and memory. Dropout stays at the verified 0.1 everywhere.
_MEMBER_ARCHS = [
...
def _train_member(X_t, Y_t, M_t, X_v, Y_v, M_v, X_f, Y_f, M_f, delta, comp_w,
    ...
    ARCHITECTURE DIVERSITY (the one new change): `hidden` is the member's
