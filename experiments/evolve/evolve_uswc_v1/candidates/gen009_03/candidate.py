...
def _train_member(X_t, Y_t, M_t, X_v, Y_v, M_v, X_f, Y_f, M_f, delta, comp_w,
    ...
    `hidden` sets this member's trunk architecture — ARCHITECTURE DIVERSITY
    (the one new change this generation): each ensemble member trains a
    different width/depth trunk so member errors decorrelate beyond what a
    seed change alone provides. Each member is still validated, EMA-shadowed,
    checkpoint-selected, and recency-fine-tuned identically, preserving the
    per-member quality floor.
    ...
    weights (warmup decay ramping to 0.999). Each epoch BOTH the raw and the
    EMA weights are validated and the better one is checkpointed, so
    selection is a strict superset of raw-only selection.
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # ARCHITECTURE-DIVERSE SEED ENSEMBLE within the wall-clock budget.
    # Member 0 keeps the verified (256, 256, 128) trunk AND half the budget,
    # so even under a tight clock the ensemble contains at least one
    # full-strength model of the proven configuration. Members 1-2 split the
    # remainder with deliberately different trunks — wider-shallower
    # (384, 192) and deeper-narrower (192, 192, 192, 128) — whose distinct
    # inductive biases decorrelate member errors far more than a seed change
    # alone: ensemble error = mean individual error - diversity, and with
    # only 3 members diversity is the cheap lever. Cumulative slice
    # boundaries as budget fractions:
    member_cfgs = [
        (0.50, (256, 256, 128)),
        (0.78, (384, 192)),
        (1.00, (192, 192, 192, 128)),
    ]
    ...
    for m, (frac, hidden) in enumerate(member_cfgs):
