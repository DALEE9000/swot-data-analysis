...
# INPUT-NOISE AUGMENTATION (THE ONE NEW CHANGE): std of the zero-mean
# Gaussian noise added to each standardized training batch in the MAIN
# training loop only. Inputs are standardized to unit variance, so 0.05 is
# 5% of a feature std — enough to stop the net keying on exact imputed-zero
# values and instrument-precision feature detail that drifts across cycles,
# small enough not to blur real mesoscale structure. Validation and the
# recency fine-tune stay noise-free: selection must measure the true
# objective, and the fine-tune's job is to absorb the freshest cycles'
# exact signal. Cross-lineage evidence: the input-noise lineage posts the
# board's best r2_v (0.4029 at gen016_03) from a weaker base.
...
def _train_member(X_t, Y_t, M_t, X_v, Y_v, M_v, X_f, Y_f, M_f, delta, comp_w,
    ...
    INPUT-NOISE AUGMENTATION (the one new change): every main-loop training
    batch is jittered with zero-mean Gaussian noise of std _INPUT_NOISE_STD
    in standardized units. The standardized inputs impute every NaN stencil
    cell to exactly 0.0, so an unregularized net can key on razor-thin
    structure around imputed values and on instrument-precision detail that
    does not persist across cycles; the jitter forces the learned residual
    correction to be locally smooth in feature space, which is what
    transfers under the temporal shift the held-out split measures.
    Validation batches and the recency fine-tune are noise-free.
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for i in range(0, n_train, bs):
            ...
            # Denoising jitter: torch's global RNG was seeded from
            # member_seed above, so this is fully reproducible.
    ...
    # shape the final weights. NO input noise here: this phase exists to
    # absorb the freshest cycles' exact signal, which jitter would blur.
