...
# GBM ensemble member (verified +0.012 in gen014_01, recency refit +0.005 in
...
# the temporal validation tail. The budget fraction covers up to two fits
# per component: the gated pre-tail fit and the recency refit on the full
# span.
_GBM_BUDGET_FRAC = 0.26
_GBM_MAX_ROWS = 500_000

# RECENT-WINDOW SPECIALIST (the one new change): the third MLP member trains
# on a hard recent window — the most recent _SPEC_TRAIN_FRAC of the pre-tail
# rows — instead of the full span, adding a third DISCRETE recency mechanism
# alongside the two verified ones (the oldest->newest fine-tune and the GBM
# recency refit). No sample weights anywhere: continuous recency weighting
# is the lineage's largest measured regression (gen011_00, gen017_02/03).
# Because a half-data same-family member is weaker than a decorrelated tree
# family, its gate is stricter than the GBM's beats-zero test: it must also
# land within _SPEC_VAL_TOL of the best full-span member's tail validation
# loss, otherwise the ensemble reverts exactly to the champion combination.
_SPEC_TRAIN_FRAC = 0.5
_SPEC_VAL_TOL = 1.05
...
def _train_member(X_t, Y_t, M_t, X_v, Y_v, M_v, X_f, Y_f, M_f, delta, comp_w,
    ...
    The training tensors (X_t, Y_t, M_t) and fine-tune tensors (X_f, Y_f,
    M_f) are supplied by the caller, so a member can be a full-span model or
    a recent-window specialist without any change to the training procedure.
    ...
    Returns (net, best_val): the net (loaded with the best-found weights,
    then recency fine-tuned) in eval mode, and the pre-fine-tune tail
    validation loss used for cross-member gating."""
    ...
    # above. Briefly fine-tune the selected model on the FULL fine-tune span
    # at low LR, sweeping oldest -> newest within each pass so the freshest
    # samples shape the final weights.
...
def _fit_gbm_members(X_raw, X_test_raw, Ys, mask, train_idx, val_idx, seed,
    ...
    shifted forward to include the tail, the data closest in time to the
    test cycles — and the refit model predicts the test set. A deadline
    check before the refit falls back to the gated pre-tail model when the
    clock is tight, so this can only add the verified recency benefit,
    never lose the gated prediction.
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # RECENT-WINDOW SPECIALIST tensors: zero-copy chronological slices of the
    # full-span tensors. Training window = most recent _SPEC_TRAIN_FRAC of
    # the PRE-TAIL rows (so the tail stays untouched for its early stopping
    # and for the inclusion gate); fine-tune window = most recent
    # _SPEC_TRAIN_FRAC of ALL rows, the same window shifted forward to
    # include the tail — mirroring the verified GBM gate-then-refit shape.
    pre_tail = n - n_val
    spec_t0 = int(pre_tail * (1.0 - _SPEC_TRAIN_FRAC))
    spec_f0 = int(n * (1.0 - _SPEC_TRAIN_FRAC))
    X_t_s, Y_t_s, M_t_s = X_f[spec_t0:pre_tail], Y_f[spec_t0:pre_tail], M_f[spec_t0:pre_tail]
    X_f_s, Y_f_s, M_f_s = X_f[spec_f0:], Y_f[spec_f0:], M_f[spec_f0:]

    # Tail loss of the zero-residual predictor (ridge alone), in the same
    # component-balanced units as each member's best_val: the specialist's
    # first (lenient, GBM-precedent) gate.
    with torch.no_grad():
        zero_tail = float(((Y_v.pow(2) * comp_w.to(device) * M_v).sum()
                           / M_v.sum().clamp(min=1)).item())
    ...
    # clock, the ensemble contains at least one full-strength model; member 1
    # adds a decorrelated full-span view; member 2 is the recent-window
    # specialist, gated on the temporal tail before it may join the average.
    ...
    for m, frac in enumerate(slice_ends):
        ...
        if m > 0 and remaining < 0.08 * remaining_total:
            break
        if m == spec_member:
            net, member_val = _train_member(
                X_t_s, Y_t_s, M_t_s, X_v, Y_v, M_v, X_f_s, Y_f_s, M_f_s,
                delta, comp_w, Xs.shape[1], seed + 1000 * m, device,
                max_epochs, t_end)
            # SPECIALIST GATE: must beat the zero-residual predictor on the
            # tail AND land within _SPEC_VAL_TOL of the best full-span
            # member's tail loss. A same-family half-data member decorrelates
            # less than the GBM's tree family, so unlike the GBM it must
            # also be near-parity in quality; otherwise the ensemble reverts
            # exactly to the champion's two full members + GBM.
            include = (member_val < zero_tail
                       and (not full_vals
                            or member_val <= _SPEC_VAL_TOL * min(full_vals)))
        else:
            net, member_val = _train_member(
                X_t, Y_t, M_t, X_v, Y_v, M_v, X_f, Y_f, M_f,
                delta, comp_w, Xs.shape[1], seed + 1000 * m, device,
                max_epochs, t_end)
            full_vals.append(member_val)
            include = True
        if include:
            with torch.no_grad():
    ...
    # COMBINE: per component, average the included MLP members plus (if it
    # passed the validation gate) the GBM as one extra equal-weight member,
    # all on top of the exact ridge baseline.
    out = lin_test.copy()
    for c in range(2):
