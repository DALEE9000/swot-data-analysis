...
# pre-tail fit and (the one new change) a recency refit on the full span.
...
def _fit_gbm_members(X_raw, X_test_raw, Ys, mask, train_idx, val_idx, seed,
    ...
    Gate-then-refit (the one new change): the gating fit uses the most
    recent _GBM_MAX_ROWS valid rows of the PRE-validation span, so the
    untouched temporal tail can gate inclusion exactly as before — a
    component's GBM joins the residual average only if it beats the
    zero-residual predictor (ridge alone) on that tail, so a failed family
    dilutes nothing. But the MLP members then get a recency fine-tune on the
    full data (including that tail, the rows closest in time to the test
    cycles) while the trees previously predicted test from a model that
    never saw the freshest ~10% of rows. So, per component that PASSES the
