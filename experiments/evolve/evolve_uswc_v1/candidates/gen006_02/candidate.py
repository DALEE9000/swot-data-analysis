...
def _masked_huber(pred, target, mask, delta, comp_w):
    ...
    COMPONENT BALANCE (the one new change): each component's loss is scaled
    by comp_w_c ~ 1/delta_c^2 (normalized to mean 1, so the overall loss
    magnitude — and the tuned LR — is unchanged). Raw m/s losses let the
    higher-variance v residual dominate the shared trunk's gradients, which
    matches the lineage's persistent r2_u << r2_v gap; in sigma units both
    components pull equally, mirroring the mean-of-R^2 metric.
    """
    err = (pred - target).abs()
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # component (v, per the lineage's persistent r2_u << r2_v gap) dominate
    # the shared trunk. In sigma units both components pull equally; the
    # mean-1 normalization keeps the loss magnitude (and tuned LR) unchanged.
