...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # STACKED COMBINATION WITH PER-COMPONENT INNER SELECTION. Evidence from
    # the lineage: tail-fitted stacking helped v but hurt u (whose HFR-derived
    # residuals are noisiest), so stacking must PROVE it generalizes forward
    # in time before being adopted for a component. Protocol per component:
    #   1. Split the chronological tail in half: fit-half (older) and
    #      eval-half (newer, most test-like).
    #   2. Fit stacking weights on the fit-half only.
    #   3. Adopt stacking only if it beats the plain member average's MSE on
    #      the eval-half by a small margin; then refit on the full tail for
    #      the final weights. Otherwise fall back to the plain average
    #      (the previously best-scoring combiner).
    M_members = len(member_tests)
    out = np.zeros((len(Xp), 2), dtype=np.float64)
    n_val_rows = len(Yv_resid)
    half = n_val_rows // 2
    pos = np.arange(n_val_rows)
    for c in range(2):
        default = lin_test[:, c]
        if M_members > 0:
        ...
        def design(msk):
            return np.column_stack(
                [vp[msk, c] for vp in member_vals] +
                [lin_val[msk, c], np.ones(int(msk.sum()))]
            )

        w0 = np.array([1.0 / M_members] * M_members + [0.0, 0.0])
        fit_m = valid & (pos < half)
        ev_m = valid & (pos >= half)
        if int(fit_m.sum()) < 500 or int(ev_m.sum()) < 500:
            out[:, c] = default
            continue

        # Inner fit + forward-in-time evaluation.
        w_fit = _stack_weights(design(fit_m), Yv_resid[fit_m, c], w0, lam=0.05)
        use_stack = False
        if np.all(np.isfinite(w_fit)) and np.max(np.abs(w_fit[:M_members])) <= 5.0:
            r_ev = Yv_resid[ev_m, c]
            mse_stack = float(np.mean((r_ev - design(ev_m) @ w_fit) ** 2))
            avg_ev = np.mean([vp[ev_m, c] for vp in member_vals], axis=0)
            mse_avg = float(np.mean((r_ev - avg_ev) ** 2))
            # Require a real margin, not a noise-level win.
            use_stack = np.isfinite(mse_stack) and mse_stack < 0.995 * mse_avg
        if not use_stack:
            out[:, c] = default
            continue

        # Stacking earned adoption for this component: refit on the full tail.
        w = _stack_weights(design(valid), Yv_resid[valid, c], w0, lam=0.05)
