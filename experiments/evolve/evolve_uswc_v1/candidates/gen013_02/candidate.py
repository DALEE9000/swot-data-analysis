...
def _augment_physics(X):
    ...
    missingness indicators so mean-imputed swath-edge rows do not masquerade
    as quiescent interior rows.
    STENCIL MIN/MAX (the one new change): per base feature, the NaN-aware
    min and max across its 9 stencil cells. The std is symmetric — it cannot
    say which SIDE of a local front or jet the row sits on, nor how extreme
    the local field gets. Min/max (together with the per-cell values) locate
    the row within its local range: jet core vs. flank, warm vs. cold side
    of an SST front, peak vs. edge of an SSH anomaly — asymmetric structure
    that governs the surface->subsurface transfer, especially for the
    weaker cross-front u component. Ordering-invariant, zero layout risk;
    all-NaN blocks yield NaN which the standard NaN->mean imputation
    already absorbs (same path as spread).
    ...
    with warnings.catch_warnings():
        ...
        smin = np.concatenate(
            [np.nanmin(blk(i), axis=1, keepdims=True).astype(np.float32)
             for i in range(_N_BASE)], axis=1)
        smax = np.concatenate(
            [np.nanmax(blk(i), axis=1, keepdims=True).astype(np.float32)
             for i in range(_N_BASE)], axis=1)
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # PHYSICS + QUALITY + EXTREMES AUGMENTATION: append wind pseudo-stress
    # (|W|, |W|*u10, |W|*v10), geostrophic current-speed (|Ug|, |Ug|*ug,
    # |Ug|*vg) blocks per stencil cell, per-base-feature stencil spread
    # (nanstd over the 9 cells), per-base-feature stencil validity fractions,
    # and per-base-feature stencil min/max: 81 -> 171 columns, all before any
    ...
    # information, data-quality indicators, and the local field extremes that
    # locate each row within its mesoscale neighborhood.
    ...
    # validity-fraction, and min/max features appended, the linear fit can
    # represent the Ekman response to wind stress, speed-dependent
    ...
    # (mean-imputed) rows, and a response to local extremes; the MLP ensemble
    # then only has to learn the remaining nonlinear RESIDUAL.
