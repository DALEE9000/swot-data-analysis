...
# TAIL-TUNED RIDGE ALPHA (verified in gen019_02, the parent): the ridge
# backbone's regularization is selected per component from the grid below —
# solved on pre-tail Gram statistics alone, scored on the untouched
# temporal tail (the forward-in-time criterion the harness measures), then
# refit at the winner on the full span by summing the additive statistic
# blocks. The verified alpha=10 stays in the grid, so the backbone can only
# match or improve the frozen-alpha ancestor's.
_RIDGE_ALPHAS = (1.0, 3.0, 10.0, 30.0, 100.0, 300.0)
_RIDGE_MIN_VAL_ROWS = 1_000

# TAIL-STACKED COMBINE (THE ONE NEW CHANGE, verified +0.0005 in gen019_01):
# every ancestor of this parent averaged the ensemble members with EQUAL
# weights per component, yet member quality is measurably unequal and
# component-dependent (the tree member alone was worth +0.012; the
# non-baseline architectures are deliberately weaker fits). Per component,
# a non-negative least squares fit on the temporal validation tail learns
# the member weights instead: the design matrix holds each member's
# LEAK-FREE tail prediction — for MLPs the selected pre-fine-tune
# checkpoint (trained strictly before the tail), for the tree member the
# gated pre-tail fits — and the target is the post-ridge residual on the
# tail's valid rows. NNLS on a later-in-time tail directly optimizes the
# forward-in-time criterion the score measures, and because its weights
# need not sum to 1 it doubles as a variance-calibrating shrinkage of the
# residual correction, which MSE/R^2 rewards when member predictions are
# noisy. The fitted weights are shrunk _STACK_UNIFORM_W of the way back
# toward the uniform average to bound tail-overfitting, guarded on
# finiteness / a sane weight sum / a minimum tail row count, and applied
# to the recency-refit test predictions; on any guard failure the combine
# falls back to the exact verified equal-weight average, so this cannot
# lose the parent's behavior — it can only reweight it.
...
def _ridge_stats(Xs, y, idx, chunk=200_000):
    """Accumulate UNCENTERED Gram/moment statistics over rows `idx`:
    A = X^T X, g = X^T y, sx = column sums of X, sy = sum of y, n = rows.

    Uncentered blocks are ADDITIVE across row subsets, which is what lets
    the tuned ridge score alphas on pre-tail statistics and then refit on
    the full span by simply summing the pre-tail and tail blocks — one pass
    over each row, no recomputation.
    """
    d = Xs.shape[1]
    ...
    for i0 in range(0, len(idx), chunk):
        ...
        yc = y[ii].astype(np.float64)
        A += Xc.T @ Xc
        g += Xc.T @ yc
        sx += Xc.sum(axis=0)
        sy += float(yc.sum())
    return A, g, sx, sy, len(idx)


def _solve_ridge(A, g, sx, sy, n, alpha):
    """Closed-form ridge with an UNPENALIZED intercept from uncentered
    statistics: center the normal equations analytically (A - n*xbar*xbar^T,
    g - sx*ybar), add alpha to the diagonal, solve, and recover the
    intercept. Exact and deterministic. Returns (w (d,), b float)."""
    d = len(g)
    if n == 0:
        return np.zeros(d, dtype=np.float64), 0.0
    xbar = sx / n
    ybar = sy / n
    Ac = A - np.outer(sx, xbar)
    gc = g - sx * ybar
    Ac[np.diag_indices(d)] += alpha
    w = np.linalg.solve(Ac, gc)
    b = ybar - float(xbar @ w)
    return w, b


def _fit_ridge_tuned(Xs, Y_raw, mask, val_frac=0.1):
    """Fit the linear backbone with a per-component, tail-tuned alpha.

    Per component: (1) accumulate statistics over the valid PRE-TAIL rows,
    (2) solve the ridge at every alpha in _RIDGE_ALPHAS from those
    statistics alone and score each on the untouched temporal tail —
    selecting for forward-in-time generalization, exactly what the harness
    measures, (3) add the tail's statistic block and refit at the winning
    alpha on the FULL span. The verified alpha=10 is in the grid and is the
    fallback whenever the tail is too small or a solve misbehaves, so this
    cannot select a backbone worse than the frozen-alpha ancestor's on the
    criterion that matters. Returns (B (d, 2) float32, b (2,) float32).
    """
    n, d = Xs.shape
    n_val = max(1, int(n * val_frac))
    B = np.zeros((d, 2), dtype=np.float64)
    b_int = np.zeros(2, dtype=np.float64)
    for c in range(2):
        valid = np.flatnonzero(mask[:, c] > 0)
        rows_tr = valid[valid < n - n_val]
        rows_va = valid[valid >= n - n_val]
        if len(rows_tr) == 0 and len(rows_va) == 0:
            continue
        if len(rows_tr) == 0:
            rows_tr, rows_va = rows_va, rows_tr
        A_p, g_p, sx_p, sy_p, n_p = _ridge_stats(Xs, Y_raw[:, c], rows_tr)
        alpha_best = 10.0
        if len(rows_va) >= _RIDGE_MIN_VAL_ROWS:
            yv = Y_raw[rows_va, c].astype(np.float64)
            best_mse = float("inf")
            for alpha in _RIDGE_ALPHAS:
                try:
                    w, bi = _solve_ridge(A_p, g_p, sx_p, sy_p, n_p, alpha)
                except np.linalg.LinAlgError:
                    continue
                pv = np.empty(len(rows_va), dtype=np.float64)
                for i0 in range(0, len(rows_va), 200_000):
                    ii = rows_va[i0:i0 + 200_000]
                    pv[i0:i0 + 200_000] = Xs[ii].astype(np.float64) @ w + bi
                mse = float(np.mean((pv - yv) ** 2))
                if np.isfinite(mse) and mse < best_mse:
                    best_mse = mse
                    alpha_best = float(alpha)
        # Refit at the tuned alpha on ALL valid rows: uncentered statistic
        # blocks are additive, so the tail joins for the price of one pass.
        if len(rows_va) > 0:
            A_t, g_t, sx_t, sy_t, n_t = _ridge_stats(Xs, Y_raw[:, c], rows_va)
            A_p += A_t
            g_p += g_t
            sx_p += sx_t
            sy_p += sy_t
            n_p += n_t
        w, bi = _solve_ridge(A_p, g_p, sx_p, sy_p, n_p, alpha_best)
        B[:, c] = w
        b_int[c] = bi
    return B.astype(np.float32), b_int.astype(np.float32)
...
def _train_member(X_t, Y_t, M_t, X_v, Y_v, M_v, X_f, Y_f, M_f, delta, comp_w,
    ...
    biases instead of seed-noise-only clones. Validation checkpointing (raw
    vs EMA, temporal tail) is unchanged, so a weaker architecture ships its
    best early-stopped state rather than diluting the average.
    ...
    (float32, (n_val, 2)). That checkpoint never trained on the tail, so
    val_pred is the leak-free signal the tail-stacked combine fits member
    weights on."""
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # RIDGE BASELINE with TAIL-TUNED ALPHA (verified in the parent
    # gen019_02): fit the linear map features -> (u, v) in closed form,
    # selecting each component's regularization from _RIDGE_ALPHAS by
    # solving on pre-tail statistics and scoring on the temporal tail, then
    # refitting at the winner on the full span (see _fit_ridge_tuned). With
    # the stress, current-speed, stencil-spread, and validity-fraction
    # features appended, the linear fit can represent the Ekman response to
    # wind stress, speed-dependent attenuation/rotation of the geostrophic
    # current, a front-strength-modulated offset, and a per-feature
    # correction for partially observed rows; the nonlinear members then
    # only have to learn the remaining RESIDUAL — and a correctly shrunk
    # backbone leaves them the smallest possible one.
    B, b_int = _fit_ridge_tuned(Xs, Y_raw, mask, val_frac=0.1)
    ...
    # ridge-alpha selection / combine-weight stacking select for
    # forward-in-time generalization instead of memorization.
    n = len(Xs)
    ...
    # SEED + ARCHITECTURE ENSEMBLE within the REMAINING wall-clock budget
    # (after the GBM slice). Member 0 gets half of what's left AND the
    # verified architecture, so even under a tight clock the ensemble
    # contains at least one full-strength known-good model; members 1-2
    # split the remainder with diverse architectures for decorrelated views.
    # Each member's test prediction is kept SEPARATELY (not summed) so the
    # tail-stacked combine can weight them per component.
    slice_ends = [0.50, 0.78, 1.0]
    Xp = _standardize(X_test, mean, scale)
    lin_test = (Xp @ B + b_int).astype(np.float64)
    member_test = []   # per-member (n_test, 2) float64
    member_val = []    # per-member (n_val, 2) float64, leak-free tail preds
    ...
    # TAIL-STACKED COMBINE (the one new change, verified in gen019_01): per
    # component, learn non-negative member weights on the temporal
    # validation tail from the LEAK-FREE tail predictions (pre-fine-tune
    # MLP checkpoints, gated pre-tail GBM fits), shrink them
    # _STACK_UNIFORM_W of the way back toward the verified uniform average,
    # and apply them to the recency-refit test predictions. Guards on tail
    # size, finiteness, and a sane weight sum; any failure falls back to
    # the exact equal-weight combine of the parent.
    out = lin_test.copy()
    for c in range(2):
