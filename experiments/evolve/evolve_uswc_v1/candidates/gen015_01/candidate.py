...
# pre-tail fit and the verified recency refit on the full span. Boosting
# capacity stays at the VERIFIED fixed 100 iterations — the tail-tuned
# 300-iteration variant (gen014_03) measurably regressed.
_GBM_BUDGET_FRAC = 0.26
_GBM_MAX_ROWS = 500_000

# ARCHITECTURE-DIVERSE SEED ENSEMBLE (the one new change): the parent's three
# MLP members are byte-identical architectures whose errors are highly
# correlated (noted at gen014_01), so their average cancels little beyond SGD
# noise. Member 0 keeps the verified (256,256,128)/dropout-0.1 config exactly
# — it anchors the ensemble and gets half the clock, so a tight budget
# reproduces the parent's strongest single model. Members 1-2 get different
# inductive biases: a wide-then-narrowing funnel with heavier dropout, and a
# shallow-in/bottleneck-expanding shape with lighter dropout. Different
# widths, depth profiles, and regularization strengths converge to
# differently-shaped functions of the same residual, decorrelating errors so
# the equal-weight average cancels genuine model error, not just seed noise.
_MEMBER_CFGS = [
    ((256, 256, 128), 0.10),
    ((384, 192, 96), 0.15),
    ((192, 320, 128), 0.05),
...
def _train_member(X_t, Y_t, M_t, X_v, Y_v, M_v, X_f, Y_f, M_f, delta, comp_w,
                  n_inputs, hidden, dropout, member_seed, device, max_epochs,
                  t_end):
    ...
    the quantity the mean-R^2 metric measures. The member's architecture
    (hidden sizes, dropout) is supplied by the caller so ensemble members can
    carry different inductive biases.
    ...
    net = MLP(n_inputs, hidden=hidden, dropout=dropout).to(device)
    ...
    ema_net = MLP(n_inputs, hidden=hidden, dropout=dropout).to(device)
    ema_net.eval()
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # ARCHITECTURE-DIVERSE SEED ENSEMBLE within the REMAINING wall-clock
    # budget (after the GBM slice). Member 0 gets half of what's left AND the
    # verified parent architecture, so that even under a tight clock the
    # ensemble contains at least one full-strength model identical to the
    # parent's; members 1-2 split the remainder with different hidden shapes
    # and dropout rates (see _MEMBER_CFGS) so their errors decorrelate from
    # member 0's instead of merely re-sampling its SGD noise.
    ...
    for m, frac in enumerate(slice_ends):
        ...
        if m > 0 and remaining < 0.08 * remaining_total:
            break
        hidden, dropout = _MEMBER_CFGS[m % len(_MEMBER_CFGS)]
        net = _train_member(X_t, Y_t, M_t, X_v, Y_v, M_v, X_f, Y_f, M_f, delta,
                            comp_w, Xs.shape[1], hidden, dropout,
