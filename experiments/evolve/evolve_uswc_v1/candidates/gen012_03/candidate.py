...
def _augment_physics(X):
    ...
    stencil aggregates (mean/min/max).
    ...
    missingness indicators for mean-imputed rows.
    STENCIL AGGREGATES (the one new change): per base feature, the NaN-aware
    mean, min, and max across its 9 stencil cells. After standardization,
    missing cells are imputed to the column mean, so on partially observed
    swath-edge rows the per-cell columns understate the true local average —
    the nanmean restores it. Min and max expose local extremes (jet cores,
    eddy edges, SSH front amplitude) that the symmetric std cannot separate
    from mere roughness; transfer to subsurface flow plausibly keys on
    extreme rather than average forcing. Ordering-invariant, near-free, and
    all-NaN stencils fall back to NaN -> 0 via the standard imputation path.
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # NaN-aware stencil aggregates (nanmean/nanmin/nanmax): 81 -> 180 columns,
    # all before any statistics are computed, so the ridge and the MLP both
    # see quadratic wind forcing, speed-dependent transfer, local front-
    # strength regime information, explicit data-quality indicators, and
    # imputation-free local averages plus local extremes.
    ...
    # validity-fraction, and stencil-aggregate features appended, the linear
    ...
    # (mean-imputed) rows, and a response to true local averages/extremes;
