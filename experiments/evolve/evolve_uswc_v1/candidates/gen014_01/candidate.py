...
# GBM ensemble member (the one new change): wall-clock fraction reserved for
# the tree models, and the recency-capped row count they train on. Trees are
# fit on RAW features with NaNs intact (native missing-value handling) and
# are gated per component on the temporal validation tail.
_GBM_BUDGET_FRAC = 0.18
...
def _fit_gbm_members(X_raw, X_test_raw, Ys, mask, train_idx, val_idx, seed,
    ...
    residuals — a DECORRELATED model family for the ensemble.
    ...
    from theirs. Training rows are the most recent _GBM_MAX_ROWS valid rows
    of the pre-validation span (recency is a verified win in this lineage);
    the temporal validation tail stays untouched and GATES inclusion: a
    component's GBM joins the residual average only if it beats the
    zero-residual predictor (i.e., ridge alone) on that tail, so a failed
    fit dilutes nothing.
    ...
    for c in range(2):
        ...
        gbm = HistGradientBoostingRegressor(
            ...
            early_stopping=False, random_state=seed + 31 * c)
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # component, trained on raw NaN-bearing features, capped to a fixed
    # slice of the wall clock so the MLP ensemble keeps the lion's share.
