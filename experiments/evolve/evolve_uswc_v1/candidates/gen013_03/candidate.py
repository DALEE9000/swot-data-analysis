...
def _augment_physics(X):
    ...
    imputed one — these missingness indicators let the ridge learn
    per-feature offsets for partially observed rows and let the MLP gate its
    correction on data quality. Never NaN, ordering-invariant.
...
def _fit_ridge(Xs, Ys, mask, alpha=10.0, row_w=None):
    """Closed-form (optionally row-weighted) ridge per target component on
    the masked rows.

    Xs is standardized (NaN->0), so features are ~zero-mean; the intercept is
    absorbed by centering y (with the weighted mean when row_w is given).
    Returns (B, b): weights (d, 2) and intercepts (2,). Exact and
    deterministic — captures the linear (geostrophic + stress) part of the
    signal with no SGD noise.

    RECENCY WEIGHTING (the one new change): rows are flattened in cycle
    (time) order and the harness's test split is LATER cycles, while ocean
    regimes drift across the record (seasonal upwelling, eddy field, wind
    climate). row_w — exponential in chronological position, newest rows 3x
    the oldest, normalized to mean 1 so the effective alpha and residual
    scales are unchanged — tilts the linear baseline toward the regime
    nearest the test cycles. Implemented exactly via sqrt-weights in the
    chunked Gram accumulation: (sqrt(w)X)^T (sqrt(w)X) = X^T diag(w) X.
    This mirrors the lineage's verified recency fine-tune of the MLP, but
    applied to the deterministic linear backbone that carries most of the
    prediction.
    """
    d = Xs.shape[1]
    ...
    for c in range(2):
        ...
        if row_w is None:
            ...
            w_m = row_w[idx]
            ybar = float(np.sum(w_m * Ys[idx, c].astype(np.float64)) / w_m.sum())
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # RECENCY-WEIGHTED RIDGE BASELINE: fit the linear map features -> (u, v)
    # in closed form. Rows are in cycle (time) order and the test split is
    # later cycles; weight rows exponentially in chronological position so
    # the newest rows count 3x the oldest (mean-1 normalization keeps the
    # effective alpha and the residual scale unchanged). The linear fit thus
    # represents the Ekman response, speed-dependent transfer, front-strength
    # offset, and missingness corrections AS THEY HOLD in the regime nearest
    # the held-out cycles; the MLP ensemble then only has to learn the
    # remaining nonlinear RESIDUAL.
    n_rows = len(Xs)
    row_w = np.exp(np.log(3.0) * (np.arange(n_rows, dtype=np.float64)
                                  / max(1, n_rows - 1)))
    row_w /= row_w.mean()

    B, b_int = _fit_ridge(Xs, Y_raw, mask, alpha=10.0, row_w=row_w)
