...
# Known flattening layout: feature-major blocks of k*k stencil cells per base
# feature, base features ordered as
#   [mdt, ssha_filtered, ugos_filtered, vgos_filtered,
...
def _augment_wind_stress(X):
    """Append physics-derived wind-stress features per stencil cell.

    Wind forces surface currents through stress, tau ~ |U| * U (quadratic in
    wind speed), not through wind velocity itself. The Ekman (wind-driven)
    response dominates the ageostrophic part of u on the US West Coast, so
    giving the model tau_u = wu*|U|, tau_v = wv*|U| and |U| directly lets the
    closed-form ridge capture the wind-driven signal that a linear map of raw
    winds cannot. NaNs propagate into the new columns and are handled by the
    same imputation + indicator machinery as the raw features. If the column
    count doesn't match the expected feature-major layout, X is returned
    unchanged (graceful degradation).
    """
    X = X.astype(np.float32)
    d = X.shape[1]
    if d % N_BASE_FEATURES != 0:
        return X
    kk = d // N_BASE_FEATURES
    wu = X[:, ERA5_U_IDX * kk:(ERA5_U_IDX + 1) * kk]
    wv = X[:, ERA5_V_IDX * kk:(ERA5_V_IDX + 1) * kk]
    speed = np.sqrt(wu * wu + wv * wv)
    tau_u = wu * speed
    tau_v = wv * speed
    return np.concatenate([X, tau_u, tau_v, speed], axis=1)
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # WIND-STRESS PHYSICS FEATURES: tau ~ |U|*U per stencil cell (see
    # _augment_wind_stress). Applied BEFORE indicators/standardization so the
    # new columns flow through the exact same NaN machinery as raw features.
    Xa = _augment_wind_stress(X_train)

    # MISSINGNESS-AS-SIGNAL: NaN->0 imputation maps a padded stencil cell to
    # the feature MEAN, so swath-edge rows are indistinguishable from interior
    # rows with genuinely average neighbors — and both the ridge baseline and
    # the MLP fit a systematically biased map at edges. Append a binary
    # validity indicator for every column that is actually sometimes-missing
    # (always-valid columns would add constant, dead inputs and are skipped).
    # The ridge learns closed-form offset corrections per missingness pattern;
    # the MLP can gate neighbor features on whether they were real.
    fin_frac = np.isfinite(Xa).mean(axis=0)
    ind_cols = np.flatnonzero((fin_frac > 0.001) & (fin_frac < 0.999))

    def _with_indicators(X):
        X = X.astype(np.float32)
        if len(ind_cols) == 0:
            return X
        return np.concatenate(
            [X, np.isfinite(X[:, ind_cols]).astype(np.float32)], axis=1)

    Xa = _with_indicators(Xa)

    # Per-column standardization ignoring NaNs; NaN -> 0 (the feature mean).
    # Indicator columns are standardized identically (they have no NaNs).
    mean = np.nan_to_num(np.nanmean(Xa, axis=0), nan=0.0).astype(np.float32)
    scale = np.nanstd(Xa, axis=0)
    scale = np.where(np.isfinite(scale) & (scale > 1e-6), scale, 1.0).astype(np.float32)

    Xs = _standardize(Xa, mean, scale)
    del Xa
    ...
    # related to the surface current, and with the stress columns the linear
    # map now also spans the quadratic-in-wind Ekman response; the MLP
    # ensemble then only has to learn the remaining nonlinear RESIDUAL.
    ...
    Xp = _standardize(_with_indicators(_augment_wind_stress(X_test)), mean, scale)
