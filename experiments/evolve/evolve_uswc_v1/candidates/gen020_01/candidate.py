...
# Budgeted cosine LR (verified ~neutral in gen016_01, kept): each MLP member
# anneals its learning rate from _LR_MAX to _LR_MIN following a cosine over
# the elapsed wall-clock fraction of its own training slice, so every member
# finishes in a converged low-LR regime exactly when its slice expires.
_LR_MAX = 1e-3
_LR_MIN = 2e-5

# RECENCY LOSS WEIGHTING (kept from the parent): per-row loss weights ramp
# linearly from _RECENCY_W_MIN at the oldest training row to _RECENCY_W_MAX
# at the newest. Rows are flattened in cycle (time) order and the test split
# is LATER cycles. The endpoints average to 1, so the mean loss magnitude —
# and the tuned LR — is unchanged. Validation selection, GBM gating, the
# ridge fit, and the Huber deltas all use UNWEIGHTED statistics.
_RECENCY_W_MIN = 0.5
_RECENCY_W_MAX = 1.5

# DIVERSE GBM FAMILIES (THE ONE NEW CHANGE — verified in gen018_03 and
# carried by every stronger sibling): family 0 is the exact configuration
# this parent already runs and always fits first, so the verified tree
# member is never lost. Family 1 is deliberately diverse along two axes at
# once: a different function class (deeper 127-leaf trees with stronger
# shrinkage and heavier L2 regularization) and a different data view — a
# stride-2 window over the last 1M valid rows, i.e. twice the time span at
# the same row count. Both families run the verified gate-then-refit
# protocol independently against the untouched temporal tail, so a family
# that fails the gate dilutes nothing; the included families are AVERAGED
# into the single tree ensemble member the combine already expects. Under a
# tight clock family 1 is simply skipped.
_GBM_FAMILIES = [
...
def _masked_huber(pred, target, mask, delta, comp_w):
    ...
    metric.
    RECENCY WEIGHTS ride in through `mask`: during training the mask tensor
    is pre-multiplied by the per-row recency weight, so newer rows pull
    proportionally harder while the sum-normalization below keeps the loss
    scale invariant. Validation always passes the unweighted mask.
    """
    err = (pred - target).abs()
...
def _train_member(X_t, Y_t, M_t, X_v, Y_v, M_v, X_f, Y_f, M_f, delta, comp_w,
    ...
    residuals) on RECENCY-WEIGHTED masks (M_t/M_f carry the per-row linear
    recency ramp, so the whole optimization tilts toward the distribution
    the held-out later cycles are drawn from); validates and early-stops on
    the UNWEIGHTED component-balanced masked MSE (M_v), the quantity the
    mean-R^2 metric measures.

    Budgeted cosine LR (kept from gen016_01): before each epoch the learning
    rate is set to a cosine interpolation between _LR_MAX and _LR_MIN driven
    by the fraction of this member's wall-clock slice already spent, so the
    member is annealed to a low LR exactly as its slice expires.
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        # v equally in sigma units — but the robust (Huber) shape and the
        # recency weights must not leak into selection. Both the raw and the
        # EMA weights are scored; the better of the two becomes the
    ...
    # shape the final weights. M_f carries the recency ramp too, so the tail
    # rows (weight ~= _RECENCY_W_MAX) dominate this pass — consistent with
    # its purpose.
...
def _fit_gbm_members(X_raw, X_test_raw, Ys, mask, train_idx, val_idx, seed,
    ...
    gen018_03 — THE ONE NEW CHANGE grafted into this parent).
    ...
    under a tight clock family 1 is simply skipped and family 0 alone
    reproduces the parent's exact tree member.

    Family 1's stride-2 window covers 2x the time span of family 0 at the
    same row count — a different data view on top of the different function
    class, decorrelating the two fits along both axes.

    The GBM path uses the UNWEIGHTED mask throughout: its recency window is
    already its recency mechanism, and the gate must stay an unweighted
    measurement of tail MSE.

    The included families are AVERAGED into ONE tree prediction per
    component. Returns (pred_test (n_test, 2) float64, included (2,) bool).
    """
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # RECENCY LOSS WEIGHTS (kept from the parent): a linear ramp over the
    # chronological row index from _RECENCY_W_MIN (oldest) to _RECENCY_W_MAX
    # (newest), mean exactly 1. Folded into the TRAINING mask tensors only —
    # validation (M_v) and the GBM path keep the unweighted mask, so
    # selection and gating measure the same unweighted tail MSE as before.
    recency_w = np.linspace(_RECENCY_W_MIN, _RECENCY_W_MAX, n,
                            dtype=np.float32)[:, None]
    mask_w = (mask * recency_w).astype(np.float32)
    ...
    # Training masks carry the recency ramp; the validation mask does not.
    X_t = torch.from_numpy(Xs[train_idx])
    Y_t = torch.from_numpy(Ys[train_idx])
    M_t = torch.from_numpy(mask_w[train_idx])
    ...
    # passed the validation gate) the averaged tree member as one extra
