...
def _ema_update(ema_net, net, decay):
    """In-place EMA of parameters: ema <- decay*ema + (1-decay)*net.

    A free ensemble along the SGD trajectory — averages out the iterate noise
    of minibatch training on noisy geophysical targets. (The MLP has no
    running-stat buffers, so averaging parameters alone is sufficient.)
    """
    with torch.no_grad():
...
def _train_member(X_t, Y_t, M_t, X_v, Y_v, M_v, X_f, Y_f, M_f,
    ...
    t_end. Validation, early stopping, and the returned predictor all use the
    EMA weights. Returns the EMA net in eval mode."""
    ...
    ema_net.eval()
    ema_decay = 0.999  # ~1000-step (≈5-epoch-tail) horizon at bs=4096
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for i in range(0, n_train, bs):
            ...
            loss.backward()
            optimizer.step()
            _ema_update(ema_net, net, ema_decay)

        # Score the EMA weights: this is the model we actually deploy, so
        # early stopping and LR scheduling must select on its loss.
        with torch.no_grad():
            val_losses = []
    ...
    # Restart both raw and EMA nets from the best EMA snapshot.
    ...
    # shape the final weights. A SHORTER-horizon EMA (0.995, ~200 steps) keeps
    # that recency tilt while smoothing away last-batch jitter.
    ...
    for _ in range(ft_epochs):
        ...
        for b0 in range(0, n, block):
            ...
            for i in range(0, len(blk_t), bs):
                ...
                loss.backward()
                ft_opt.step()
                _ema_update(ema_net, net, ft_decay)
            if time.time() > t_end:
                break
