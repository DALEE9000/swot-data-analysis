...
# INPUT-NOISE AUGMENTATION (verified in the parent): std of the isotropic
# Gaussian noise added to standardized training inputs. Acts as a Jacobian
# penalty pushing the residual MLP toward locally flat functions that
# transfer across the temporal train->test feature drift, and reflects the
# real measurement noise in SWOT/ERA5/SST inputs. Validation stays clean.
_INPUT_NOISE_STD = 0.05

# GBM ensemble member (the one new change; verified +0.012 in gen014_01 and
# +0.005 for the recency refit in gen014_02): wall-clock fraction reserved
# for the tree models and the recency-capped row count they train on. Trees
# are fit on RAW features with NaNs intact (native missing-value handling)
# and are gated per component on the temporal validation tail. The budget
# fraction covers up to two fits per component: the gated pre-tail fit and
# a recency refit on the full span.
...
def _train_member(X_t, Y_t, M_t, X_v, Y_v, M_v, X_f, Y_f, M_f, delta, comp_w,
    ...
    residuals) on NOISE-AUGMENTED inputs; validates and early-stops on clean
    inputs with component-balanced masked MSE, the quantity the mean-R^2
    metric measures.

    INPUT-NOISE AUGMENTATION (verified in the parent): every training batch
    has isotropic Gaussian noise (std _INPUT_NOISE_STD in standardized units)
    added to its inputs. To first order this penalizes the squared input
    Jacobian of the network, selecting locally flat residual corrections
    that survive the temporal train->test feature drift; it also mirrors the
    genuine measurement noise in the SWOT/ERA5/SST inputs. Validation and
    prediction always use clean inputs. Returns the net in eval mode."""
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for i in range(0, n_train, bs):
            ...
            xb = xb + _INPUT_NOISE_STD * torch.randn_like(xb)
            optimizer.zero_grad()
            loss = _masked_huber(net(xb), yb, mb, delta_d, comp_w_d)
            loss.backward()
            optimizer.step()

        # Validation / checkpoint selection on CLEAN inputs with component-
        # balanced masked MSE: the score averages per-component R^2, so
        # selection must weigh u and v equally in sigma units — but neither
        # the robust (Huber) shape nor the training noise may leak into
        # selection.
        net.eval()
        with torch.no_grad():
            val_losses = []
    ...
    # shape the final weights. The same input-noise augmentation is applied
    # so the fine-tune keeps the flatness the main phase selected for.
...
def _fit_gbm_members(X_raw, X_test_raw, Ys, mask, train_idx, val_idx, seed,
    ...
    residuals — a DECORRELATED model family for the ensemble (the one new
    change here; verified gain in gen014_01/gen014_02).
    ...
    mean-imputation the noise-trained MLPs rely on, so its errors
    decorrelate from theirs.
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # GBM MEMBERS (the one new change): a tree-family residual model per
    # component, gated on the pre-tail fit and then recency-refit on the
