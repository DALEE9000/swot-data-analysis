...
def _augment_physics(X):
    ...
    stencil-spread features, stencil validity fractions, and RELATIVE-WIND
    pseudo-stress features.
    ...
    missingness indicators so mean-imputed swath-edge rows are
    distinguishable from genuinely quiescent interior rows.
    RELATIVE-WIND PSEUDO-STRESS (the one new change): the stress the ocean
    actually feels depends on the wind RELATIVE to the moving surface,
    tau ~ rho*Cd*|W-Ug|(W-Ug) — the "current feedback" / eddy-killing
    effect, an O(10%) modulation of momentum flux that is largest exactly
    where currents are strong (eddies, fronts), the regimes where residual
    error concentrates. Per stencil cell append |Wr|, |Wr|*wr_u, |Wr|*wr_v
    with Wr = (u10-ug, v10-vg). Its expansion contains wind-current coupling
    cross terms (|W|*ug-type products) that no linear combination of the
    existing absolute-stress and current-speed blocks can represent, so both
    the ridge and the MLP see a genuinely new, physically-correct signal.
    NaNs propagate and fall back to the column mean via standard imputation.
    ...
    # Relative wind: wind minus surface geostrophic current, per stencil cell.
    wr_u = u10 - ug
    wr_v = v10 - vg
    wr_spd = np.sqrt(wr_u * wr_u + wr_v * wr_v)
    ...
    return np.concatenate(
        [X,
         ...
         spread,
         valid_frac,
         wr_spd.astype(np.float32),
         (wr_spd * wr_u).astype(np.float32),
         (wr_spd * wr_v).astype(np.float32)], axis=1)
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # relative-wind pseudo-stress (|Wr|, |Wr|*wr_u, |Wr|*wr_v with
    # Wr = W - Ug) blocks per stencil cell, plus per-base-feature stencil
    # spread (nanstd over the 9 cells) and stencil validity fractions:
    # 81 -> 180 columns, all before any statistics are computed, so the ridge
    # and the MLP both see quadratic wind forcing, speed-dependent transfer,
    # current-feedback (relative-wind) stress, local front-strength regime
    # information, and explicit data-quality/missingness indicators.
    ...
    # the full training set. With the stress, current-speed, relative-wind
    # stress, stencil-spread, and validity-fraction features appended, the
    # linear fit can represent the Ekman response to the stress the ocean
    # actually feels (wind relative to the moving surface), speed-dependent
    ...
    # (mean-imputed) rows; the MLP ensemble then only has to learn the
