...
# DIVERSE GBM FAMILIES (THE ONE NEW CHANGE): ensemble diversity is the
# biggest verified lever in this lineage (the GBM family +0.012 in
# gen014_01, architecture-diverse MLPs +0.003 in gen018_02), yet the tree
# side is still a single model per component. Family 0 is the exact
...
# nothing. Under a tight clock family 1 is simply skipped and the program
# degrades to exactly the parent.
_GBM_FAMILIES = [
...
# each). Raised from 0.26 to cover the second family; this is a deadline,
# not a reservation — the MLP slices scale to the time actually remaining
# when the trees finish, so unused tree budget flows back to the MLPs.
...
def _fit_gbm_members(X_raw, X_test_raw, Ys, mask, train_idx, val_idx, seed,
    ...
    recency refit verified in gen014_02).
    ...
    DIVERSE FAMILIES (the one new change): _GBM_FAMILIES holds two
    configurations. Family 0 is the exact verified config on the verified
    recency window (stride 1) and runs completely first for both
    components, so a tight clock degrades to exactly the previous champion.
    Family 1 uses deeper trees with stronger shrinkage/regularization AND a
    strided window — every 2nd row of the last 2*_GBM_MAX_ROWS valid rows —
    so it fits a different function class on a twice-as-long time span at
    the same row count. Both diversity axes push its errors away from
    family 0's.
