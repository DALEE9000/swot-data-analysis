...
def _train_member(X_t, Y_t, M_t, X_v, Y_v, M_v, X_f, Y_f, M_f, delta, comp_w,
    ...
    ARCHITECTURAL DIVERSITY (the one new change): `hidden`/`dropout` are now
    per-member. Identical nets that differ only in seed converge to highly
    correlated functions, so averaging removes little variance; nets with
    different depth/width/regularization make decorrelated errors, which is
    exactly what an average exploits. Each config still early-stops on the
    same component-balanced temporal validation, so a config that suits this
    data less simply checkpoints at its own best epoch — the diversity is
    close to free. Returns the net in eval mode."""
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # HETEROGENEOUS SEED ENSEMBLE within the wall-clock budget. Member 0 gets
    # half the budget and keeps the verified (256, 256, 128)/0.1 config, so
    # even under a tight clock the ensemble contains at least one
    # full-strength model of the proven shape. Members 1-2 split the
    # remainder with DIFFERENT architectures and dropout — wider-shallower
    # (stronger low-level feature mixing, heavier regularization) and
    # deeper-narrower (more compositional depth, lighter regularization) —
    # so their residual-correction errors decorrelate from member 0's
    # instead of merely re-sampling the same function with a new seed.
    # Cumulative slice boundaries as budget fractions:
    member_cfgs = [
        (0.50, (256, 256, 128), 0.10),  # verified anchor config
        (0.78, (384, 256), 0.15),       # wide-shallow, more dropout
        (1.00, (192, 192, 192, 96), 0.05),  # deep-narrow, less dropout
    ]
    ...
    for m, (frac, hidden, dropout) in enumerate(member_cfgs):
        ...
        net = _train_member(X_t, Y_t, M_t, X_v, Y_v, M_v, X_f, Y_f, M_f, delta,
                            ...
                            max_epochs, t_end, hidden=hidden, dropout=dropout)
        with torch.no_grad():
