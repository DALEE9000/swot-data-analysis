...
def _augment_physics(X):
    ...
    anomaly-current-speed features, stencil-spread features, and stencil
    validity fractions.
    ...
    ANOMALY CURRENT SPEED (the one new change): the same quadratic treatment
    for the anomaly pair (ugosa, vgosa): per stencil cell append |Ua|,
    |Ua|*ua, |Ua|*va. |Ua| is the local eddy-intensity signal — mesoscale
    eddy kinetic energy is the regime variable that sets how deeply surface
    geostrophic anomalies imprint on subsurface flow, and drag/shear in the
    anomaly flow is quadratic exactly like in the mean flow. Reuses the
    twice-verified mechanism on the one velocity pair still entering only
    linearly.
    ...
    fraction of finite cells across its 9 stencil cells, letting downstream
    models distinguish genuinely mean-valued cells from mean-imputed ones.
    ...
    ua, va = blk(_IDX_UGOSA), blk(_IDX_UGOSA + 1)
    aspd = np.sqrt(ua * ua + va * va)
    ...
    return np.concatenate(
        [X,
         ...
         aspd.astype(np.float32),
         (aspd * ua).astype(np.float32),
         (aspd * va).astype(np.float32),
         spread,
         valid_frac], axis=1)
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # |W|*v10), geostrophic current-speed (|Ug|, |Ug|*ug, |Ug|*vg), anomaly
    # current-speed (|Ua|, |Ua|*ua, |Ua|*va) blocks per stencil cell, plus
    # per-base-feature stencil spread (nanstd over the 9 cells) and stencil
    # validity fractions: 81 -> 180 columns, all before any statistics are
    # computed, so the ridge and the MLP both see quadratic wind forcing,
    # speed-dependent transfer for both the mean and eddy flow, local
    ...
    # the full training set. With the stress, current-speed, anomaly-speed,
    # stencil-spread, and validity-fraction features appended, the linear fit
    # can represent the Ekman response to wind stress, speed-dependent
    # attenuation/rotation of both the mean and anomaly geostrophic currents,
    # a front-strength-modulated offset, and a per-feature correction for
    # partially observed (mean-imputed) rows; the MLP ensemble then only has
