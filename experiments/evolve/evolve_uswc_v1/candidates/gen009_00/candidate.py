...
def _train_member(X_t, Y_t, M_t, X_v, Y_v, M_v, X_f, Y_f, M_f, delta, comp_w,
    ...
    ARCHITECTURE DIVERSITY (the one new change): the member's hidden-layer
    configuration and dropout are supplied by the caller instead of being
    fixed. Seed-only ensembles of identical nets have highly correlated
    errors; varying width, depth, and regularization across members induces
    genuinely different inductive biases, decorrelating residual errors so
    the ensemble average cancels more variance on the temporally shifted
    test split. Returns the net in eval mode."""
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # Member 0 keeps the verified (256, 256, 128)/0.1 configuration and gets
    # half the budget so that, even under a tight clock, the ensemble
    # contains at least one full-strength known-good model. Members 1-2
    # split the remainder with a wider net (more capacity, stronger dropout)
    # and a deeper-narrower net (weaker dropout) — different widths, depths,
    # and regularization strengths give decorrelated errors, so averaging
    # cancels more variance than seed-only replicas of one architecture.
    # Cumulative slice boundaries as budget fractions:
    member_cfgs = [
        (0.50, (256, 256, 128), 0.10),
        (0.78, (384, 256, 128), 0.15),
        (1.00, (192, 192, 192, 96), 0.05),
    ]
