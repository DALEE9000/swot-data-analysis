...
def _fit_ridge(Xs, Ys, mask, alpha=10.0):
    ...
    of the signal on ALL training data with no SGD noise. Deliberately
    UNWEIGHTED in time: the linear physics it captures is time-stationary,
    so every cycle is equally informative for it.
    """
    d = Xs.shape[1]
...
def _masked_huber(pred, target, mask, delta):
    ...
    `mask` may carry fractional importance weights (recency weighting); the
    weighted-mean normalization handles that transparently.
    """
    err = (pred - target).abs()
...
def _train_member(X_t, Y_t, M_t, X_v, Y_v, M_v, X_f, Y_f, M_f, delta,
    ...
    t_end. Trains with recency-weighted masked Huber (robust to outlier
    residuals, emphasizing recent cycles); validates and early-stops on plain
    UNWEIGHTED masked MSE, the quantity the R^2 metric measures.
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        # Validation / checkpoint selection on plain UNWEIGHTED masked MSE:
        # the score is R^2 (an MSE quantity) on the held-out tail, so neither
        # the robust training loss nor the recency weights may leak into
        # model selection.
        net.eval()
        with torch.no_grad():
            val_losses = []
    ...
    # shape the final weights. M_f carries the recency weights too, so old
    # samples nudge the weights gently while recent ones dominate.
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # the full training set (unweighted — the linear physics is stationary).
    # With the stress and current-speed features appended, the linear fit can
    # represent both the Ekman response to wind stress and speed-dependent
    # attenuation/rotation of the geostrophic current, so this exact fit
    # captures the dominant signal deterministically; the MLP ensemble then
    # only has to learn the remaining nonlinear RESIDUAL.
    ...
    # component's post-ridge residual (unweighted). Residuals within ~1 sigma
    # get exact quadratic treatment; the heavy tail (HFR radar outliers, rare
    # ageostrophic spikes) gets linear gradients so it cannot dominate the
    # update the way squared error lets it.
    delta_np = np.array([
    ...
    # RECENCY WEIGHTING (the one new change): rows are flattened in cycle
    # (time) order and the test split is strictly LATER cycles, but plain
    # training treats a months-old sample as exactly as informative about the
    # test period as yesterday's. Mesoscale eddy positions and the seasonal
    # upwelling regime decorrelate over weeks, so the residual (the
    # non-stationary part left after the stationary ridge) is best learned
    # with an exponential time-decay importance weight: newest row -> 1.0,
    # oldest -> exp(-1) ~ 0.37, tau = the full training span. Folded into the
    # Huber loss MASK for the main loop and the fine-tune only; the ridge and
    # the validation criterion stay unweighted.
    age = (n - 1 - np.arange(n, dtype=np.float32)) / max(1, n - 1)  # 0=newest
    w_time = np.exp(-age).astype(np.float32)
    Mw = mask * w_time[:, None]

    # TEMPORAL validation split: hold out the last 10% of rows as a contiguous
    # tail so early stopping / LR scheduling select for forward-in-time
    # generalization instead of interleaved memorization.
    n_val = max(1, int(n * 0.1))
    train_idx = np.arange(0, n - n_val)
    val_idx = np.arange(n - n_val, n)

    # Build all tensors ONCE and share them across ensemble members.
    # Training/fine-tune masks carry the recency weights; validation does not.
    X_t = torch.from_numpy(Xs[train_idx])
    Y_t = torch.from_numpy(Ys[train_idx])
    M_t = torch.from_numpy(Mw[train_idx])
