...
def _augment_physics(X):
    ...
    mean across its 9 stencil cells. For fully observed rows this is a
    linear combination of the raw cells, but for partially observed
    swath-edge rows the raw cells are imputed to the GLOBAL column mean
    while the nanmean averages only the genuinely observed cells — it
    recovers the denoised local feature value exactly where imputation
    corrupts it, and its dependence on the per-row valid count makes it
    information the ridge cannot reconstruct from the imputed cells.
    Ordering-invariant, zero layout risk, near-free to compute.
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # stencil NaN-aware means: 81 -> 162 columns, all before any statistics
    ...
    # explicit data-quality indicators, and imputation-robust local averages.
    ...
    # (mean-imputed) rows, and an imputation-robust local average of each
    # base feature; the MLP ensemble then only has to learn the remaining
    # nonlinear RESIDUAL.
