...
def _augment_physics(X):
    ...
    stencil-spread features, stencil validity fractions, and NaN-aware
    stencil means.
    ...
    STENCIL MEANS (the one new change): per base feature, the NaN-aware mean
    across its 9 stencil cells. Mean imputation pulls every missing cell to
    the GLOBAL column mean, so any linear combination of raw cells — the
    local average a ridge would otherwise form — is corrupted toward the
    global mean exactly where SWOT coverage degrades. nanmean ignores the
    missing cells and provides the correct LOCAL estimate at swath edges,
    and in fully valid interiors it is a smoothed, denoised summary of the
    field that generalizes across time better than any single noisy cell.
    Complements the validity fraction (which says how much is missing, not
    what the corrected value is). Ordering-invariant, zero layout risk;
    all-missing rows yield NaN which standardization maps to the mean (0).
    ...
    with warnings.catch_warnings():
        ...
        stencil_mean = np.concatenate(
            [np.nanmean(blk(i), axis=1, keepdims=True).astype(np.float32)
             for i in range(_N_BASE)], axis=1)
...
def _train_member(X_t, Y_t, M_t, X_v, Y_v, M_v, X_f, Y_f, M_f, delta, comp_w,
    ...
    EMA weight averaging (from gen008_03): alongside the raw SGD iterates we
    maintain an exponential moving average of the weights (warmup decay
    ramping to 0.999). Each epoch BOTH the raw and the EMA weights are
    validated and the better one is checkpointed, so selection is a strict
    superset of the raw-only scheme. The recency fine-tune then starts from
    whichever weights were selected. Returns the net in eval mode."""
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        # leak into selection. Both the raw iterate and its EMA are scored;
        # the better one is checkpointed.
        net.eval()
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # per-base-feature stencil validity fractions, and per-base-feature
    # NaN-aware stencil means: 81 -> 162 columns, all before any statistics
    # are computed, so the ridge and the MLP both see quadratic wind forcing,
    # speed-dependent transfer, local front-strength regime information,
    # explicit data-quality/missingness indicators, and imputation-free local
    # field estimates at swath edges.
    ...
    # validity-fraction, and stencil-mean features appended, the linear fit
    # can represent the Ekman response to wind stress, speed-dependent
    # attenuation/rotation of the geostrophic current, a front-strength-
    # modulated offset, a per-feature correction for partially observed
    # (mean-imputed) rows, and an uncorrupted local average of each field;
    # the MLP ensemble then only has to learn the remaining nonlinear
    # RESIDUAL.
