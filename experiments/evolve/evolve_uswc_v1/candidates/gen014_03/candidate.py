...
# in gen014_02): wall-clock fraction reserved for the tree models and the
...
# the temporal validation tail. THE ONE NEW CHANGE: the boosting capacity is
# no longer a fixed guess (100) — the gating fit runs up to _GBM_MAX_ITER
# trees and the tail-optimal iteration count is selected via staged
# prediction on the untouched temporal tail; the recency refit then trains
# at exactly that validated count.
_GBM_BUDGET_FRAC = 0.30
...
def _fit_gbm_members(X_raw, X_test_raw, Ys, mask, train_idx, val_idx, seed,
    ...
    TAIL-TUNED CAPACITY (the one new change): the gating fit — on the most
    recent _GBM_MAX_ROWS valid rows of the PRE-validation span — now runs up
    to _GBM_MAX_ITER trees instead of a fixed 100, and staged predictions on
    the untouched temporal tail select the tail-optimal iteration count
    best_iter per component. That same staged curve is the inclusion gate:
    the component's GBM joins the residual average only if its best staged
    tail MSE beats the zero-residual predictor (ridge alone), so a failed
    family dilutes nothing. The recency refit (same hyperparameters, same
    window size, shifted forward to include the tail — the rows closest in
    time to the test cycles) then trains at exactly best_iter trees, giving
    the deployed model validation-selected capacity rather than a guess.

    Wall-clock safety: the measured cost of the gating fit projects the
    refit's cost (scaled by best_iter/_GBM_MAX_ITER); if that would blow the
    deadline, the fallback predicts the test set from the GATED model
    truncated to best_iter stages, so a tight clock never loses the member.
    If staged_predict is unavailable in this sklearn build, the code falls
    back to the parent's exact fixed-iteration gate-then-refit behavior.
    ...
    for c in range(2):
        ...
        gbm = _make_gbm(seed + 31 * c, _GBM_MAX_ITER)
        t_fit0 = time.time()
        gbm.fit(X_raw[rows], Ys[rows, c])
        fit_cost = time.time() - t_fit0
        vm = mask[val_idx, c] > 0
        if not vm.any():
            del gbm
            continue
        ...
        if hasattr(gbm, "staged_predict"):
            # Sweep the staged tail-MSE curve; its argmin is the validated
            # boosting capacity, its min is the gating statistic.
            best_iter, best_mse = 0, float("inf")
            X_val_c = X_raw[vv]
            for k, pv in enumerate(gbm.staged_predict(X_val_c), start=1):
                mse = float(np.mean((pv - y_val) ** 2))
                if mse < best_mse - 1e-12:
                    best_mse, best_iter = mse, k
        else:
            # Parent-equivalent fallback: gate on the full-depth prediction.
            best_iter = _GBM_MAX_ITER
            pv = gbm.predict(X_raw[vv])
            best_mse = float(np.mean((pv - y_val) ** 2))

        if best_mse < mse_zero and best_iter > 0:
            included[c] = True
            # Refit only if its projected cost fits inside the deadline;
            # otherwise predict from the gated model truncated to best_iter.
            refit_cost = fit_cost * best_iter / max(1, _GBM_MAX_ITER)
            if time.time() + refit_cost < deadline:
                full_rows = np.flatnonzero(mask[:, c] > 0)[-_GBM_MAX_ROWS:]
                gbm_r = _make_gbm(seed + 31 * c + 7, best_iter)
                gbm_r.fit(X_raw[full_rows], Ys[full_rows, c])
                pred_test[:, c] = gbm_r.predict(X_test_raw)
                del gbm_r
            elif hasattr(gbm, "staged_predict"):
                pv_test = None
                for pv_test in islice(gbm.staged_predict(X_test_raw), best_iter):
                    pass
                pred_test[:, c] = pv_test
            else:
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # GBM MEMBERS: a tree-family residual model per component with
    # tail-tuned boosting capacity, gated on the pre-tail fit's staged tail
    # curve and then recency-refit at the selected iteration count on the
    # full span (see _fit_gbm_members), capped to a fixed slice of the wall
    # clock so the MLP ensemble keeps the lion's share.
