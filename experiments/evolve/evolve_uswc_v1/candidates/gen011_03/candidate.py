...
def _train_member(X_t, Y_t, M_t, X_v, Y_v, M_v, X_f, Y_f, M_f, delta, comp_w,
    ...
    Returns (net, val_pred): the fine-tuned net in eval mode, plus the
    SELECTED (pre-fine-tune) checkpoint's predictions on the validation tail
    as a float32 numpy array. The val predictions are computed BEFORE the
    recency fine-tune touches the tail, so they are an uncontaminated
    out-of-sample measurement of how well this member's nonlinear correction
    transfers forward in time — exactly what the shrinkage fit needs."""
    ...
    # Record the selected checkpoint's predictions on the temporal validation
    # tail BEFORE fine-tuning trains on that tail. These feed the ensemble-
    # level shrinkage fit in train_and_predict.
    net.eval()
    with torch.no_grad():
        val_pred = np.concatenate(
            [net(X_v[i:i + bs]).cpu().numpy() for i in range(0, len(X_v), bs)],
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    val_pred_sum = np.zeros((len(val_idx), 2), dtype=np.float64)
    n_members = 0
    ...
    for m, frac in enumerate(slice_ends):
        ...
        val_pred_sum += val_pred.astype(np.float64)
        n_members += 1
        del net
        if device.type == "cuda":
            torch.cuda.empty_cache()

    # VALIDATION-FITTED SHRINKAGE (the one new change): the ensemble-mean
    # nonlinear correction is the component of the prediction most exposed to
    # the temporal train->test shift — the ridge part is a low-variance
    # linear fit, but the MLP correction can encode era-specific structure
    # that decays forward in time. Using each member's SELECTED
    # (pre-fine-tune) predictions on the held-out temporal tail — the only
    # labeled data from the future of the training era — solve per component
    # the closed-form least squares
    #     beta_c = <m * p, y> / <m * p, p>
    # of the mean correction p against the tail's true post-ridge residuals
    # y, then clip beta to [0, 1]: shrink-only, so beta=1 reproduces the
    # current program exactly and validation evidence can only temper, never
    # amplify, the correction. Fitting per component lets the tail
    # independently down-weight the weaker u correction without touching v.
    beta = np.ones(2, dtype=np.float64)
    if n_members > 0:
        P_val = val_pred_sum / n_members
        Y_val = Ys[val_idx].astype(np.float64)
        M_val = mask[val_idx].astype(np.float64)
        for c in range(2):
            den = float((M_val[:, c] * P_val[:, c] * P_val[:, c]).sum())
            if den > 1e-8:
                num = float((M_val[:, c] * P_val[:, c] * Y_val[:, c]).sum())
                beta[c] = min(1.0, max(0.0, num / den))

    out = (lin_test + beta * (pred_sum / max(1, n_members))).astype(np.float32)
