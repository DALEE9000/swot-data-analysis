...
# RECENCY LOSS WEIGHTING (THE ONE NEW CHANGE): per-row loss weights ramp
...
# is LATER cycles; every verified recency mechanism in this lineage (MLP
# recency fine-tune, GBM recency refit) confirms that time-proximity to the
# test cycles matters. This bakes that prior into the entire MLP training
# trajectory instead of only the final two fine-tune epochs. The endpoints
# average to 1, so the mean loss magnitude — and the tuned LR — is
# unchanged; the modest 3:1 tilt keeps effective sample size high.
# Validation selection, GBM gating, the ridge fit, and the Huber deltas all
# use UNWEIGHTED statistics, so checkpoint selection stays clean.
_RECENCY_W_MIN = 0.5
_RECENCY_W_MAX = 1.5
...
def _fit_gbm_members(X_raw, X_test_raw, Ys, mask, train_idx, val_idx, seed,
    ...
    shifted forward to include the tail, the rows closest in time to the
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # RECENCY LOSS WEIGHTS (the one new change): a linear ramp over the
