...
def _augment_physics(X):
    ...
    STENCIL SPREAD (the one new change): per base feature, the NaN-aware std
    across its 9 stencil cells. Linear stencil combinations (gradients,
    vorticity, strain) are already representable by the ridge, but the SPREAD
    — local front strength / mesoscale roughness — is nonlinear in the cells
    and is the regime variable that modulates surface->subsurface transfer
    (eddy edges and fronts carry enhanced ageostrophic shear). The std is
    invariant to the unknown stencil cell ordering, so it carries no layout
    risk; all-NaN rows yield NaN and fall back to the column mean via the
    standard imputation.
