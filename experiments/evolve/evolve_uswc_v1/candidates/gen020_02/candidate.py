...
# RECENCY-RIDGE DRIFT MEMBER (THE ONE NEW CHANGE): the ridge baseline —
# the single largest term in every prediction this lineage makes — has
# always been fit on the FULL training span with uniform weight, even
# though the test split is strictly later in time and the linear
# surface->subsurface transfer is seasonally nonstationary (wind regimes,
# stratification, mean-flow drift). A second closed-form ridge is fit on
# only the most recent _RIDGE_REC_ROWS pre-tail rows; the DIFFERENCE
# between its prediction and the full ridge's is a direct, deterministic
# estimate of coefficient/intercept drift, and it enters the tail-stacked
# combine as one extra NNLS column whose uniform-prior weight is ZERO. The
# temporal tail therefore decides, per component, how much drift
# correction to apply: if drift is real the column earns weight; if not,
# NNLS zeroes it and — like every guard-failure path — the combine is
# byte-identical to the parent. For the test-set column the drift ridge is
# recency-refit on the last _RIDGE_REC_ROWS rows of the FULL span (the
# verified gate/refit protocol: gate on pre-tail data, refit shifted
# forward to end at the rows nearest the test cycles). Cost is two chunked
# closed-form solves, seconds of CPU, stealing no time from any member.
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # RECENCY-RIDGE DRIFT MEMBER (the one new change), gate side: fit a
    ...
    # TAIL-STACKED COMBINE (verified in gen019_01), now with the drift
    # column: per component, learn non-negative member weights on the
    # temporal validation tail from the LEAK-FREE tail predictions
    # (pre-fine-tune MLP checkpoints, gated pre-tail GBM fits, and the
    # pre-tail recency-ridge drift), shrink them _STACK_UNIFORM_W of the
    # way back toward the uniform prior, and apply them to the recency-refit
    # test predictions. The drift column's uniform-prior weight is ZERO, so
    # under the shrinkage it enters at half its NNLS weight and every guard
    # failure (tail too small, non-finite, pathological weight sum, NNLS
    # exception) reproduces the parent's combine exactly.
    out = lin_test.copy()
    for c in range(2):
