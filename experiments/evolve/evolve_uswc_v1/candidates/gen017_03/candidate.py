...
# Cosine LR schedule (verified in gen016_01): each MLP member anneals its
# learning rate from _LR_MAX to _LR_MIN following a cosine over the ELAPSED
# WALL-CLOCK FRACTION of its own training slice, so every member finishes
# in a converged low-LR regime exactly when its slice expires.
_LR_MAX = 1e-3
_LR_MIN = 2e-5

# GBM stage (member verified +0.012 in gen014_01, recency refit verified in
# gen014_02): wall-clock fraction reserved for the tree models and the
# recency-capped row count they train on. Trees are fit on RAW features with
# NaNs intact (native missing-value handling) and are gated per component on
# the temporal validation tail. THE ONE NEW CHANGE: the GBM is now a
# BOOSTING STAGE rather than an averaged member — its pre-tail-fit
# predictions on the full training span define the residual the MLPs learn
# (see train_and_predict). The budget fraction covers the gating fit, the
# full-span train predict, and the recency refit per component.
_GBM_BUDGET_FRAC = 0.28
...
def _fit_gbm_stage(X_raw, X_test_raw, Ys, mask, train_idx, val_idx, seed,
                   deadline):
    """Fit one HistGradientBoostingRegressor per component on the post-ridge
    residuals — now a BOOSTING STAGE between the ridge and the MLPs (the one
    new change; the member itself verified +0.012 in gen014_01, the recency
    refit verified in gen014_02).
    ...
    mean-imputation the ridge and MLPs rely on, so what it captures is
    genuinely unavailable to them.

    Gate-then-refit (unchanged): the gating fit uses the most recent
    _GBM_MAX_ROWS valid rows of the PRE-validation span, so the untouched
    temporal tail can gate inclusion — a component's GBM enters the stack
    only if it beats the zero-residual predictor (ridge alone) on that tail,
    so a failed family contaminates nothing. Per component that PASSES the
    gate, the GBM is refit with identical hyperparameters on the most recent
    _GBM_MAX_ROWS valid rows of the FULL span and that refit model predicts
    the test set (deadline-guarded fallback to the pre-tail model).

    NEW: for gated components the PRE-TAIL model also predicts the ENTIRE
    training span. These predictions define the residual the MLPs will
    learn. Using the pre-tail model (never fit on the tail) keeps the
    temporal validation clean: tail rows get true out-of-sample GBM
    predictions, so MLP early stopping still measures forward-in-time
    generalization of the full stack.

    Returns (pred_test (n_test, 2) f64, pred_train (n_train, 2) f64,
    included (2,) bool).
    """
    pred_test = np.zeros((len(X_test_raw), 2), dtype=np.float64)
    pred_train = np.zeros((len(X_raw), 2), dtype=np.float64)
    ...
    for c in range(2):
        ...
        if mse_gbm < mse_zero:
            included[c] = True
            # Full-span train predictions from the PRE-TAIL model define the
            # MLP residual target (out-of-sample on the validation tail).
            pred_train[:, c] = gbm.predict(X_raw)
            pred_test[:, c] = gbm.predict(X_test_raw)
            # RECENCY REFIT for the TEST prediction only: same
            # hyperparameters, same window size, window shifted forward to
            # include the tail — the data closest to the test cycles. Only
            # attempted when the clock allows; otherwise the gated pre-tail
            # prediction above stands.
            if time.time() < deadline:
                ...
                del gbm_r
        del gbm
    return pred_test, pred_train, included
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # nonlinear stages then only have to learn the remaining RESIDUAL.
    ...
    # as a contiguous tail so early stopping / GBM gating select for
    ...
    # GBM STAGE: a tree-family residual model per component, gated on the
    # pre-tail fit and recency-refit on the full span for the test
    # prediction, capped to a fixed slice of the wall clock so the MLP
    # ensemble keeps the lion's share.
    gbm_deadline = t0 + _GBM_BUDGET_FRAC * time_budget_s
    gbm_pred_test, gbm_pred_train, gbm_included = _fit_gbm_stage(
        X_train, X_test, Ys, mask, train_idx, val_idx, seed, gbm_deadline)

    # STACK (the one new change): per gated component, subtract the pre-tail
    # GBM's train predictions from the residual targets. The MLPs now learn
    # the POST-GBM residual — the structure the trees provably cannot
    # capture — instead of independently re-learning the same post-ridge
    # residual the GBM already explains, which is what kept the averaged
    # members' errors correlated. Because the defining model never saw the
    # validation tail, tail residuals are true out-of-sample targets and MLP
    # early stopping remains a forward-in-time measurement of the full
    # stack. An ungated component keeps the plain post-ridge residual — the
    # parent's exact verified behavior.
    Ys_mlp = Ys.copy()
    for c in range(2):
        if gbm_included[c]:
            Ys_mlp[:, c] -= gbm_pred_train[:, c].astype(np.float32)
    Ys_mlp = np.where(mask > 0, Ys_mlp, 0.0).astype(np.float32)
    del gbm_pred_train

    # ROBUST-LOSS SCALE: one Huber delta per component, set to the std of that
    # component's MLP-stage residual (post-GBM where gated). Residuals within
    # ~1 sigma get exact quadratic treatment; the heavy tail (HFR radar
    # outliers, rare ageostrophic spikes) gets linear gradients so it cannot
    # dominate the update the way squared error lets it.
    delta_np = np.array([
        float(np.sqrt(np.mean(Ys_mlp[mask[:, c] > 0, c] ** 2))) if (mask[:, c] > 0).any() else 1.0
        for c in range(2)
    ], dtype=np.float32)
    delta_np = np.clip(delta_np, 1e-3, None)
    delta = torch.from_numpy(delta_np)

    # COMPONENT BALANCE WEIGHTS: inverse MLP-stage residual variance per
    ...
    # Build all tensors ONCE and share them across ensemble members. The MLP
    # targets are the stacked residuals Ys_mlp.
    X_t, Y_t, M_t = (torch.from_numpy(a[train_idx]) for a in (Xs, Ys_mlp, mask))
    X_v = torch.from_numpy(Xs[val_idx]).to(device)
    Y_v = torch.from_numpy(Ys_mlp[val_idx]).to(device)
    M_v = torch.from_numpy(mask[val_idx]).to(device)
    X_f = torch.from_numpy(Xs)
    Y_f = torch.from_numpy(Ys_mlp)
    ...
    # COMBINE as a boosting chain: ridge baseline + (per gated component) the
    # GBM stage's recency-refit prediction + the mean of the MLP members'
    # post-GBM residual corrections. No 1/(n+1) dilution of the tree
    # contribution — the MLPs were trained to correct it, not to compete
    # with it. Ungated components reduce to ridge + mean(MLPs), exactly the
    # parent's verified fallback.
    out = lin_test.copy()
    for c in range(2):
        if gbm_included[c]:
            out[:, c] += gbm_pred_test[:, c]
        out[:, c] += pred_sum[:, c] / max(1, n_members)
