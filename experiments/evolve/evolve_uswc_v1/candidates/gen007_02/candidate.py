...
def _augment_physics(X):
    ...
    VALIDITY FRACTIONS (the one new change): per base feature, the fraction
    of finite cells across its 9 stencil cells. After standardization every
    NaN cell is imputed to the column mean, so downstream models cannot
    tell a genuinely mean-valued cell from an imputed one — swath-edge rows
    masquerade as quiescent interior rows. These missingness indicators let
    the ridge learn per-feature offsets for partially observed rows and let
    the MLP gate its correction on data quality. Never NaN, ordering-
    invariant, zero layout risk.
