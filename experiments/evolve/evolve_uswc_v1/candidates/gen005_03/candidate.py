...
def _augment_wind(X):
    """Append wind-stress-like features derived from the ERA5 wind columns.

    Ekman/upwelling currents respond to wind STRESS tau ~ rho*Cd*|U|*U — a
    quadratic function of the 10 m wind — while the raw inputs are only the
    linear components. For each stencil cell append |U|, |U|*u10, |U|*v10
    (27 new columns) so the linear ridge can represent the stress -> current
    map (including its rotation) exactly, and the MLP gets it for free.
    NaNs propagate and are handled by the standard NaN->0 imputation.
    ...
    u10 = X[:, _IDX_ERA5_U * n_cells:(_IDX_ERA5_U + 1) * n_cells]
    v10 = X[:, (_IDX_ERA5_U + 1) * n_cells:(_IDX_ERA5_U + 2) * n_cells]
    spd = np.sqrt(u10 * u10 + v10 * v10)
    return np.concatenate(
        [X, spd.astype(np.float32),
         (spd * u10).astype(np.float32),
         (spd * v10).astype(np.float32)], axis=1)
...
def _fit_ridge(Xs, Ys, mask, alpha=10.0):
    ...
    Exact and deterministic — captures the linear (geostrophic + Ekman-stress)
    part of the signal on ALL training data with no SGD noise.
    """
    d = Xs.shape[1]
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # WIND-STRESS AUGMENTATION: append |U|, |U|*u10, |U|*v10 per stencil cell
    # (81 -> 108 columns) before any statistics are computed, so the ridge and
    # the MLP both see the quadratic wind forcing that drives the cross-shore
    # (u) Ekman/upwelling response.
    X_train = _augment_wind(X_train)
    X_test = _augment_wind(X_test)
    ...
    # related to the surface current — and with the stress features appended,
    # the linear Ekman response to wind stress is representable too — so this
    # exact fit captures the dominant signal deterministically; the MLP
    # ensemble then only has to learn the nonlinear RESIDUAL.
