...
def _augment_physics(X):
    ...
    eddy-speed features, stencil-spread features, and stencil validity
    fractions.
    ...
    EDDY SPEED (the one new change): the same transformation applied to the
    ageostrophic ANOMALY pair (ugosa, vgosa). |Ua| = sqrt(ua^2 + va^2) is a
    per-cell eddy-kinetic-energy proxy — the regime variable that governs how
    strongly surface eddies project to subsurface flow — and |Ua|*ua, |Ua|*va
    give the quadratic self-advection-like terms. Cross-shore (u) flow on the
    USWC is eddy-dominated, so this targets the weaker component. Exactly the
    feature class already verified twice (wind stress, current speed), applied
    to the one velocity pair it was never applied to.
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # |W|*v10), geostrophic current-speed (|Ug|, |Ug|*ug, |Ug|*vg), and eddy
    # anomaly-speed (|Ua|, |Ua|*ua, |Ua|*va) blocks per stencil cell, plus
    ...
    # speed-dependent transfer, eddy-intensity (EKE-proxy) regime information,
    # local front strength, and explicit data-quality/missingness indicators.
    ...
    # the full training set. With the stress, current-speed, eddy-speed,
    ...
    # attenuation/rotation of the geostrophic current, eddy-intensity-
    # modulated transfer of the anomaly flow, a front-strength-modulated
