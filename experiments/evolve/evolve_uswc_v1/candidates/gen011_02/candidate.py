...
# WEIGHT EMA (the one new change): decay of the exponential moving average of
# MLP weights, updated after every optimizer step. 0.999 with ~220 batches
# per epoch averages over roughly the last 4-5 epochs of the trajectory —
# long enough to cancel SGD batch noise, short enough to track the schedule.
_EMA_DECAY = 0.999
...
def _train_member(X_t, Y_t, M_t, X_v, Y_v, M_v, X_f, Y_f, M_f, delta, comp_w,
    ...
    WEIGHT EMA (the one new change): an exponential moving average of the
    network weights (decay _EMA_DECAY) is updated after every optimizer
    step. Each epoch BOTH the raw net and the EMA net are scored on the
    clean validation tail, and the checkpoint keeps whichever is better.
    Averaging along the SGD trajectory cancels batch-gradient noise and
    selects flatter minima that transfer across the temporal train->test
    drift; because the raw net remains a selection candidate, the scheme
    cannot underperform the original checkpointing. Returns the net (loaded
    with the best-found weights) in eval mode."""
