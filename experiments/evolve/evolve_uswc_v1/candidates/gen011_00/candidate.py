...
def _masked_huber(pred, target, mask, delta, comp_w):
    ...
    `mask` may carry per-row SAMPLE WEIGHTS (validity 0/1 times a recency
    weight): the sum-over-mask normalization then computes a weighted mean,
    so weighting needs no other code changes.
...
def _train_member(X_t, Y_t, M_t, X_v, Y_v, M_v, X_f, Y_f, M_f, delta, comp_w,
    ...
    residuals) on RECENCY-WEIGHTED masks — M_t/M_f are validity masks scaled
    by a per-row weight ramping from 0.6 (oldest) to 1.4 (newest), so the
    optimizer favors the temporal regimes closest to the held-out test
    cycles throughout training, not just in the final fine-tune. Validation
    and early stopping use the UNWEIGHTED masked MSE on the temporal tail
    (M_v), the exact forward-in-time quantity the mean-R^2 metric measures.

    EMA weight averaging (verified in gen008_03): alongside the raw SGD
    iterates we maintain an exponential moving average of the weights
    (warmup decay ramping to 0.999). Each epoch BOTH the raw and the EMA
    weights are validated and the better one is checkpointed. The recency
    fine-tune then starts from whichever weights were selected.
    Returns the net in eval mode."""
    ...
    # EMA shadow of the network. Parameters only (LayerNorm keeps its affine
    # weights as parameters; this net has no running-stat buffers). Kept in
    # eval mode and never trained directly.
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for i in range(0, n_train, bs):
            ...
            loss.backward()
            optimizer.step()
            # EMA update with warmup: decay ramps (1+s)/(10+s) -> 0.999 so
            # early steps do not anchor the average at the random init.
            ema_step += 1
            d_ema = min(0.999, (1.0 + ema_step) / (10.0 + ema_step))
            with torch.no_grad():
                for pe, p in zip(ema.parameters(), net.parameters()):
                    pe.mul_(d_ema).add_(p.detach(), alpha=1.0 - d_ema)
        ...
        # v equally in sigma units — but neither the robust (Huber) shape nor
        # the recency weights may leak into selection. Both the raw iterate
        # and its EMA are scored; the better one is checkpointed.
        net.eval()
        val_raw = _val_loss(net)
        val_ema = _val_loss(ema)
        val_loss = min(val_raw, val_ema)
        scheduler.step(val_loss)

        if val_loss < best_val - 1e-7:
            best_val = val_loss
            src = ema if val_ema <= val_raw else net
    ...
    # shape the final weights. M_f carries the same recency weights, so the
    # fine-tune's emphasis is consistent with the main phase.
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # The ridge stays UNWEIGHTED: it anchors the stable, physics-dominated
    # linear part, while temporal drift is handled by the weighted MLP.
    ...
    # RECENCY WEIGHTS (the one new change): rows are in cycle/time order and
    # the test split is strictly LATER cycles, so the target distribution is
    # the future of the training distribution. The verified recency fine-tune
    # shows late-period samples matter more; this makes that preference
    # explicit for the WHOLE optimization. Per-row weight ramps linearly
    # 0.6 (oldest) -> 1.4 (newest): mild (2.3x range, mean ~1 so the tuned
    # LR and loss magnitude are unchanged), every sample stays in play, and
    # the weight is folded into the validity mask so the masked losses turn
    # into weighted means with no loss-function changes. Validation masks
    # remain unweighted — selection must measure, not prefer.
    w_time = (0.6 + 0.8 * (np.arange(n, dtype=np.float32) / max(1, n - 1)))
    mask_w = mask * w_time[:, None]

    # Build all tensors ONCE and share them across ensemble members.
    # Training/fine-tune masks carry the recency weights; the validation
    # mask stays pure 0/1.
