...
class _EMA:
    """Exponential moving average of a network's weights (Polyak averaging).

    THE ONE NEW CHANGE this generation. SGD iterates on a noisy masked-Huber
    residual objective bounce around the loss basin; the temporally held-out
    test split punishes exactly that iterate noise. The EMA shadow sits at a
    flatter, averaged point in the basin and generalizes better forward in
    time. Validation, checkpoint selection, and the final model all use the
    shadow, never the raw iterate. The warm-up ramp min(decay, (1+n)/(10+n))
    avoids the shadow lagging at cold start. Cost: one fused mul_/add_ over
    ~150k params per batch — negligible.
    """
...
def _train_member(X_t, Y_t, M_t, X_v, Y_v, M_v, X_f, Y_f, M_f, delta, comp_w,
    ...
    residuals); validates, early-stops, and checkpoints on the EMA (Polyak-
    averaged) weights, scored with component-balanced masked MSE — the
    quantity the mean-R^2 metric measures. Returns the net (EMA weights
    loaded) in eval mode."""
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        # Validation / checkpoint selection on the EMA weights, scored with
        # component-balanced masked MSE: the score averages per-component
        # R^2, so selection must weigh u and v equally in sigma units — but
        # neither the robust (Huber) shape nor the raw SGD iterate may leak
        # into selection. Swap in the shadow, evaluate, swap back.
        raw_state = copy.deepcopy(net.state_dict())
        net.load_state_dict(ema.state_dict())
        net.eval()
        with torch.no_grad():
            val_losses = []
    ...
    # shape the final weights. The EMA restarts from the selected weights
    # with a SHORTER horizon (decay 0.995 ~ 200 batches < one full pass), so
    # averaging still suppresses iterate noise without washing out the
    # recency emphasis of the oldest->newest sweep.
