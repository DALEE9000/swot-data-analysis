...
# ROTATED-TARGET MEMBER (THE ONE NEW CHANGE): ensemble diversity is the only
# lever with large verified gains in this lineage (decorrelated GBM families
# +0.012 and +0.004, architecture-diverse MLPs +0.003), and it has so far
# been applied to the model family, the architecture, and the data view —
# never to the TARGET basis. Member _ROT_MEMBER trains on the post-ridge
# residuals rotated 45 degrees in the (u, v) plane — it predicts
# (u+v)/sqrt(2) and (v-u)/sqrt(2) — and its predictions are rotated back
# before entering the average. Velocity is a vector field, so a coordinate
# rotation is exactly as physical as the original basis (for USWC roughly an
# along-/across-shore decomposition); a learner whose loss, component
# balancing, and early stopping all operate along different axes develops
# per-component errors that decorrelate from every axis-aligned member in a
# way seed or width changes cannot. Rotated targets require BOTH components
# valid, so the member gets its own joint-validity mask and its own rotated
# Huber deltas / component weights; if fewer than _ROT_MIN_ROWS joint-valid
# rows exist it silently falls back to the unrotated targets and the program
# degrades to exactly gen018_03. Members 0-1, the tree side, and the combine
# are untouched.
_ROT_MEMBER = 2
...
# families are AVERAGED into the single tree ensemble member, so the
# verified MLP:tree combine ratio is untouched and a failed family dilutes
# nothing. Under a tight clock family 1 is simply skipped.
_GBM_FAMILIES = [
...
def _fit_gbm_members(X_raw, X_test_raw, Ys, mask, train_idx, val_idx, seed,
    ...
    DIVERSE FAMILIES: _GBM_FAMILIES holds two configurations. Family 0 is
    the exact verified config on the verified recency window (stride 1) and
    runs completely first for both components, so a tight clock degrades to
    exactly the previous champion. Family 1 uses deeper trees with stronger
    shrinkage/regularization AND a strided window — every 2nd row of the
    last 2*_GBM_MAX_ROWS valid rows — so it fits a different function class
    on a twice-as-long time span at the same row count.
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # ROTATED-BASIS TARGETS (the one new change): build the 45-degree-rotated
    # residuals, joint-validity mask, and rotated deltas/component weights
    # for member _ROT_MEMBER. Rotation mixes u and v, so a rotated target is
    # valid only where BOTH components are; the joint mask is applied to
    # both rotated columns. Enabled only when enough joint-valid rows exist
    # — otherwise the member silently trains on the unrotated targets and
    # the program is byte-equivalent to gen018_03.
    c_, s_ = _ROT_C, _ROT_C
    ...
    for m, frac in enumerate(slice_ends):
        ...
        if use_rot:
            net = _train_member(X_t, Yr_t, Mr_t, X_v, Yr_v, Mr_v, X_f, Yr_f,
                                Mr_f, delta_rot, comp_w_rot, Xs.shape[1],
                                hidden, seed + 1000 * m, device, max_epochs,
                                t_end)
        else:
            net = _train_member(X_t, Y_t, M_t, X_v, Y_v, M_v, X_f, Y_f, M_f,
                                delta, comp_w, Xs.shape[1], hidden,
