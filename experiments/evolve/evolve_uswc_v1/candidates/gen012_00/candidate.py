...
def _augment_physics(X):
    ...
    fraction of finite cells across its 9 stencil cells, so downstream
    models can tell a genuinely mean-valued cell from an imputed one at
    swath edges.
    NAN-AWARE STENCIL MEANS (the one new change): per base feature, the
    nanmean across its 9 stencil cells. After standardization every NaN
    cell is imputed to the column mean, so any local average a downstream
    model forms from the raw cells is biased toward the global mean in
    proportion to how many cells are missing — worst exactly at swath
    edges. The nanmean is an unbiased estimate of the true local field
    from the valid cells only; combined with the validity fraction it
    gives the ridge and the MLP a coverage-independent local value. For
    fully observed rows it is a redundant linear combination (harmless
    under ridge regularization); for partial rows it is new information.
    Ordering-invariant, zero layout risk; NaN only when a whole block is
    missing, which standardization already imputes to 0.
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # explicit data-quality indicators, and coverage-independent local field
    # values at partially observed swath-edge rows.
    ...
    # validity-fraction, and NaN-aware stencil-mean features appended, the
    # linear fit can represent the Ekman response to wind stress, speed-
    # dependent attenuation/rotation of the geostrophic current, a front-
    # strength-modulated offset, a per-feature correction for partially
    # observed (mean-imputed) rows, and an unbiased local field estimate at
    # swath edges; the MLP ensemble then only has to learn the remaining
    # nonlinear RESIDUAL.
