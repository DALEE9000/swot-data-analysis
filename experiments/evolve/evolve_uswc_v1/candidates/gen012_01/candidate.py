...
def _augment_physics(X):
    ...
    stencil-spread features, stencil validity fractions, and stencil
    nanmeans.
    ...
    fraction of finite cells across its 9 stencil cells — explicit
    missingness indicators for the mean-imputed cells.
    STENCIL NANMEAN (the one new change): per base feature, the NaN-aware
    mean across its 9 stencil cells. Any linear local average the ridge can
    form from the raw cells is corrupted on partially observed rows, because
    NaN cells are imputed to the GLOBAL column mean and drag the average
    toward climatology exactly where SWOT coverage degrades. The nanmean is
    the uncorrupted local level of each base field — the first-order
    smoothed input to the surface->subsurface transfer — complementing the
    nanstd (roughness) and validity fraction (quality) siblings that both
    measurably helped. Ordering-invariant, computed before standardization,
    near-free.
...
def _train_member(X_t, Y_t, M_t, X_v, Y_v, M_v, X_f, Y_f, M_f, delta, comp_w,
    ...
    EMA weight averaging (verified neutral-or-better in gen008_03): alongside
    the raw SGD iterates we maintain an exponential moving average of the
    weights (warmup decay ramping to 0.999). Averaged iterates cancel SGD
    noise and sit in flatter regions of the loss surface. Each epoch BOTH the
    raw and the EMA weights are validated and the better one is checkpointed,
    so selection is a strict superset of the raw-only scheme. The recency
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # stencil nanmeans (the NaN-robust local level of each field):
    # 81 -> 162 columns, all before any statistics are computed, so the ridge
    # and the MLP both see quadratic wind forcing, speed-dependent transfer,
    # local front-strength regime information, explicit data-quality
    # indicators, and imputation-free local averages.
    ...
    # validity-fraction, and stencil-nanmean features appended, the linear
    # fit can represent the Ekman response to wind stress, speed-dependent
    ...
    # (mean-imputed) rows, and an uncorrupted local-average response; the
