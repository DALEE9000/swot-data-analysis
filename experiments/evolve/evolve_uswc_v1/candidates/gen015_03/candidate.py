...
# RECENCY-BLENDED RIDGE (the one new change): the ridge baseline is the
# largest single term in every prediction and defines the residual targets
# for every nonlinear member, yet it was fit stationarily over the full
# span. The linear surface->subsurface map drifts seasonally (wind-driven
# upwelling regime on the USWC), so a second ridge is fit on the most
# recent _RIDGE_RECENT_ROWS pre-tail rows, a per-component blend weight
# over _RIDGE_ALPHAS is selected on the untouched temporal tail (alpha=0
# reproduces the parent exactly, so the gate cannot pick something worse
# on the selection measure), and the recent ridge is then refit on a
# window shifted forward to include the tail — the rows closest in time
# to the test cycles — before the blend is deployed.
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # contiguous tail so ridge blending / early stopping / LR scheduling /
    ...
    # RECENCY-BLENDED RIDGE (the one new change): gate-then-refit, exactly
    # the pattern verified for the GBM member. A second ridge is fit on the
    # most recent _RIDGE_RECENT_ROWS rows of the PRE-tail span; per
    # component, blends between the full-span and recent ridge are scored on
    # the untouched temporal tail and the tail-optimal alpha is selected
    # (alpha=0 — the full-span ridge alone — is the default, so the gate
    # cannot pick something worse than the parent on the selection measure).
    # For any component with alpha > 0 the recent ridge is REFIT on a window
    # shifted forward to include the tail (the rows closest in time to the
    # test cycles) and the deployed blend uses that refit. The blended ridge
    # then defines both lin_test and the residual targets, so every
    # nonlinear member automatically corrects the better baseline.
    B, b_int = B_full.copy(), b_full.copy()
    rec_gate = train_idx[-_RIDGE_RECENT_ROWS:]
    alpha_sel = np.zeros(2, dtype=np.float64)
    if len(rec_gate) >= 50_000 and len(val_idx) > 0:
        ...
        for c in range(2):
            ...
            best_mse = float(np.mean((pf - yv) ** 2))
            for a in _RIDGE_ALPHAS:
                mse = float(np.mean(((1.0 - a) * pf + a * pr - yv) ** 2))
                if mse < best_mse - 1e-12:
                    best_mse, alpha_sel[c] = mse, a
        if alpha_sel.any():
            rec_full = np.arange(n)[-_RIDGE_RECENT_ROWS:]
            B_rr, b_rr = _fit_ridge(Xs, Y_raw, mask, alpha=10.0, rows=rec_full)
            for c in range(2):
                a = np.float32(alpha_sel[c])
                if a > 0:
                    B[:, c] = (1.0 - a) * B_full[:, c] + a * B_rr[:, c]
                    b_int[c] = (1.0 - a) * b_full[c] + a * b_rr[c]
    ...
    # of the exact (recency-blended) ridge baseline.
    out = lin_test.copy()
    for c in range(2):
