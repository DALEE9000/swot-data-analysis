...
def _ema_update(ema_net, net, step, max_decay=0.999):
    """In-place EMA of parameters. Dynamic decay min(max, (1+t)/(10+t)) so the
    average tracks fast at the start of training and settles to a ~1000-step
    horizon later — a temporal ensemble of the SGD trajectory whose flat-minimum
    bias is what survives the forward-in-time distribution shift."""
    d = min(max_decay, (1.0 + step) / (10.0 + step))
    with torch.no_grad():
        for p_e, p in zip(ema_net.parameters(), net.parameters()):
            p_e.mul_(d).add_(p, alpha=1.0 - d)
...
def _train_member(X_t, Y_t, M_t, X_v, Y_v, M_v, X_f, Y_f, M_f, delta,
    ...
    early-stops, and PREDICTS with the EMA of the weights, scored by plain
    masked MSE — the quantity the R^2 metric measures.
    Returns the EMA net in eval mode."""
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for i in range(0, n_train, bs):
            ...
            optimizer.step()
            step += 1
            _ema_update(ema_net, net, step)

        # Validation / checkpoint selection: EMA weights scored with plain
        # masked MSE. The EMA's validation curve is smooth, so patience-based
        # stopping selects a genuinely better model rather than a lucky epoch,
        # and MSE keeps selection aligned with the R^2 metric.
        with torch.no_grad():
            val_losses = []
            for i in range(0, len(X_v), bs):
                val_losses.append(_masked_mse(ema_net(X_v[i:i + bs]), Y_v[i:i + bs], M_v[i:i + bs]).item())
    ...
    # Restart both the live net and the EMA from the selected (averaged)
    # checkpoint, then recency fine-tune.
    ...
    # above. Briefly fine-tune on the FULL dataset at low LR, sweeping
    # oldest -> newest within each pass so the freshest samples shape the
    # final weights; the EMA keeps averaging so the prediction weights stay
    # in the flat basin instead of jumping to the last noisy fine-tune step.
