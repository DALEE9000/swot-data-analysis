...
# TAIL-TUNED RIDGE ALPHA (THE ONE NEW CHANGE): the ridge backbone's
# regularization has been fixed at alpha=10 for the entire lineage while
# the feature space grew from 81 raw columns to 153 columns of very
# different signal-to-noise (raw stencil cells, quadratic stress products,
# spread/validity summaries). Every downstream member fits this ridge's
# residual, so a mis-shrunk backbone leaks linear-explainable variance
# into the noisy SGD/tree fits. Per component, Gram/moment statistics are
# accumulated separately over the pre-tail span and the temporal tail; the
# grid below is solved from the PRE-TAIL statistics alone and scored on
# the untouched tail (the forward-in-time criterion the harness measures);
# the winning alpha is then refit in closed form on the FULL span by
# summing the two statistic blocks, so no data is discarded. The verified
# alpha=10 stays in the grid and ~100k tail rows judge only 6 candidates,
# so tail-overfit risk is negligible and the program can only match or
# improve the parent's backbone.
...
def _fit_ridge_tuned(Xs, Y_raw, mask, val_frac=0.1):
    ...
    cannot select a backbone worse than the parent's on the criterion that
    matters. Returns (B (d, 2) float32, b (2,) float32).
    """
    n, d = Xs.shape
...
def _fit_gbm_members(X_raw, X_test_raw, Ys, mask, train_idx, val_idx, seed,
    ...
    (skip family 1 / keep the gated pre-tail prediction) when the clock is
    tight.

    The included families are AVERAGED into ONE tree prediction per
    component, so the tree member's weight in the final combine is unchanged
    from the verified MLP:tree ratio.
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # RIDGE BASELINE with TAIL-TUNED ALPHA (the one new change): fit the
    # linear map features -> (u, v) in closed form, but instead of the
    # frozen alpha=10, select each component's regularization from
    # _RIDGE_ALPHAS by solving on pre-tail statistics and scoring on the
    # temporal tail, then refit at the winner on the full span (see
    # _fit_ridge_tuned). With the stress, current-speed, stencil-spread,
    # and validity-fraction features appended, the linear fit can represent
    # the Ekman response to wind stress, speed-dependent attenuation/
    # rotation of the geostrophic current, a front-strength-modulated
    # offset, and a per-feature correction for partially observed rows; the
    # nonlinear members then only have to learn the remaining RESIDUAL —
    # and a correctly shrunk backbone leaves them the smallest possible one.
    ...
    # (The same tail tuned the ridge alpha above — 6 candidates against
    # ~100k rows is far too small a search to contaminate it.)
    n = len(Xs)
    ...
    for m, frac in enumerate(slice_ends):
        ...
        net = _train_member(X_t, Y_t, M_t, X_v, Y_v, M_v, X_f, Y_f, M_f, delta,
                            comp_w, Xs.shape[1], hidden, seed + 1000 * m,
                            device, max_epochs, t_end)
        with torch.no_grad():
    ...
    # COMBINE: per component, average the MLP members plus (if any family
    # passed the validation gate) the averaged tree prediction as one extra
    # equal-weight member, all on top of the exact ridge baseline.
    out = lin_test.copy()
    for c in range(2):
