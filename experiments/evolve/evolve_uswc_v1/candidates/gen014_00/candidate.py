...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    val_corr_sum = np.zeros((n_val, 2), dtype=np.float64)
    n_members = 0
    ...
    for m, frac in enumerate(slice_ends):
        ...
        with torch.no_grad():
            ...
            # Same member's correction on the temporal-validation tail (X_v is
            # already on device), for the calibration step below.
            for i in range(0, n_val, 65536):
                val_corr_sum[i:i + 65536] += net(X_v[i:i + 65536]).cpu().numpy()
        n_members += 1
        del net
        if device.type == "cuda":
            torch.cuda.empty_cache()

    # VALIDATION-FITTED CORRECTION SHRINKAGE (the one new change): the run's
    # wall-clock budget is soft, so on a slow run the ensemble can end up
    # undertrained or single-membered, yet its nonlinear correction is always
    # added at full weight — the likely source of the large score variance
    # observed when re-running this exact program. Regress the temporal-
    # validation ridge residuals on the ensemble's validation correction and
    # solve the closed-form per-component scale
    #   alpha_c = sum(m * corr * resid) / sum(m * corr^2),
    # i.e. the MSE-optimal amount of this correction to add on held-forward-
    # in-time data. A well-trained ensemble yields alpha ~= 1 (behavior
    # unchanged); a weak or overconfident one is shrunk toward the exact
    # ridge baseline. Clipped to [0, 1.15]: the tail was seen for at most two
    # low-LR fine-tune epochs, so the raw estimate is mildly optimistic and
    # the upper clip bounds that bias; the lower clip keeps the correction
    # from flipping sign on noise.
    corr_val = val_corr_sum / max(1, n_members)
    resid_val = Ys[val_idx].astype(np.float64)
    m_val = mask[val_idx].astype(np.float64)
    alpha_c = np.ones(2, dtype=np.float64)
    for c in range(2):
        den = float((m_val[:, c] * corr_val[:, c] ** 2).sum())
        if den > 1e-9:
            num = float((m_val[:, c] * corr_val[:, c] * resid_val[:, c]).sum())
            alpha_c[c] = float(np.clip(num / den, 0.0, 1.15))

    out = (lin_test + alpha_c * (pred_sum / max(1, n_members))).astype(np.float32)
