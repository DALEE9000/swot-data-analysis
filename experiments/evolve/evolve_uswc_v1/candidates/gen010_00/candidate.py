...
def _augment_physics(X):
    ...
    swath edges. Never NaN, ordering-invariant, zero layout risk.
...
class EMA:
    ...
    THE ONE NEW CHANGE this generation. Averaged SGD iterates land in
    flatter, wider minima and strip mini-batch noise from the final
    weights — the classic nearly-free generalization win, most valuable
    when the test split is a temporal extrapolation. Decay warms up as
    (1+t)/(warmup+t) so the shadow is not dominated by the random init.
    Validation, checkpoint selection, and final prediction all use the
    EMA weights; the raw iterate is only used to keep training moving.
    """

    def __init__(self, net, decay=0.999, warmup=10.0):
        self.decay = decay
        self.warmup = warmup
        self.step = 0
    ...
    def update(self, net):
        self.step += 1
        d = min(self.decay, (1.0 + self.step) / (self.warmup + self.step))
        for k, v in net.state_dict().items():
...
def _train_member(X_t, Y_t, M_t, X_v, Y_v, M_v, X_f, Y_f, M_f, delta, comp_w,
    ...
    computed on the EMA (Polyak-averaged) weights, the quantity the mean-R^2
    metric measures. Returns the net (EMA weights loaded) in eval mode."""
    ...
    # EMA horizon ~1000 steps (~4-5 epochs at this batch size): long enough
    # to average across many minima-basin oscillations, short enough to
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        # of the EMA weights: the score averages per-component R^2, so
        # selection must weigh u and v equally in sigma units, and it must
        # judge the averaged weights we will actually deploy — not the raw
        # noisy iterate. Backup/restore keeps training unaffected.
    ...
    # Start the fine-tune from the best EMA weights — the smoothest, best-
    # validated point in the trajectory.
    ...
    # shape the final weights. A fresh short-horizon EMA (decay 0.998,
    # ~500-step horizon, matching the fine-tune's length) averages the whole
    # sweep so the final weights are not dictated by the last noisy batches.
    ft_opt = torch.optim.AdamW(net.parameters(), lr=1e-4, weight_decay=1e-4)
    ft_ema = EMA(net, decay=0.998, warmup=1.0)
    ...
    # Deploy the averaged fine-tuned weights. If the fine-tune never ran
    # (ft_epochs == 0 or budget exhausted), ft_ema still holds best_state,
