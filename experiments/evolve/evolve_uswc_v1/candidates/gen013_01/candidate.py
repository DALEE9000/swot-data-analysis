...
_IDX_UGOSA = 4   # ugosa_filtered block; vgosa_filtered is the following block
_IDX_ERA5_U = 6  # era5_u block; era5_v is the following block


def _nancov(A, B):
    """NaN-aware per-row covariance between two aligned stencil blocks.

    Uses only cells where BOTH components are finite; rows with fewer than
    two jointly valid cells get 0. Because the (unknown) stencil cell
    ordering is the same for every feature block, any permutation applies
    identically to A and B, so the covariance is ordering-invariant.
    """
    ...
    ma = A0.sum(axis=1, keepdims=True) / denom
    mb = B0.sum(axis=1, keepdims=True) / denom
    cov = (A0 * B0).sum(axis=1, keepdims=True) / denom - ma * mb
    return np.where(cnt >= 2.0, cov, 0.0).astype(np.float32)


def _augment_physics(X):
    """Append quadratic stress-like features, current-speed features,
    stencil-spread features, stencil validity fractions, and cross-stencil
    u-v covariances.
    ...
    mesoscale roughness. Invariant to the unknown stencil cell ordering.
    Validity fractions (verified gain in gen007_02): per base feature, the
    fraction of finite cells across its 9 stencil cells, so mean-imputed
    swath-edge rows are distinguishable from genuinely quiescent rows.
    CROSS-STENCIL U-V COVARIANCE (the one new change): for each velocity
    pair — (ugos, vgos), (ugosa, vgosa), (era5_u, era5_v) — the NaN-aware
    covariance between the two components across the 9 stencil cells. The
    per-feature spread measures the MAGNITUDE of local variability but not
    the u-v coupling; the covariance is a Reynolds-stress-like (u'v') /
    strain-orientation proxy indicating which way the local eddy strain
    axis points — regime information that rotates/attenuates the surface->
    subsurface transfer differently for u and v. Ordering-invariant (the
    same cell permutation applies to both blocks), never NaN, near-free.
    ...
    uga, vga = blk(_IDX_UGOSA), blk(_IDX_UGOSA + 1)
    ...
    uv_cov = np.concatenate(
        [_nancov(ug, vg), _nancov(uga, vga), _nancov(u10, v10)], axis=1)
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # per-base-feature stencil validity fractions, and per-velocity-pair
    # cross-stencil u-v covariances: 81 -> 156 columns, all before any
    # statistics are computed, so the ridge and the MLP both see quadratic
    # wind forcing, speed-dependent transfer, local front-strength regime
    # information, data-quality indicators, and eddy strain-axis orientation.
    ...
    # validity-fraction, and u-v covariance features appended, the linear fit
    ...
    # modulated offset, a per-feature correction for partially observed rows,
    # and a strain-orientation-dependent term; the MLP ensemble then only has
    # to learn the remaining nonlinear RESIDUAL.
