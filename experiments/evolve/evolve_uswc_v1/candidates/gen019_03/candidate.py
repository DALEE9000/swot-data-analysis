...
# DIVERSE GBM FAMILIES (verified in gen018_03; loss-diverse family NEW):
# family 0 is the exact verified configuration and always runs first.
# Family 1 is deliberately diverse along two axes at once: deeper trees
# with stronger shrinkage and heavier regularization (different function
# class), and a strided data window — every 2nd row of the last 1M valid
# rows — so it trains on twice the time span at the same row count
# (different data view).
#
# Family 2 (THE ONE NEW CHANGE) extends the family-diversity lever — the
# single largest verified gain in this lineage — along a third,
# still-unexplored axis: the LOSS FUNCTION. It fits the residuals with
# loss="absolute_error" (median regression) on a stride-3 window (~1.5M
# rows of history, the longest time span of any member). Every other
# member of the ensemble — ridge, Huber MLPs, squared-error GBMs —
# minimizes a quadratic-core objective, so their errors share the same
# sensitivity to heavy-tailed HFR noise; an L1 family targets the
# conditional median, is immune to tail outliers by construction, and
# sees a third data view, decorrelating its errors in a way
# hyperparameter or stride tweaks cannot. It runs LAST inside the
# unchanged GBM wall-clock cap (skipped under a tight clock), and the
# verified gate — beat ridge-alone MSE on the temporal tail, the metric's
# own currency — rejects it per component if median bias hurts squared
# error, so it can dilute nothing. Included families are AVERAGED into
# the single tree ensemble member, which the tail-stacked combine then
# weights by measured tail skill.
_GBM_FAMILIES = [
    ...
    dict(max_iter=70, learning_rate=0.12, max_leaf_nodes=127,
         ...
         loss="squared_error"),
    dict(max_iter=80, learning_rate=0.10, max_leaf_nodes=63,
         min_samples_leaf=60, l2_regularization=1.0, stride=3,
         loss="absolute_error"),
]

# Wall-clock cap for ALL tree fits (all families, gate + recency refit
...
# checkpoints, gated pre-tail GBM fits); weights are shrunk halfway back to
# uniform and guarded, falling back to the exact equal-weight average on
# any failure.
...
def _make_gbm(seed, fam):
    return HistGradientBoostingRegressor(
        loss=fam.get("loss", "squared_error"),
...
def _fit_gbm_members(X_raw, X_test_raw, Ys, mask, train_idx, val_idx, seed,
    ...
    gen018_03; loss-diverse family 2 is the one new change this generation).
    ...
    from theirs. Family 2 additionally fits with absolute-error (median)
    loss — a different loss geometry from every quadratic-core member — on
    the longest strided time window.
    ...
    predictor (ridge alone) on that tail IN SQUARED ERROR, the metric's own
    currency, so a failed family (including a median fit whose bias hurts
    MSE) dilutes nothing. A family that passes is refit with identical
    hyperparameters on its window over the FULL span — shifted forward to
    include the tail, the rows closest in time to the test cycles — and the
    refit model predicts the test set. Deadline checks before every fit
    fall back gracefully (skip later families / keep the gated pre-tail
    prediction) when the clock is tight; family ordering puts the verified
    configurations first, so a tight clock degrades to exactly the
    previous champion.
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # GBM MEMBERS: three diverse tree families per component (squared-error
    # stride-1, squared-error stride-2, absolute-error stride-3), each gated
    # on the pre-tail fit and recency-refit on the full span (see
    # _fit_gbm_members), averaged into ONE tree ensemble member and capped
    # to a slice of the wall clock; unused tree time flows back to the MLP
    # ensemble because the MLP slices are computed from the time actually
    # remaining.
    ...
    # TAIL-STACKED COMBINE (verified in gen019_01): per component, learn
