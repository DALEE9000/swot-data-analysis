...
def _average_states(states):
    """Average a list of same-architecture state dicts (weight averaging).

    All checkpoints come from the SAME training run — the same SGD basin — so
    parameter-space averaging is valid and acts like a cheap SWA: it lands in
    a flatter region of the loss surface than any single checkpoint. The net
    is LayerNorm-based (no BatchNorm running statistics), so plain averaging
    of every floating tensor is exact; non-float entries (none expected) are
    taken from the first (best) checkpoint.
    """
    avg = {}
    for k, v in states[0].items():
        if torch.is_floating_point(v):
            acc = v.detach().clone().to(torch.float64)
            for s in states[1:]:
...
def _train_member(X_t, Y_t, M_t, X_v, Y_v, M_v, X_f, Y_f, M_f, delta, comp_w,
    ...
    CHECKPOINT AVERAGING (the one new change): instead of restoring the
    single best-validation checkpoint, retain the top-3 checkpoints by
    balanced validation MSE and average their weights; keep the average only
    if it validates at least as well as the best single checkpoint. Flatter
    minima generalize better forward in time, which is what the temporally
    held-out test measures. Returns the net in eval mode."""
    ...
    best_val = float("inf")
    best_epoch = 0
    ckpts = []  # top-k list of (val_loss, epoch, cpu_state_dict)
    n_train = len(X_t)
    n = len(X_f)
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        # leak into selection.
        val_loss = eval_val(net)
        scheduler.step(val_loss)

        # Retain this checkpoint if it ranks among the k_avg best so far.
        if len(ckpts) < k_avg or val_loss < ckpts[-1][0]:
            state_cpu = {k: v.detach().cpu().clone()
                         for k, v in net.state_dict().items()}
            ckpts.append((val_loss, epoch, state_cpu))
    ...
    # Restore: averaged top-k weights if they validate at least as well as
    # the single best checkpoint, else the best checkpoint (strict fallback —
    # the change can only match or improve validation selection).
    best_state = ckpts[0][2]
    if len(ckpts) > 1:
        avg_state = _average_states([c[2] for c in ckpts])
        net.load_state_dict(avg_state)
        avg_val = eval_val(net)
        if avg_val > ckpts[0][0]:
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # geostrophic current, and a front-strength-modulated offset; the MLP
