...
# Cosine LR schedule (THE ONE NEW CHANGE): each MLP member anneals its
# learning rate from _LR_MAX to _LR_MIN following a cosine over the ELAPSED
# WALL-CLOCK FRACTION of its own training slice. ReduceLROnPlateau with
# patience 5 rarely fires more than once in the ~20-40 epochs a slice
# affords, so members previously ended their slice still at a high, noisy
# LR; the budgeted cosine guarantees every member finishes in a converged
# low-LR regime exactly when its slice expires, which also makes the EMA
# checkpoint an average of a converging (not bouncing) trajectory.
_LR_MAX = 1e-3
_LR_MIN = 2e-5
...
def _train_member(X_t, Y_t, M_t, X_v, Y_v, M_v, X_f, Y_f, M_f, delta, comp_w,
    ...
    BUDGETED COSINE LR (the one new change): before each epoch the learning
    rate is set to a cosine interpolation between _LR_MAX and _LR_MIN driven
    by the fraction of this member's wall-clock slice already spent. The
    previous ReduceLROnPlateau(patience=5) rarely decayed more than once in
    the ~20-40 epochs a slice affords, so training ended at a high, noisy
    LR; the budgeted cosine guarantees the member is annealed to a low LR
    exactly as its slice expires, regardless of how many epochs that turns
    out to be. Peak LR is unchanged from the verified configuration.
