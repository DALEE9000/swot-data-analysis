...
def _masked_nll_anchored(mu, logvar, target, mask, row_w, comp_w):
    ...
    scaled by a per-row weight (recency ramp x edge-missingness boost — see
    train_and_predict) AND a per-component inverse-variance weight. The
    score is mean per-component R^2 = mean_c(1 - MSE_c/Var_c), so a unit of
    squared error on the lower-variance component (u) buys more R^2 than
    the same error on v; comp_w = (1/Var_u, 1/Var_v) mean-1 normalized
    makes the loss's exchange rate between u and v errors match the
    metric's. The anchor keeps a gradient floor on variance-downweighted
    rows, as in the lineage."""
...
def _masked_mse(pred, target, mask, comp_w):
    ...
    optimize the metric directly instead of pooled MSE. Deliberately
    row-UNWEIGHTED: the metric is uniform over rows, so selection must
    stay uniform even though training upweights edge rows."""
...
def _train_member(member_seed, arch, deadline, device, max_epochs,
    ...
    row- and component-weighted anchored heteroscedastic NLL; selection
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # THE ONE CHANGE: per-row missingness fraction, computed from the RAW
    # X_train NaN pattern (before featurize imputes them away), feeding an
    # edge-row loss upweight below. Chunked so the boolean temp stays small.
    n_rows = len(X_train)
    miss_frac = np.empty(n_rows, dtype=np.float32)
    for i in range(0, n_rows, 1_000_000):
        miss_frac[i:i + 1_000_000] = np.isnan(
            X_train[i:i + 1_000_000]).mean(axis=1).astype(np.float32)
    ...
    # MULTIPLIED by the new edge-missingness boost (1 + 1.5 * miss_frac):
    # full-stencil rows keep weight ~1, truncated-swath rows get up to
    # ~1.75-2.5x extra gradient mass. The edge bins are the parent's worst
    # (r2_u 0.287 edge vs 0.476 full), i.e. the largest per-row squared
    # error lives exactly where the loss currently pays least attention
    # relative to difficulty; the conv mask channel, per-cell missingness
    # block, and mirror imputation already give the nets the inputs needed
    # to specialize there — this makes the loss ask them to. Mean-1
    # normalized as always so the tuned loss scale is unchanged. Validation
    # (_masked_mse) stays row-uniform: selection still optimizes the
    # metric, only the training gradient shifts.
    pos = (np.arange(n, dtype=np.float32) / max(n - 1, 1)).astype(np.float32)
    w_all = np.exp(np.log(3.0) * pos).astype(np.float32)
    w_all *= (1.0 + 1.5 * miss_frac)
    w_all /= w_all.mean()
    del miss_frac
