...
def _featurize(X, mean, scale, n_cells):
    ...
    neighborhood. Ingredients:
      ...
      * per-feature SECOND-order operators (n, 3F): the 5-point Laplacian
        ...
        axis-aligned differences cannot resolve; fast fronts and eddy rims
        are exactly where these are large. Because the mirror imputation is
        Taylor-LINEAR, filled cells satisfy filled + mirror - 2*center = 0,
        so at swath edges the second differences collapse toward zero — a
        conservative "no measured curvature" default rather than amplified
        edge noise — and the missingness block tells the net when that zero
        is imputed rather than observed.
    """
...
def _masked_nll_anchored(mu, logvar, target, mask, row_w, comp_w):
    ...
    scaled by a per-row recency weight AND a per-component R^2-alignment
    weight.
    ...
    comp_w is the R^2-alignment weight (shape (2,), mean 1): raw MSE summed
    over (u, v) implicitly weights each component by its target VARIANCE,
    but the score is the MEAN of per-component R^2, which normalizes each
    component's error by 1/variance. comp_w = inverse target variance
    (normalized to mean 1) makes the optimized quantity proportional to the
    scored one, shifting capacity toward the lower-variance component
    instead of letting the higher-variance component dominate the gradient.
    Normalizing to mean 1 keeps the overall loss scale — and hence the
    tuned LR, scheduler behavior, and logvar calibration — unchanged; the
    variance head simply absorbs a constant per-component log-shift.

    Both weights multiply the whole per-component loss and the normalizer,
    so they reweight without changing the loss scale.
    """
...
def _masked_mse(pred, target, mask, comp_w):
    """Masked MSE with the same per-component inverse-variance weights as
    the training loss, so validation-based early stopping, EMA-vs-raw
    selection, and the ensemble quality guard all rank models by (a
    monotone proxy of) mean per-component R^2 — the scored metric — rather
    than by raw MSE, which over-rewards fitting the higher-variance
    component."""
    w = mask * comp_w.unsqueeze(0)
    diff2 = (pred - target).pow(2) * w
    return diff2.sum() / w.sum().clamp(min=1)
...
def _train_member(member_seed, deadline, device, max_epochs,
    ...
    """Train one ensemble member with the recency-weighted, R^2-aligned
    anchored heteroscedastic NLL. Each epoch, BOTH the raw net and its
    weight-EMA shadow are scored on the component-weighted temporal-val
    masked MSE of mu — a monotone proxy of the scored mean R^2 — and model
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        # Model selection on the component-weighted masked MSE of mu — the
        # metric-aligned quantity — NOT on the training loss, whose value
        # mixes in the variance term and the recency weighting. Both the
        # raw and EMA weights compete; early in training the EMA lags and
        # loses, near convergence it usually wins. The scheduler keeps
    ...
    # same recency-weighted, component-weighted anchored loss so relative row
    # AND component weighting stay consistent (the tail carries the largest
    # weights, as it should).
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # R^2-alignment weights: the harness scores mean(R^2(u), R^2(v)), which
    # normalizes each component's squared error by that component's target
    # variance — but raw masked MSE weights components by their variance
    # instead. comp_w = 1/nanvar per component, normalized to mean 1 so the
    # overall loss scale (and the tuned LR / scheduler / logvar init) is
    # unchanged. Applied identically in the training loss, the validation
    # selection metric, and the fine-tune, so every optimization and
    # selection decision targets the scored quantity.
    var_c = np.nanvar(Y_train, axis=0).astype(np.float64)
    inv_c = 1.0 / np.maximum(var_c, 1e-8)
    comp_np = (2.0 * inv_c / inv_c.sum()).astype(np.float32)
    comp_w = torch.from_numpy(comp_np).to(device)
