...
class ConvWideDeep(nn.Module):
    ...
    THE ONE CHANGE (with HeteroMLP below): ~2x width everywhere. After the
    lineage's GPU-staging throughput win, members converge inside their
    wall-clock slices (early stopping fires), so the ensemble is
    bias-limited, not epoch-starved: ~150k-param nets underfit 3.4M rows of
    ~170 engineered features. Conv channels 32/48 -> 48/96, conv projection
    128 -> 192, dense path 256/128 -> 384/192, trunk 128 -> 192. Dropout,
    weight decay, EMA, and temporal-val selection are unchanged and contain
    the added variance.
    ...
    layer's receptive field covers the whole window.
    ...
    def __init__(self, d_total, n_feat, n_cells, dropout=0.1,
        ...
        self.conv_proj = nn.Sequential(
            nn.Linear(ch2 * n_cells, 192), nn.LayerNorm(192), nn.SiLU(),
            nn.Dropout(dropout),
        )
        ...
        self.dense = nn.Sequential(
            ...
            nn.Linear(384, 192), nn.LayerNorm(192), nn.SiLU(), nn.Dropout(dropout),
        )
...
class HeteroMLP(nn.Module):
    ...
    leans on. Widened (256,256,128) -> (512,384,256) as part of the
    capacity scale-up (see ConvWideDeep docstring)."""

    def __init__(self, n_inputs, hidden=(512, 384, 256), dropout=0.1,
                 logvar_init=-3.5):
        super().__init__()
        layers = []
        d = n_inputs
...
def _make_net(d_total, n_feat, n_cells, device, arch):
    ...
    reshape would be invalid), so the pooled k=3 dataset gets conv members
    where requested and degenerate layouts get MLPs — never a crash."""
    k = int(round(math.sqrt(n_cells)))
    if arch == "conv" and n_cells > 1 and k * k == n_cells:
        return ConvWideDeep(d_total, n_feat, n_cells, dropout=0.1).to(device)
    return HeteroMLP(d_total, hidden=(512, 384, 256), dropout=0.1).to(device)
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # alongside the fp16 val block, the nets, and optimizer state; the
    # widened nets add <10 MB of parameters + optimizer moments).
    ...
    # Three-member ensemble (unchanged from the parent): conv, mlp, and a
    # second conv under a distinct seed, per-member wall-clock slices,
    # member order keeping the two proven architectures first so a squeezed
    # third slice never hurts the original pair. THE ONE CHANGE of this
    # generation lives in the architectures themselves (see ConvWideDeep /
    # HeteroMLP docstrings): ~2x width, because post-throughput-win members
    # converge inside their slices and the lineage is bias-limited. The 15%
    # val guard below drops any member the extra width leaves undertrained,
    # so the worst case degenerates to a smaller ensemble of converged
    # members rather than averaging in a starved net.
    archs = ["conv", "mlp", "conv"]
    M_ens = 3
    members = []
    ...
    # degenerates to the proven smaller ensemble instead of averaging in an
