...
def _masked_mse(pred, target, mask, comp_w):
    ...
    optimize the metric directly. Note: NO row weight here — the edge
    upweight biases only the fit, never the selection (the parent's
    argument, kept), while comp_w belongs in selection because it IS part
    of the metric."""
...
def _train_member(member_seed, arch, deadline, device, max_epochs,
    ...
    heteroscedastic NLL; selection over raw-vs-EMA on component-weighted
    temporal-val masked MSE (the scored quantity, row-unweighted); then the
    proven recency fine-tune ending on a short-horizon EMA. Returns
    (net, best_val)."""
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # Per-component inverse-variance weights (THE grafted change vs the
    # parent, from the top-scoring sibling). Score = mean_c(1 - MSE_c/Var_c):
    # weighting each component's squared error by 1/Var_c (of the ORIGINAL
    # targets — the additive ridge baseline preserves MSE identity, and R^2
    # is computed on the final predictions) makes both the training loss and
    # every validation-based selection decision optimize mean R^2 rather
    # than pooled MSE, which implicitly favors the higher-variance component
    # (v, the one already ahead). Mean-1 normalized so the tuned loss scale
    # is unchanged; clipped to [0.5, 2] relative so a pathological variance
    # ratio cannot destabilize training.
