...
def _featurize(X, mean, scale, n_cells):
    ...
    Proven plumbing (unchanged from the lineage):
      ...
      * nonlinear flow-regime block: per-feature |grad f| (n, F) and, for
        the pooled 8-feature layout, center flow speeds, relative vorticity,
        normal/shear strain, Okubo-Weiss, and geostrophic advection of every
        feature — front/eddy discriminants the linear features cannot
        express. Quadratic terms clipped to +/-16.
    """
...
def _augment_edges(X_raw, rng, n_cells):
    """Synthetic edge-truncation: mask one full side (row or column) of the
    k x k stencil, across ALL features, to NaN — the exact missingness
    pattern swath-edge padding produces. Returns the masked copy of X_raw.

    Why supervised augmentation and not just better imputation: real
    edge-stencil rows are a training minority AND their targets are
    confounded with genuinely different near-boundary ocean conditions, so
    the net cannot cleanly learn "how much to trust imputed cells" from
    them. Truncating INTERIOR rows keeps the target fixed while removing a
    side of the neighborhood — the counterfactual pair (full stencil, cut
    stencil) -> same (u, v) isolates the missingness effect and teaches the
    net to discount the mirror-filled structure, the collapsed second
    differences, and the zeroed spatial std that _featurize produces at
    edges. The center cell is never masked (matches real edge rows, whose
    center is by construction observed).
    """
    Xa = X_raw.copy()
    n = len(Xa)
    d = Xa.shape[1]
    ...
    if k < 2 or k * k != n_cells:
        return Xa
    sides = [
        [r * k for r in range(k)],             # left column of the window
        [r * k + (k - 1) for r in range(k)],   # right column
        list(range(k)),                        # top row
        [(k - 1) * k + c for c in range(k)],   # bottom row
    ]
    which = rng.integers(0, 4, size=n)
    for s, cells in enumerate(sides):
        rows = np.where(which == s)[0]
        if len(rows) == 0:
            continue
        cols = np.array([f * n_cells + c for f in range(F) for c in cells])
...
def _masked_nll_anchored(mu, logvar, target, mask, row_w):
    ...
    augmented edge-truncated copies carry half their source row's weight):
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # Edge-truncation augmentation (new this generation): a 20% sample of
    # TRAINING rows (never validation — it must stay a clean forward-in-time
    # measurement) is copied, one full side of its raw stencil masked to NaN,
    # and re-featurized through the exact edge pipeline (mirror fill,
    # missingness flags, collapsed second differences). Targets are the
    # source rows' — the ocean state did not change, only what the "swath"
    # shows of it — so the net gets counterfactual supervision that isolates
    # the missingness effect, directly attacking the edge-stencil R^2 gap
    # (u 0.2876 edge vs 0.4632 full). Half weight keeps the copies auxiliary
    # so the full-stencil fit is not diluted; recency ramp is inherited so
    # the augmented distribution matches the real one in time.
    aug_rng = np.random.default_rng(seed + 777)
    n_tr = len(train_idx)
    n_aug = int(n_tr * 0.2)
    if n_aug > 0:
        aug_rows = aug_rng.choice(n_tr, size=n_aug, replace=False)
        Xa = _augment_edges(X_train[aug_rows], aug_rng, n_cells)
        Xa_feat = _featurize(Xa, mean, scale, n_cells)
        del Xa
        X_t = torch.cat([X_t, torch.from_numpy(Xa_feat)])
        del Xa_feat
        Y_t = torch.cat([Y_t, torch.from_numpy(Ys[aug_rows])])
        M_t = torch.cat([M_t, torch.from_numpy(mask[aug_rows])])
        W_t = torch.cat([W_t, torch.from_numpy(w_all[aug_rows] * np.float32(0.5))])
