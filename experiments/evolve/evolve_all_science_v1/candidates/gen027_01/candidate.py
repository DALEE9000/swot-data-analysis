...
class HeteroMLP(nn.Module):
    ...
    decorrelation is what the prediction blend harvests. Notably the
    stronger-r2_u member in the lineage, which the per-component blend
    weights can now actually exploit."""
...
def _val_preds(net, X_v, bs):
    """Full residual-space mu predictions on the validation split, on CPU."""
    net.eval()
    outs = []
    with torch.no_grad():
        for i in range(0, len(X_v), bs):
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # one architecture, so the same-cost combination cancels more variance,
    ...
    # ---- Per-COMPONENT validation-optimal convex blend (THE change) ----
    # The parent averages members uniformly, forcing one exchange rate on
    # both components even though the members' skills differ BY COMPONENT
    # (MLP stronger on u, conv on v). For two kept members, fit one scalar
    # alpha_c per component by masked least squares on the temporal
    # validation split — residual space, which the additive ridge baseline
    # makes identical to final-prediction MSE, i.e. the scored quantity:
    #   pred_c = alpha_c * mu0_c + (1 - alpha_c) * mu1_c
    #   alpha_c* = sum(m (y - p1)(p0 - p1)) / sum(m (p0 - p1)^2)
    # Clipped to [0.15, 0.85]: keeps genuine ensemble variance reduction in
    # every component (never collapses to one net) and bounds the damage if
    # the recency fine-tune's light exposure to the val rows biases the
    # estimate. One scalar per component cannot meaningfully overfit a
    # 380k-row split. Degenerate denominator or a single kept member falls
    # back to the parent's uniform average exactly.
    bs_v = 4096
    if len(keep) == 2:
        p0 = _val_preds(keep[0], X_v, bs_v)
        p1 = _val_preds(keep[1], X_v, bs_v)
        y_v = Y_v_cpu.numpy()
        m_v = M_v_cpu.numpy()
        blend = np.full((2, 2), 0.5, dtype=np.float32)  # (member, comp)
        for c in range(2):
            m = m_v[:, c] > 0
            diff = p0[m, c] - p1[m, c]
            denom = float(np.sum(diff * diff))
            if denom > 1e-8:
                num = float(np.sum((y_v[m, c] - p1[m, c]) * diff))
                a = float(np.clip(num / denom, 0.15, 0.85))
                blend[0, c] = a
                blend[1, c] = 1.0 - a
        w_blend = torch.from_numpy(blend).to(device)
    else:
        w_blend = torch.full((len(keep), 2), 1.0 / len(keep), device=device)

    # Predict: ridge baseline + per-component blended residual mu over
    # accepted members.
