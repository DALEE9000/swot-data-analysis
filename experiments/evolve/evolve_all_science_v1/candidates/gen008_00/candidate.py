...
def _featurize(X, mean, scale, n_cells):
    ...
    neighborhood. Two appended blocks:
      ...
      * per-feature stencil standard deviation (n, F), computed on the
        ...
        intensity signal, and exactly the conditioning information the
        heteroscedastic variance head needs to downweight the rows it is
        meant to downweight. Rows with a single valid cell get std 0
        ("no local gradient information").
    """
...
def _augment_stencil_dropout(xb, F, C, row_p=0.25, cell_p=0.5):
    """Train-time swath-edge simulation, applied in standardized feature space.

    For a random subset of rows, drop random non-center stencil cells the same
    way test-time padding manifests after _featurize: standardized value -> 0
    (the feature mean) and that cell's missing-fraction entry -> 1. The center
    cell is never dropped (real edge rows always retain their center). The
    trailing spatial-std block is left as computed from the full stencil — a
    deliberate approximation; the missingness block carries the padding signal
    the net needs to condition on. Trains robustness to padded neighborhoods
    on the abundant full-stencil rows instead of only the rare true edge rows.
    Mutates xb in place (callers pass a fresh batch copy).
    """
    bs = xb.shape[0]
    dev = xb.device
    row = torch.rand(bs, 1, device=dev) < row_p
    cell = torch.rand(bs, C, device=dev) < cell_p
    drop = row & cell
    drop[:, C // 2] = False
    if not drop.any():
        return xb
    featmask = drop.unsqueeze(1).expand(-1, F, -1).reshape(bs, F * C)
    xb[:, :F * C] = xb[:, :F * C].masked_fill(featmask, 0.0)
    xb[:, F * C:F * C + C] = xb[:, F * C:F * C + C].masked_fill(drop, 1.0)
    return xb
...
def _train_member(member_seed, deadline, device, max_epochs, F, C,
    ...
    """Train one ensemble member with the heteroscedastic NLL and stencil-
    dropout augmentation (early stop on temporal-val masked MSE of mu, then
    the proven recency fine-tune) within its wall-clock slice.
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for i in range(0, n_train, bs):
            ...
            xb = _augment_stencil_dropout(xb, F, C)
            optimizer.zero_grad()
            mu, lv = net(xb)
        ...
        # Validation stays UNaugmented: it estimates the real held-out error.
        net.eval()
        with torch.no_grad():
            val_losses = []
    ...
    # Recency fine-tune (proven in lineage, +~0.01): the validation tail is
    # the most recent — and hence most test-like — 10% of the training window,
    # and the model above never trained on it. After model selection, absorb
    # it with a short low-LR pass over the FULL window (tail included), using
    # the same heteroscedastic NLL and the same augmentation so the training
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # heteroscedastic parent instead of averaging in an undertrained net.
