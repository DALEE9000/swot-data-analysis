...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # Stats are computed on the ORIGINAL matrix, before augmentation, so the
    # standardization is identical to the parent's.
    ...
    # THE ONE CHANGE: edge-truncation augmentation. The edge-stencil bins
    # are the parent's worst by a wide margin (r2_u 0.287 vs 0.476
    # full-stencil), and real swath-edge missingness is structured — a
    # whole SIDE of the stencil is gone at once, for every feature — not
    # the random cell dropout a mask channel alone teaches. Simulate
    # exactly that on 15% of training rows BEFORE featurization: pick a
    # random side (left/right/top/bottom line of the k x k window, center
    # cell never touched) and NaN those cells across all F features.
    # Injecting into the raw matrix keeps every engineered quantity —
    # missing fraction, mirror imputation, spatial std, gradients, mask
    # channel — self-consistent with genuine edge rows, so both members
    # learn to predict THROUGH truncation from rows whose targets came
    # with full context. The temporal-validation slice (last 10%) is left
    # clean so early stopping still measures the natural test mixture.
    # Cost: one host-side fp32 copy (~1.1 GB, freed right after
    # featurize); training loop, schedule, ensemble, guard, fine-tune are
    # byte-identical to the parent.
    k_aug = int(round(math.sqrt(n_cells)))
    X_in = X_train
    if n_cells > 1 and k_aug * k_aug == n_cells:
        n_rows = len(X_train)
        lim = n_rows - max(1, int(n_rows * 0.1))  # never augment the val slice
        rng_a = np.random.default_rng(seed + 7)
        n_aug = int(lim * 0.15)
        if n_aug > 0:
            rows = rng_a.choice(lim, size=n_aug, replace=False)
            sides = rng_a.integers(0, 4, size=n_aug)
            X_in = X_train.copy()
            Xv3 = X_in.reshape(n_rows, n_feat, n_cells)
            side_cells = [
                [r * k_aug for r in range(k_aug)],              # left column
                [r * k_aug + k_aug - 1 for r in range(k_aug)],  # right column
                list(range(k_aug)),                             # top row
                list(range(n_cells - k_aug, n_cells)),          # bottom row
            ]
            ci = n_cells // 2
            for s in range(4):
                rsel = rows[sides == s]
                if len(rsel):
                    for c in side_cells[s]:
                        if c == ci:
                            continue
                        Xv3[rsel, :, c] = np.nan
            del Xv3

    Xs = _featurize(X_in, mean, scale, n_cells)
    if X_in is not X_train:
        del X_in
    ...
    # Proven throughput staging (unchanged from the parent): the full
    # featurized train matrix lives on the GPU in float16 (~1.1 GB, well
    # inside 4 GB alongside the fp16 val block, the small nets, and
    # optimizer state); on-device indexing + per-batch fp32 cast removes
    # host fancy-indexing and PCIe transfer from the epoch. fp16 rounding
    # (~1e-3 relative on standardized, clipped features) is far below data
    # noise. On any CUDA OOM we fall back to the CPU pipeline, so the
    # worst case is slower, not a crash.
    if device.type == "cuda":
        try:
    ...
    # Test features are NEVER augmented — real edge rows arrive with their
    # natural missingness, which the augmentation taught the nets to handle.
