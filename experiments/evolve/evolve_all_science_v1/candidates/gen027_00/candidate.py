...
def _loc_fingerprint(X, mean, scale, n_cells):
    """2-D geographic key from the mdt stencil (feature 0, feature-major so
    it occupies the first n_cells raw columns): standardized center value +
    central-difference gradient magnitude. mdt is a TIME-INVARIANT field, so
    the derived cluster ids carry zero drift across the temporal train->test
    split — safe to condition on even in the late-test bins."""
    n = len(X)
    Z = (X[:, :n_cells].astype(np.float32) - mean[:n_cells]) / scale[:n_cells]
    Z = np.nan_to_num(Z, nan=0.0, posinf=0.0, neginf=0.0)
    ci = n_cells // 2
    k = int(round(math.sqrt(n_cells)))
    if n_cells > 1 and k * k == n_cells:
        gx = 0.5 * (Z[:, ci + 1] - Z[:, ci - 1])
        gy = 0.5 * (Z[:, ci + k] - Z[:, ci - k])
        g = np.sqrt(gx * gx + gy * gy)
    else:
        g = np.zeros(n, dtype=np.float32)
    return np.stack([Z[:, ci], g], axis=1).astype(np.float32)
...
def _featurize(X, mean, scale, n_cells):
    ...
    consumes the whole vector flat. The locale one-hot (the one new block
    this generation) is hstacked AFTER this function's output by the
    caller, so it lands in the dense path / flat vector automatically.
    """
...
class ConvWideDeep(nn.Module):
    ...
    the proven engineered features kept as a shortcut; this generation that
    tail also carries the 16-way locale one-hot, so the dense path (and the
    trunk after the join) can condition residual corrections on WHERE the
    row is without any change to this class.
    ...
    def __init__(self, d_total, n_feat, n_cells, dropout=0.1,
        ...
        dd = d_total - self.img_dim  # mask block + engineered scalars + locale
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # ---- Locale one-hot (THE one change vs the parent) ----
    # The pooled dataset time-interleaves six regions with very different
    # dynamics, but no explicit locale signal reaches the model: one shared
    # trunk must average region-specific residual corrections. Cluster a
    # 2-D STATIC fingerprint (standardized center-cell mdt, |grad mdt|)
    # into 16 locales with seeded MiniBatchKMeans and append the one-hot
    # after the engineered scalars: the ridge baseline gains per-locale
    # intercept/slope corrections (region-specific mean-flow bias — a
    # direct pooled-training error term), and both members' dense paths
    # condition on locale without any learned gating (the MoE wildcard's
    # failure mode) or any change to conv image slicing. mdt is static, so
    # unlike flow-state gates this key cannot drift across the temporal
    # split — the late-test bins (u 0.391, the parent's worst) see exactly
    # the training-time locale distribution.
    K_LOC = 16
    fp_tr = _loc_fingerprint(X_train, mean, scale, n_cells)
    rng_loc = np.random.default_rng(seed + 977)
    fit_idx = rng_loc.choice(len(fp_tr), size=min(len(fp_tr), 400_000),
                             replace=False)
    km = MiniBatchKMeans(n_clusters=K_LOC, random_state=seed,
                         batch_size=8192, n_init=5, max_iter=200)
    km.fit(fp_tr[fit_idx].astype(np.float64))
    eyeK = np.eye(K_LOC, dtype=np.float32)
    loc_tr = eyeK[km.predict(fp_tr.astype(np.float64))]
    del fp_tr

    Xs = np.hstack([_featurize(X_train, mean, scale, n_cells), loc_tr])
    del loc_tr
    ...
    # Ridge from the full engineered block (now including the locale
    # one-hot) to (u, v) on a seeded subsample; the nets fit the residual.
    # The linear map extrapolates the dominant geostrophic signal into the
    # fast tail where bounded activations saturate; baseline clipped to
    # +/-3 m/s. Additive per-row baseline means residual val MSE ==
    ...
    # Test rows get their locale from the SAME fitted KMeans — mdt is
    # static, so the assignment is distribution-matched to training.
    out = np.zeros((len(X_test), 2), dtype=np.float32)
    with torch.no_grad():
        for i in range(0, len(X_test), 65536):
            chunk = X_test[i:i + 65536]
            fpc = _loc_fingerprint(chunk, mean, scale, n_cells)
            loc_c = eyeK[km.predict(fpc.astype(np.float64))]
            Xf = np.hstack([_featurize(chunk, mean, scale, n_cells), loc_c])
            base = _baseline(Xf)
