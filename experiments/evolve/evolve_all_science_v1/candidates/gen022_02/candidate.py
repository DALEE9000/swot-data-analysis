...
class HeteroMLP(nn.Module):
    ...
    Shared trunk -> two heads:
      mu     : (u, v) point predictions (what the harness scores). With the
               new linear baseline, mu is the RESIDUAL from the ridge
               prediction; the baseline is added back outside the net.
    ...
    def __init__(self, n_inputs, hidden=(256, 256, 128), dropout=0.1,
        ...
        # early training behaves like plain (scaled) MSE. The ridge residual
        # has somewhat smaller variance; the clamp range covers it and the
        # bias adapts within the first epochs.
...
def _train_member(member_seed, deadline, device, max_epochs,
    ...
    scored quantity (with a fixed additive baseline, residual val MSE equals
    final-prediction val MSE, so selection is unchanged in effect) — and
    model selection keeps the better of the two; the LR schedule still steps
    on the raw net's val loss (its training signal). Then the proven recency
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # ---- Linear baseline + residual learning (the one new mechanism) ----
    # Fit a per-component ridge regression from the FULL engineered feature
    # block to (u, v) on a seeded subsample, then train the net on the
    # residual y - clip(ridge(x)). Why: the fast-regime deficit is an
    # extrapolation failure — fast fronts/eddies sit far from the data bulk,
    # where a bounded-activation MLP saturates and regresses toward the
    # mean, while a linear map extrapolates the dominant geostrophic signal
    # indefinitely. Hardwiring that linear component leaves the net a
    # smaller, better-conditioned ageostrophic residual to fit. The baseline
    # is clipped to +/-3 m/s (beyond any physical surface current) so a rare
    # outlier row cannot inject an extreme residual target. Because the
    # baseline is a fixed per-row additive term, residual val MSE ==
    # final-prediction val MSE: early stopping, EMA-vs-raw selection, and
    # the ensemble val-loss guard all still optimize the scored quantity.
    ...
    # Residual targets; masked components are zeroed (never enter the loss).
