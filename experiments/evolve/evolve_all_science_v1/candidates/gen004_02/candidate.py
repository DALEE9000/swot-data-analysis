...
def _featurize(X, mean, scale, n_cells):
    """Standardize (NaN -> feature mean/0) and append per-cell missingness.

    Feature-major layout: column f * n_cells + c is feature f at stencil
    cell c, so reshaping to (n, F, C) and averaging NaN-ness over the
    feature axis yields one missing-fraction per spatial cell. Interior
    rows get all-zero flags — the network's behavior there is unchanged —
    while swath-edge rows get an explicit signal distinguishing padding
