...
def _augment_edges(X_raw, rng, n_cells, frac=0.25):
    """Structured edge-truncation augmentation on RAW (pre-featurize) rows.

    The diagnostics' widest gap is edge-stencil rows (r2_u 0.2886 vs 0.4618
    full-stencil). Real edge rows are a minority of training data and sit in
    physically noisier locations, so the net undersamples the edge-conditional
    input distribution. This synthesizes more of it with clean supervision:
    sample frac of the rows, and for each NaN-out one full side of the k x k
    stencil (left/right/top/bottom line of cells, across ALL features) —
    exactly the truncation geometry real swath-edge padding produces. The
    center cell is never on a side, so the mirror imputation downstream stays
    well-posed, and the copies then flow through the SAME _featurize path as
    real edge rows: identical imputed values, missingness fractions, damped
    spatial std and damped derivatives. Targets and recency weights are
    inherited from the source rows.

    Returns (source_indices, augmented_raw_rows) or None if the stencil is
    degenerate or frac rounds to zero rows.
    """
    k = int(round(math.sqrt(n_cells)))
    ...
    if n_aug == 0:
        return None
    sel = rng.choice(n, size=n_aug, replace=False)
    Xa = X_raw[sel].copy()
    grid = np.arange(n_cells).reshape(k, k)
    sides = [grid[:, 0], grid[:, -1], grid[0, :], grid[-1, :]]
    side = rng.integers(0, 4, size=n_aug)
    for s in range(4):
        rows = np.where(side == s)[0]
        if len(rows) == 0:
            continue
        cols = (np.arange(F)[:, None] * n_cells + sides[s][None, :]).ravel()
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # Stats are computed on REAL rows only — augmentation happens after.
    ...
    # Edge-truncation augmentation: append synthetic edge rows (25% of the
    # train split, one stencil side masked in RAW space, re-featurized with
    # the same stats and path as real rows) to the TRAINING tensors only.
    # Targets, masks and recency weights are inherited from the source rows
    # (train_idx is a contiguous prefix, so source indices line up directly).
    # Validation stays 100% real rows: early stopping and the member guard
    # keep measuring genuine forward-in-time generalization, and the fine-
    # tune (which concatenates X_t with the val tail) inherits the augmented
    # rows with consistent weighting for free.
    aug_rng = np.random.default_rng(seed + 777)
    aug = _augment_edges(X_train[:n_tr], aug_rng, n_cells, frac=0.25)
    if aug is not None:
        sel, Xa_raw = aug
        Xa = _featurize(Xa_raw, mean, scale, n_cells)
        X_t = torch.cat([X_t, torch.from_numpy(Xa)])
        Y_t = torch.cat([Y_t, torch.from_numpy(Ys[sel])])
        M_t = torch.cat([M_t, torch.from_numpy(mask[sel])])
        W_t = torch.cat([W_t, torch.from_numpy(w_all[sel])])
        del Xa, Xa_raw
