...
class HeteroMLP(nn.Module):
    ...
    decorrelation is what the (now per-component-weighted) prediction
    blend harvests. Notably the stronger-r2_u member in the lineage."""
...
def _val_preds(net, X_v, bs):
    """Residual-space mu predictions of `net` on the whole val block,
    returned on CPU as float32 (n_val, 2). Used to fit the per-component
    stacking weights — THE one change vs the parent."""
    net.eval()
    outs = []
    with torch.no_grad():
        for i in range(0, len(X_v), bs):
...
def _train_member(member_seed, arch, deadline, device, max_epochs,
    ...
    short-horizon EMA. Returns (net, best_val, val_pred) where val_pred is
    the BEST-STATE (pre-fine-tune) prediction on the val block — captured
    before the fine-tune absorbs val, so the stacking weights fitted from
    it downstream are unbiased."""
    ...
    # Capture the selected (pre-fine-tune) member's val predictions NOW:
    # the fine-tune below trains on val rows, so any weights fitted from
    # post-fine-tune val errors would favor whichever member overfit val
    # harder. These pre-fine-tune errors are honest out-of-sample errors,
    # and the fine-tune is a short low-LR nudge that preserves the members'
    # per-component error structure.
    val_pred = _val_preds(net, X_v, bs)
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # one architecture; the per-component stacking below is what turns that
    # decorrelation into R^2. Distinct seeds per member so data order and
    # init both differ.
    archs = ["conv", "mlp"]
    M_ens = 2
    members = []
    for m in range(M_ens):
        ...
        if time.time() > deadline:
            break
        net, best_val, val_pred = _train_member(
            seed + 101 * m, archs[m % len(archs)], deadline, device, max_epochs,
            n_feat, n_cells, comp_w,
            X_t, Y_t, M_t, W_t, X_v, Y_v, M_v,
            X_v_cpu, Y_v_cpu, M_v_cpu, W_v_cpu)
        members.append((net, best_val, val_pred))
    ...
    finite = [(net, v, vp) for net, v, vp in members if np.isfinite(v)]
    if finite:
        v_best = min(v for _, v, _ in finite)
        keep = [(net, vp) for net, v, vp in finite if v <= v_best * 1.15]
    else:
        keep = [(members[0][0], members[0][2])]

    # THE ONE CHANGE vs the parent: per-component optimal stacking instead
    # of a uniform member average. For two predictors with per-component
    # val error second moments s1, s2 and cross moment c12, the MSE-optimal
    # blend weight on member 0 is w* = (s2 - c12) / (s1 + s2 - 2*c12) —
    # this accounts for the members' error CORRELATION, which plain
    # inverse-variance weighting ignores and which is substantial for two
    # nets trained on the same data. Fitted from the pre-fine-tune val
    # errors (unbiased: captured before the fine-tune absorbed val), on
    # ~380k rows per component, so estimation noise is negligible. Clipped
    # to [0, 1] so a noisy cross moment can never produce a variance-
    # increasing extrapolated blend; any degenerate case (one member,
    # tiny mask, zero denominator) falls back to the parent's uniform
    # average exactly. The u blend leans toward the u-stronger MLP and the
    # v blend toward the v-stronger conv simultaneously — buying mean-R^2
    # on both components at zero training cost.
    Yv_np = Y_v_cpu.numpy()
    Mv_np = M_v_cpu.numpy()
    if len(keep) == 2:
        w0 = np.full(2, 0.5, dtype=np.float32)
        for c in range(2):
            r = Mv_np[:, c] > 0
            if r.sum() > 1000:
                e1 = keep[0][1][r, c] - Yv_np[r, c]
                e2 = keep[1][1][r, c] - Yv_np[r, c]
                s1 = float(np.mean(e1 * e1))
                s2 = float(np.mean(e2 * e2))
                c12 = float(np.mean(e1 * e2))
                den = s1 + s2 - 2.0 * c12
                if den > 1e-12:
                    w0[c] = float(np.clip((s2 - c12) / den, 0.0, 1.0))
        Wnp = np.stack([w0, 1.0 - w0]).astype(np.float32)  # (2 members, 2 comps)
    else:
        Wnp = np.ones((len(keep), 2), dtype=np.float32) / max(len(keep), 1)
    W_blend = torch.from_numpy(Wnp).to(device)

    # Predict: ridge baseline + per-component weighted residual mu blend
    # over accepted members.
    ...
    with torch.no_grad():
        for i in range(0, len(X_test), 65536):
            ...
            for j, (net, _) in enumerate(keep):
                mu, _ = net(xb)
                acc += mu * W_blend[j].unsqueeze(0)
