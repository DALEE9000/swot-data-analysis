...
class ConvWideDeep(nn.Module):
    """NEW predictive core: conv path over the raw stencil image + dense
    path over the engineered scalars, joined into one trunk.
    ...
    the second layer already covers the whole window, so this is a learned
    generalization of the hand-coded gx/gy/Laplacian bank, not a deep
    vision net (~20k conv params, cheap on a 4 GB GPU).
...
def _train_member(member_seed, deadline, device, max_epochs,
    ...
    short-horizon EMA. Identical to the lineage except the net is the new
    conv-wide-and-deep core. Returns (net, best_val)."""
