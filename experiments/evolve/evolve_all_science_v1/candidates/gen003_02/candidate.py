...
def _prep(X, mean, scale):
    """Standardize, impute NaN->0 (the feature mean), and append binary
    missingness indicators so the net can distinguish padded stencil cells
    from genuinely mean-valued features."""
    X = X.astype(np.float32, copy=False)
    miss = np.isnan(X).astype(np.float32)
    Xs = (X - mean) / scale
    Xs = np.nan_to_num(Xs, nan=0.0, posinf=0.0, neginf=0.0)
    return np.concatenate([Xs, miss], axis=1)
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # Recency fine-tune (grafted from gen002_00, the proven win): the
    # validation tail is the most recent — and hence most test-like — 10% of
    # the training window, and the model above never trained on it. After
    # model selection, absorb it with a short low-LR pass over the FULL
    # window (tail included). Low LR + few epochs keeps the selected solution
    # basin intact while adapting to the freshest ocean state; there is no
    # validation signal during this phase by construction, so it is
    # deliberately conservative and budget-checked in-loop.
    if time.time() - t0 < time_budget_s:
    ...
    # Predict on the test set in batches (augment per batch to bound memory).
    out = np.empty((len(X_test), 2), dtype=np.float32)
    with torch.no_grad():
        for i in range(0, len(X_test), 65536):
            xb = torch.from_numpy(_prep(X_test[i:i + 65536], mean, scale)).to(device)
