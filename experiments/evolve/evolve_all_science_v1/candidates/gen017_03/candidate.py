...
def _featurize(X, mean, scale, n_cells):
    ...
    Proven plumbing (unchanged from the lineage):
      ...
      * when F == 8 (the pooled layout [mdt, ssha, ugos, vgos, ugosa,
        ...
        |u_wind|, relative vorticity zeta = dv/dx - du/dy, normal strain
        sn = du/dx - dv/dy, shear strain ss = dv/dx + du/dy, the
        Okubo-Weiss parameter W = sn^2 + ss^2 - zeta^2, and geostrophic
        advection u_g * gx_f + v_g * gy_f of every feature (n, F).
    ...
    layer. The regime block doubles as the gating signal for the
    mixture-of-experts: speed, vorticity, strain, and Okubo-Weiss are the
    classic discriminants between quiescent flow and fronts/eddies.
    """
...
class MoEHetero(nn.Module):
    """Gated mixture-of-experts over flow regimes with heteroscedastic heads.

    NEW predictive core (the wildcard). Motivation: the diagnostics show a
    3.3x RMSE gap between fast rows (0.2278) and slow rows (0.0685). Slow
    flow is close to quasi-linear geostrophic balance; fast flow lives in
    fronts and eddies with strongly nonlinear dynamics. A single shared
    trunk must average these regimes; a soft mixture lets specialists
    develop per-regime weights while the gate — conditioned on the full
    engineered input, including speed/vorticity/strain/Okubo-Weiss — learns
    the partition end-to-end.

    Structure:
      encoder: Linear(d, 256) + LN + SiLU + Dropout — shared representation.
      gate:    Linear(256, E) -> softmax — soft regime assignment per row.
      experts: E independent MLPs 256 -> 192 -> 128, each with its own
               mu (2) and logvar (2) heads.
    Outputs are the gate-weighted mixture:
      mu     = sum_e g_e * mu_e            (what the harness scores)
      logvar = sum_e g_e * logvar_e        (loss weighting only; the linear
               mixture of logvars is a geometric-mean variance — a
               conservative, well-behaved approximation that suffices since
               logvar only reweights the loss, it is never scored)
    All experts run densely on every row (E=4 is cheap at this width; no
    routing sparsity needed on 4k batches), so gradients stay smooth and
    the EMA/ensemble plumbing works unchanged. forward() keeps the parent's
    (mu, logvar) return signature; the training loop requests the gate
    probabilities separately for the load-balancing penalty.
    """

    def __init__(self, n_inputs, n_experts=4, enc_dim=256,
                 expert_hidden=(192, 128), dropout=0.1, logvar_init=-3.5):
        super().__init__()
        self.n_experts = n_experts
        self.encoder = nn.Sequential(
            nn.Linear(n_inputs, enc_dim), nn.LayerNorm(enc_dim),
            nn.SiLU(), nn.Dropout(dropout))
        self.gate = nn.Linear(enc_dim, n_experts)
        # Zero-init gate weights -> uniform mixture at step 0, so every
        # expert receives gradient from the start and specialization emerges
        # from the data rather than from init noise.
        ...
        for _ in range(n_experts):
            ...
            # and early training behaves like plain (scaled) MSE.
            nn.init.zeros_(lv.weight)
            nn.init.constant_(lv.bias, logvar_init)
            self.mu_heads.append(mu)
    ...
    def _mix(self, x):
        ...
        for e in range(self.n_experts):
            h = self.experts[e](z)
            mus.append(self.mu_heads[e](h))
            lvs.append(self.lv_heads[e](h))
        mu_s = torch.stack(mus, dim=1)   # (B, E, 2)
        lv_s = torch.stack(lvs, dim=1)   # (B, E, 2)
        w = g.unsqueeze(2)
        mu = (w * mu_s).sum(dim=1)
        lv = (w * lv_s).sum(dim=1)
        return mu, lv, g
...
def _load_balance(g, n_experts):
    """Importance load-balancing penalty (Shazeer-style, simplified).

    g: (B, E) softmax gate probabilities. Penalize the squared batch-mean
    importance so no expert monopolizes the gate: E * sum(imp^2) is 1.0 at
    perfect balance and E at total collapse; subtracting 1 makes the penalty
    zero at balance. Small coefficient (applied by the caller) — enough to
    prevent collapse early, weak enough that a genuinely skewed regime
    distribution (most ocean rows ARE slow) can still be expressed.
    """
    imp = g.mean(dim=0)
    return float(n_experts) * (imp * imp).sum() - 1.0
...
class _EMA:
    ...
    of the net plus one lerp per optimizer step.
...
def _train_member(member_seed, deadline, device, max_epochs,
    ...
    """Train one MoE ensemble member with the recency-weighted anchored
    heteroscedastic NLL plus a small gate load-balancing penalty. Each
    epoch, BOTH the raw net and its weight-EMA shadow are scored on the
    UNWEIGHTED temporal-val masked MSE of mu — the scored quantity — and
    model selection keeps the better of the two; the LR schedule still
    steps on the raw net's val loss (its training signal). Then the proven
    recency fine-tune runs, ending on a short-horizon EMA of the fine-tune
    trajectory rather than the last minibatch's weights.
    Returns (net, best_val)."""
    torch.manual_seed(member_seed)
    rng = np.random.default_rng(member_seed)

    net = MoEHetero(X_t.shape[1], n_experts=4, enc_dim=256,
                    expert_hidden=(192, 128), dropout=0.1).to(device)
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for i in range(0, n_train, bs):
            ...
            optimizer.zero_grad()
            mu, lv, g = net.forward_with_gate(xb)
        ...
        # variance term, the recency weighting, and the balance penalty.
        # Both the raw and EMA weights compete; early in training the EMA
        # lags and loses, near convergence it usually wins. The scheduler
        # keeps stepping on the raw net's val loss so LR decisions reflect
        # the weights actually being optimized.
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # seeded nets make decorrelated errors — including in their learned gate
    # partitions and variance surfaces — and averaging their mu predictions
