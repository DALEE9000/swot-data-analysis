...
def _featurize(X, mean, scale, n_cells):
    ...
    NOTE: the edge-augmentation pass (see train_and_predict) feeds synthetic
    truncated rows through THIS function unchanged, so augmented rows are
    featurized byte-identically to real swath-edge rows.
    """
...
def _train_member(member_seed, arch, deadline, device, max_epochs,
    ...
    recency- and component-weighted anchored heteroscedastic NLL.

    Budget-aware cosine LR schedule (proven in the parent): after the first
    ...
    floor on a cosine over exactly that horizon (capped by max_epochs), so
    every member finishes its slice fully annealed. Because the horizon is
    sized from the MEASURED epoch cost, the ~15% extra rows contributed by
    edge augmentation are absorbed automatically. Selection (raw-vs-EMA on
    component-weighted temporal-val masked MSE), patience-15 backstop, and
    the recency fine-tune are unchanged."""
    ...
    # on a short-horizon EMA of the fine-tune trajectory. Note X_t here
    # includes the augmented edge rows — the fine-tune keeps seeing them,
    # so the edge robustness is not washed out by the final pass.
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # temporal test holdout measures. The val split contains ONLY real rows
    # (augmented rows below go into train only), so selection stays an
    # unbiased estimate of the scored distribution.
    ...
    # ---- Edge augmentation (THE one change vs the parent) ----
    # The edge-stencil bins are the parent's largest deficit (r2_u 0.287 vs
    # 0.476 full-stencil, r2_v 0.358 vs 0.562). Real edge rows are a noisy
    # minority; here we manufacture clean supervision for the truncated-
    # stencil regime: duplicate 15% of FULLY-VALID train rows, NaN out one
    # random stencil side in RAW input space (30% of the time an adjacent
    # side too — a corner cut; real swath truncation removes sides/corners
    # but never the center observation), and run the copies through the
    # SAME _featurize — so mirror imputation, the conv mask channel, and
    # every derivative block match genuine edge rows exactly. Residual
    # targets use the ridge baseline of the TRUNCATED features, matching
    # what the baseline produces at real edges. Val stays clean; the
    # budget-aware cosine absorbs the ~15% epoch-cost increase.
    k = int(round(math.sqrt(n_cells)))
    Xa = Ya = Ma = Wa = None
    if k >= 3 and k * k == n_cells:
        full_ok = ~np.isnan(X_train).any(axis=1)
        cand = train_idx[full_ok[train_idx]]
        n_aug = int(0.15 * len(train_idx))
        if len(cand) > 1000 and n_aug > 0:
            sel = rng0.choice(cand, size=min(n_aug, len(cand)), replace=False)
            Xr = X_train[sel].copy()
            sides = [
                [r * k for r in range(k)],            # 0: left column
                [r * k + k - 1 for r in range(k)],    # 1: right column
                list(range(k)),                       # 2: top row
                list(range((k - 1) * k, k * k)),      # 3: bottom row
            ]
            side_id = rng0.integers(0, 4, size=len(sel))
            corner = rng0.random(len(sel)) < 0.3
            # Second side orthogonal to the first, so corners are L-shaped.
            second = np.where(side_id < 2,
                              rng0.integers(2, 4, size=len(sel)),
                              rng0.integers(0, 2, size=len(sel)))
            for s in range(4):
                cols = [f * n_cells + c for f in range(n_feat) for c in sides[s]]
                rows = np.where(side_id == s)[0]
                if len(rows):
                    Xr[np.ix_(rows, cols)] = np.nan
                rows2 = np.where(corner & (second == s))[0]
                if len(rows2):
                    Xr[np.ix_(rows2, cols)] = np.nan
            Xa = _featurize(Xr, mean, scale, n_cells)
            del Xr
            base_a = _baseline(Xa)
            Ma = mask[sel]
            Ya = np.where(Ma > 0,
                          np.nan_to_num(Y_train[sel].astype(np.float32),
                                        nan=0.0) - base_a,
                          0.0).astype(np.float32)
            Wa = w_all[sel]  # keep each source row's recency weight
            del base_a

    if Xa is not None:
        X_t = torch.from_numpy(np.vstack([Xs[train_idx], Xa]))
        Y_t = torch.from_numpy(np.vstack([Ys[train_idx], Ya]))
        M_t = torch.from_numpy(np.vstack([mask[train_idx], Ma]))
