...
def _featurize(X, mean, scale, n_cells):
    ...
    Grafted verbatim from the gen031_00 lineage (its biggest documented
    single win over plain NaN->0 + indicator prep):
...
class MLP(nn.Module):
    """Joint (u, v) MLP: shared trunk, 2-unit head (unchanged from parent)."""
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # THE ONE CHANGE: replace the parent's _prep (NaN->0 + binary
    # missingness indicators) with the lineage's proven physics-aware
    # featurization. Chunked to bound peak memory (~3.8M x ~152 output
    # columns built 1M rows at a time).
    n_cells = _cells(X_train.shape[1])
    n = len(X_train)
    chunk = 1_000_000
    first = _featurize(X_train[:min(chunk, n)], mean, scale, n_cells)
    Xs = np.empty((n, first.shape[1]), dtype=np.float32)
    Xs[:len(first)] = first
    del first
    for i in range(chunk, n, chunk):
        Xs[i:i + chunk] = _featurize(X_train[i:i + chunk], mean, scale, n_cells)
