...
def _masked_nll_anchored(mu, logvar, target, mask, row_w, anchor_w):
    """Gaussian NLL plus a SPEED-WEIGHTED MSE anchor, per valid component,
    ...
    and it is WORST on fast rows, because the residual sum in R^2 is
    dominated by exactly the high-|y| rows the variance head downweights
    hardest (fast-regime rmse runs ~3.3x the slow-regime rmse).

    anchor_w (shape (batch,)) is a per-row anchor multiplier in [1, 4]:
    1 + 2*clip(speed/p90, 0, 1.5), computed once from the valid target
    components. On the slow majority it is exactly 1, so the parent's tuned
    anchor behavior there is bit-for-bit unchanged; on energetic rows the
    mu head keeps a gradient floor 2-4x stronger, counteracting — but not
    overriding — the NLL's variance-based downweighting. The variance head
    itself is untouched, so calibration and relative NLL weighting survive.
    ...
    per = 0.5 * (logvar + err2 * torch.exp(-logvar)) + err2 * anchor_w.unsqueeze(1)
...
def _train_member(member_seed, deadline, device, max_epochs,
                  X_t, Y_t, M_t, W_t, A_t, X_v, Y_v, M_v,
                  X_v_cpu, Y_v_cpu, M_v_cpu, W_v_cpu, A_v_cpu):
    """Train one ensemble member with the recency-weighted, speed-anchored
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for i in range(0, n_train, bs):
            ...
            optimizer.zero_grad()
            mu, lv = net(xb)
            loss = _masked_nll_anchored(mu, lv, yb, mb, wb, ab)
            loss.backward()
            optimizer.step()
            ema.update(net)
    ...
    # same recency-weighted, speed-anchored loss so relative row weighting
    # stays consistent (the tail carries the largest recency weights, as it
    # should, and fast rows keep their anchor emphasis).
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # Speed-proportional anchor weights (new this generation). The R^2 metric
    # loses most of its residual mass on high-|y| rows, but the NLL's
    # variance head downweights exactly those rows; the flat anchor only
    # floors them at weight 1. Per-row anchor multiplier
    #   a = 1 + 2 * clip(speed / p90, 0, 1.5)   in [1, 4]
    # keeps the slow majority at exactly 1 (parent behavior unchanged) and
    # gives energetic rows a 2-4x stronger mu-gradient floor. Speed uses the
    # valid components (a missing component contributes 0 — a mild
    # underestimate on single-component rows, which only makes the weight
    # conservative there). p90 is data-driven so the ramp adapts to the
    # pooled multi-region speed distribution.
    spd = np.sqrt(Ys[:, 0] ** 2 + Ys[:, 1] ** 2).astype(np.float32)
    s90 = float(np.quantile(spd[np.isfinite(spd)], 0.9)) if np.isfinite(spd).any() else 0.0
    if not np.isfinite(s90) or s90 < 1e-3:
        s90 = 0.3
    a_all = (1.0 + 2.0 * np.clip(spd / s90, 0.0, 1.5)).astype(np.float32)
    ...
    A_t = torch.from_numpy(a_all[train_idx])
    ...
    A_v_cpu = torch.from_numpy(a_all[val_idx])
    ...
    # slack this loop never runs and the module degenerates exactly to the
    # two-member parent.
