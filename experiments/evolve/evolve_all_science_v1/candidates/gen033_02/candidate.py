...
def _masked_nll_anchored(mu, logvar, target, mask, row_w, comp_w):
    ...
    scaled by a per-row weight (recency ramp x edge-stencil boost, see
...
def _masked_mse(pred, target, mask, comp_w):
    ...
    optimize the metric directly instead of pooled MSE. Deliberately NOT
    row-weighted: the edge boost shapes training gradients only, never
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # Per-row edge flag BEFORE featurization consumes the NaNs: a row with
    # ANY missing stencil cell belongs to the edge-stencil bin.
    row_edge = (np.isnan(X_train).sum(axis=1) > 0).astype(np.float32)
    ...
    # THE ONE CHANGE: edge-stencil loss boost. Edge rows are the worst
    # held-out bins by a wide margin (r2_u 0.287 / r2_v 0.359 vs 0.476 /
    # 0.562 full-stencil), yet the heteroscedastic NLL learns HIGH variance
    # on exactly those rows and scales their squared-error gradient by
    # exp(-logvar) — an implicit downweighting that lets the net neglect
    # the mask channel and imputation features it was given to handle them.
    # Multiply the per-row loss weight by 1.75 on rows with any missing
    # stencil cell (binary — matches the diagnostic bin definition), then
    # re-normalize the combined weight to mean 1 so the tuned loss scale is
    # unchanged. Validation selection (_val_mse) stays row-unweighted, so
    # early stopping, raw-vs-EMA choice, and the member guard still
    # optimize the true scored quantity — the boost shapes gradients only.
    w_all *= (1.0 + 0.75 * row_edge)
    w_all /= w_all.mean()
