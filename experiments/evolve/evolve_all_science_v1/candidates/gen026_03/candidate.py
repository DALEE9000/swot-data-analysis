...
def _featurize(X, mean, scale, n_cells):
    ...
    Proven plumbing (unchanged from the lineage):
      ...
      * flow-regime block when F == 8 (speeds, vorticity, strains,
        Okubo-Weiss, geostrophic advection) — this block is what the MoE
        gate exploits to route rows by dynamical regime.
    """
...
class RegimeMoE(nn.Module):
    """WILDCARD CORE: soft mixture-of-experts over flow regimes.

    A shared two-layer encoder builds a representation from the full
    engineered vector (which includes speeds, vorticity, strain,
    Okubo-Weiss, and missing fractions — the regime descriptors). From it:
      * a gate head produces softmax weights over K=4 experts;
      * each expert is a small independent head predicting a (u, v) mu;
      * the prediction is the gate-weighted mixture of expert mus;
      * one shared logvar head keeps the lineage's anchored-NLL loss.

    Rationale: slow geostrophic-balance rows, fast front/eddy rows, and
    edge-missingness rows want different input->velocity mappings; a single
    trunk averages their conflicting gradients. Soft routing lets experts
    specialize while staying fully differentiable and cheap (experts only
    branch after the shared encoder). `aux_lb` (switch-style importance
    penalty, K * sum(mean_gate^2), minimized at uniform usage) is returned
    for the training loss to prevent gate collapse onto one expert.
    """

    def __init__(self, n_inputs, n_experts=4, enc=(256, 256), expert_h=128,
        ...
        self.experts = nn.ModuleList([
            nn.Sequential(nn.Linear(d, expert_h), nn.LayerNorm(expert_h),
                          nn.SiLU(), nn.Dropout(dropout),
                          nn.Linear(expert_h, 2))
            for _ in range(n_experts)
        ])
        self.gate = nn.Sequential(nn.Linear(d, 64), nn.SiLU(),
    ...
    def forward(self, x):
        ...
        mus = torch.stack([e(z) for e in self.experts], dim=1)  # (b, K, 2)
        mu = (g.unsqueeze(-1) * mus).sum(dim=1)                 # (b, 2)
        imp = g.mean(dim=0)
        aux_lb = self.K * (imp * imp).sum()
        return mu, self.logvar(z), aux_lb
...
def _masked_nll_anchored(mu, logvar, target, mask, row_w, comp_w):
    ...
    scaled by per-row recency weight and per-component inverse-variance
...
def _masked_mse(pred, target, mask, comp_w):
    """Component-weighted masked MSE — proportional to the mean per-
    component R^2 deficit, so all selection optimizes the scored metric."""
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # Per-component inverse-variance weights (proven +0.002): make the
    # loss's u/v exchange rate match mean-R^2's. Mean-1 normalized, clipped.
    ...
    # ---- Linear baseline + residual learning (unchanged plumbing) ----
    # Ridge from the full engineered block to (u, v) on a seeded subsample;
    # the net fits the residual. Additive per-row baseline preserves the
    # MSE identity, so all selection still optimizes the scored quantity.
    ...
    # ---- Single full-budget MoE (replaces the two-member ensemble) ----
    # The whole wall-clock slice funds one routed model; what this run
    # measures is the value of regime routing, not ensemble variance
    # reduction.
    rng = np.random.default_rng(seed + 1)
    net = RegimeMoE(X_t.shape[1], n_experts=4).to(device)
    ...
    bs = 4096
    patience = 15
    lb_coef = 0.01  # importance-penalty weight; small so routing stays free
    best_val = float("inf")
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for i in range(0, n_train, bs):
            ...
            optimizer.zero_grad()
            mu, lv, aux_lb = net(xb)
            loss = _masked_nll_anchored(mu, lv, yb, mb, wb, comp_w) + lb_coef * aux_lb
            loss.backward()
            optimizer.step()
            ema.update(net)
