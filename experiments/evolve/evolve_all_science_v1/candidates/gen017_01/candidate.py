...
def _featurize(X, mean, scale, n_cells):
    ...
    spatial std + directional stencil gradients + second-order stencil
    operators.
    ...
    neighborhood. Ingredients:
      ...
      * directional gradients (n, 2F), computed AFTER imputation: per
        ...
        fast-regime deficit. Computing after mirror-fill means edge rows get
        their linearly-continued gradient rather than a truncated one;
      * per-feature SECOND-order operators (n, 3F), new this generation and
        proven at the top of the lineage: the 5-point Laplacian
        ...
        Taylor-LINEAR, filled cells satisfy fill + mirror - 2*center = 0, so
        at swath edges the second differences collapse toward zero — a
    ...
    for c in range(n_cells):
        ...
        if hole.any():
            ...
            col[hole] = fill[hole]
    # Derivative operators on the imputed (still-standardized) stencil.
    # Cells are flattened row-major over the k x k window (the ordering the
    # mirror trick relies on): the center's horizontal neighbors sit at
    # +/-1, vertical neighbors at +/-k, diagonals at +/-(k+1) and +/-(k-1).
    # Orientation and sign conventions don't matter to the net — only that
    # the directions are consistent and (for the diagonals) mutually
    # orthogonal.
    ...
    if k >= 2 and k * k == n_cells:
        ...
        if k >= 3:
            ci = n_cells // 2
            lap = (Z[:, :, ci + 1] + Z[:, :, ci - 1] + Z[:, :, ci + k]
                   + Z[:, :, ci - k] - 4.0 * Z[:, :, ci]).astype(np.float32)
            inv2 = np.float32(0.5 / math.sqrt(2.0))
            gp = (inv2 * (Z[:, :, ci + k + 1] - Z[:, :, ci - k - 1])).astype(np.float32)
            gq = (inv2 * (Z[:, :, ci - k + 1] - Z[:, :, ci + k - 1])).astype(np.float32)
            pieces += [lap, gp, gq]
        grad = np.hstack(pieces)
    else:
