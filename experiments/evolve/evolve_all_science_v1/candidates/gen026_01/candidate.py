...
def _featurize(X, mean, scale, n_cells):
    ...
    consumes the whole vector flat. The train-time edge augmentation
    mutates exactly those two blocks (image + mask) with the same
    imputation rule used here.
    """
...
def _edge_dropout(xb, n_feat, n_cells, p_row=0.2):
    """THE new mechanism: swath-edge truncation augmentation, in place on a
    training batch already on the device.

    With probability p_row per row, one full side of the k x k stencil
    (left/right column or top/bottom row — the exact geometry real swath
    edges produce, unlike iid cell dropout) is treated as missing: its
    image cells are refilled with the SAME rule the featurizer uses for
    real holes (clip(2*center - mirror, +/-4 sigma); the mirror side is the
    opposite side, which stays valid — exactly as at a real edge), and the
    per-cell mask channel is set to 1 so the conv path sees a bona fide
    edge signature. Interior rows thus become synthetic edge rows that
    keep their true targets, multiplying edge-like training exposure
    severalfold. Engineered scalar blocks are left untouched (at real
    edges they are computed from the same imputed values, so the filled
    image is consistent with them to first order). Validation and test
    batches are never augmented."""
    C = n_cells
    k = int(round(math.sqrt(C)))
    if k < 3 or k * k != C:
        return
    B = xb.shape[0]
    dev = xb.device
    sel = torch.rand(B, device=dev) < p_row
    if not bool(sel.any()):
        return
    side = torch.randint(0, 4, (B,), device=dev)
    ar = torch.arange(C, device=dev)
    rr, cc = ar // k, ar % k
    side_masks = torch.stack([cc == 0, cc == k - 1, rr == 0, rr == k - 1])
    drop = side_masks[side] & sel.unsqueeze(1)  # (B, C)
    ci = C // 2
    drop[:, ci] = False
    img_dim = n_feat * C
    img = xb[:, :img_dim].view(B, n_feat, C)
    center = img[:, :, ci].unsqueeze(2)
    fill = torch.clamp(2.0 * center - img.flip(2), -4.0, 4.0)
    dexp = drop.unsqueeze(1).expand_as(img)
    img[dexp] = fill[dexp]
    msk = xb[:, img_dim:img_dim + C]
    msk[drop] = 1.0
...
class HeteroMLP(nn.Module):
    ...
    stronger-r2_u member in the lineage."""
...
def _masked_nll_anchored(mu, logvar, target, mask, row_w, comp_w):
    ...
    weight (grafted verbatim from gen026_00). The score is mean per-
    component R^2 = mean_c(1 - MSE_c/Var_c), so a unit of squared error on
    the lower-variance component (u) buys more R^2 than the same error on
    v; comp_w = (1/Var_u, 1/Var_v) mean-1 normalized makes the loss's
    exchange rate between u and v errors match the metric's. The anchor
    keeps a gradient floor on variance-downweighted rows, as in the
    lineage."""
...
def _train_member(member_seed, arch, deadline, device, max_epochs,
    ...
    recency- and component-weighted anchored heteroscedastic NLL and the
    swath-edge truncation augmentation on every training batch; selection
    over raw-vs-EMA on component-weighted temporal-val masked MSE (the
    scored quantity, never augmented); then the proven recency fine-tune
    (also augmented) ending on a short-horizon EMA. Returns (net, best_val)."""
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for i in range(0, n_train, bs):
            ...
            _edge_dropout(xb, n_feat, n_cells, p_row=0.2)
            optimizer.zero_grad()
            mu, lv = net(xb)
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # Per-component inverse-variance weights (grafted from gen026_00).
    ...
    # Mean-1 normalized so the tuned loss scale is unchanged; clipped to
    # [0.5, 2] relative so a pathological variance ratio cannot
