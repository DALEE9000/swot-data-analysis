...
def _featurize(X, mean, scale, n_cells):
    ...
    per-feature per-cell NaN mask, per-feature spatial std, and explicit
    per-feature central differences.
    ...
    cell c. Output layout (consumed by the CNN, which reshapes it):
      [Xs (F*C, imputed values), mask (F*C, 1.0 where the cell was NaN),
       ...
       gx (F), gy (F) — central differences across the stencil].
    ...
    Central differences (grafted from the top MLP program): gx =
    (right - left)/2 and gy = (down - up)/2 per feature, computed AFTER
    imputation. These are the finite-difference operators whose
    cross-feature linear combos are shear, strain, divergence and vorticity
    — the front/eddy dynamics behind the fast-regime deficit. They are fed
    STRAIGHT to the head (bypassing the conv stack), so the derivative
    signal does not have to survive two convolutions and a bottleneck. The
    mirror imputation is Taylor-consistent with a central difference, so
    edge rows get coherent, conservatively damped derivatives; the mask
    channels tell the net when to trust them less.
    """
    ...
    blocks = [Xs, mask_full.reshape(n, F * n_cells), spat]
    # Central differences on the filled, finite stencil. Cells are flattened
    # row-major over the k x k window: horizontal neighbors of the center sit
    # at +/-1, vertical at +/-k. Only consistency of the two directions
    # matters, not orientation/sign conventions.
    ...
    if k >= 2 and k * k == n_cells:
        ...
        blocks += [np.zeros((n, F), np.float32), np.zeros((n, F), np.float32)]
    return np.hstack(blocks)


class StencilCNN(nn.Module):
    """Per-feature 2D stencil CNN with a heteroscedastic head and a direct
    derivative path.

    Input (flat, from _featurize) is split into a (F, k, k) value image, a
    (F, k, k) missing-mask image, F spatial-std scalars, and 2F explicit
    central-difference scalars (gx, gy per feature). Values and mask are
    stacked into 2F channels; two small convolutions turn the k x k
    neighborhood into a 128-dim descriptor. The std and derivative scalars
    join that descriptor at the head input, so shear/strain/vorticity —
    linear combos of the gx/gy columns — reach the heads through one Linear
    layer instead of having to survive the conv stack.
    Heads:
    ...
    def __init__(self, F, n_cells, dropout=0.1, logvar_init=-3.5):
        ...
        d_in = 128 + 3 * F  # conv descriptor + spat (F) + gx (F) + gy (F)
    ...
    def forward(self, x):
        ...
        scalars = x[:, 2 * F * C:]  # spat (F) + gx (F) + gy (F)
        h = self.conv(torch.cat([vals, mask], dim=1)).flatten(1)
        z = self.head(torch.cat([h, scalars], dim=1))
...
def _train_member(member_seed, deadline, device, max_epochs, F, n_cells,
    ...
    heteroscedastic NLL (early stop on UNWEIGHTED temporal-val masked MSE of
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for bi, i in enumerate(range(0, n_train, bs)):
            ...
            loss.backward()
            optimizer.step()
            # Convs cost more per batch than an MLP; check the deadline
            # mid-epoch so a member cannot blow its slice.
            if bi % 200 == 0 and time.time() > deadline:
                out_of_time = True
                break

        # Model selection and LR scheduling on UNWEIGHTED masked MSE of mu —
        # the scored quantity — not the training loss.
        net.eval()
        with torch.no_grad():
            val_losses = []
