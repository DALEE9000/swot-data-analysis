...
def _featurize(X, mean, scale, n_cells):
    ...
    Plumbing carried over from the parent (unchanged):
    ...
    THE ONE CHANGE — the lineage's proven physics-derivative block, grafted
    verbatim after the parent's three blocks:
      * per-feature central differences gx, gy (n, 2F) — geostrophic
        velocity IS a spatial derivative of SSH, so giving the net the
        derivative directly removes a hard-to-learn linear map over cells;
      * per-feature 5-point Laplacian and both diagonal central differences
        (n, 3F) — curvature/eddy-core structure;
      * |grad f| per feature (n, F) — front intensity;
      * flow-regime block when F == 8 (speeds of geostrophic/anomaly/wind
        vectors, vorticity, normal & shear strain, Okubo-Weiss, geostrophic
        advection) — the nonlinear fast-front/eddy descriptors behind the
        parent's worst bin (fast-regime RMSE 0.2295).
    The mirror imputation runs BEFORE the derivatives, so edge rows get
    Taylor-continued (not zero-flattened) gradients, and the missingness
    block still lets the net discount imputed structure.
    """
