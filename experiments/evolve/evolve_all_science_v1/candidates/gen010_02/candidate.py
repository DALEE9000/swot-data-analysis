...
def _featurize(X, mean, scale, n_cells):
    ...
    neighborhood. Three ingredients:
      ...
      * gradient-preserving imputation: a missing stencil cell at offset
        ...
        to +/-4 standardized sigma so edge noise is not amplified; where the
        mirror is itself missing the fill falls back to the center value,
        and rows whose center is NaN fall back to the mean (0 after
        standardization) via the final nan_to_num. The missingness block
        still marks which cells were padded, so the model can discount
        imputed structure.
    """
...
def _ema_update(ema_net, net, decay):
    # Shadow-average parameters along the SGD trajectory. LayerNorm has no
    # running buffers, but copy buffers anyway so the helper stays correct if
    # the architecture grows some.
    with torch.no_grad():
        for pe, p in zip(ema_net.parameters(), net.parameters()):
            pe.mul_(decay).add_(p, alpha=1.0 - decay)
        for be, b in zip(ema_net.buffers(), net.buffers()):
            be.copy_(b)
...
def _train_member(member_seed, deadline, device, max_epochs,
    ...
    New in this generation: an EMA (exponential moving average, decay 0.999)
    shadow copy of the weights is maintained alongside the raw net. Weight
    averaging along the trajectory lands in flatter minima, which generalize
    better under distribution shift — and the test split IS a distribution
    shift (temporal holdout; the parent's r2_u drops from 0.4052 early-test
    to 0.3946 late-test). Each epoch BOTH the raw and the EMA weights are
    scored on the temporal-validation masked MSE of mu, and whichever is
    better becomes the selection candidate — the raw training trajectory is
    untouched, so this can only match or improve the parent's selection.
    The recency fine-tune then runs under a fresh EMA initialized at the
    selected weights and returns the averaged weights, replacing the
    parent's un-validated "wherever the last batch left us" ending with a
    smoothed one. Returns (net, best_val)."""
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for i in range(0, n_train, bs):
            ...
            optimizer.step()
            step += 1
            # Warmup-adjusted decay: early on the EMA tracks the raw weights
            # closely (a freshly initialized average would lag uselessly),
            # then settles at 0.999 (~1000-step / ~1-epoch horizon).
            _ema_update(ema_net, net, min(0.999, (1.0 + step) / (10.0 + step)))
        ...
        # variance term. Score raw AND EMA weights; keep the better.
        val_raw = _val(net)
        val_ema = _val(ema_net)
        val_loss = min(val_raw, val_ema)
        scheduler.step(val_loss)

        if val_loss < best_val - 1e-7:
            best_val = val_loss
            src = ema_net if val_ema <= val_raw else net
    ...
    # same anchored loss so relative row weighting stays consistent. The EMA
    # restarts at the selected weights (fixed 0.999 decay — weights are
    # already converged, no warmup needed) and its average is the final
    # model, so the member ends at a smoothed point of the fine-tune
    # trajectory instead of wherever the last batch happened to leave it.
