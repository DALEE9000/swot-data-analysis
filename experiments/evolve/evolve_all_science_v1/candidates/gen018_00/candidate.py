...
def _featurize(X, mean, scale, n_cells):
    ...
    order), and NONLINEAR flow-regime features (grafted from gen017_02).
    ...
    Nonlinear regime block (proven in gen017_02's lineage): the fast-regime
    deficit lives in fronts and eddies, whose signature is NONLINEAR in the
    derivatives — magnitudes and products a SiLU MLP approximates poorly far
    from the data bulk (exactly the fast tail). Hand them over explicitly,
    computed from the standardized, imputed stencil:
      ...
      * when F == 8 (pooled layout [mdt, ssha, ugos, vgos, ugosa, vgosa,
        ...
        (3), relative vorticity zeta = dv/dx - du/dy, normal strain
        ...
        feature (n, F) — the leading nonlinear momentum/tracer term.
    ...
    layer or the ridge normal equations. In THIS lineage the block does
    double duty: the ridge-initialized wide path is linear in these
    quadratic features, so the physics backbone itself gains flow-regime
    conditioning, and the logvar head gets direct regime conditioning for
    loss weighting on the fast rows.
    """
    ...
    if k >= 2 and k * k == n_cells:
        ...
        if k >= 3:
            ...
            blocks += [lap, gp, gq]
        # ---- nonlinear flow-regime block (grafted from gen017_02) ----
...
def _ridge_fit(Xs, Ys, mask, rng, n_sub=1_000_000, lam_rel=1e-3):
    ...
    model captures the physics backbone of this task. With the regime block
    appended, the "linear" fit is additionally linear in vorticity, strain,
    Okubo-Weiss, and advection — i.e. a second-order flow model, which is
    exactly the correction the fast regime needs. The ridge solution
    ...
    A 1M-row subsample is plenty for a ~150-dim linear fit and keeps the
...
class WideDeep(nn.Module):
    ...
    Two additive paths trained jointly end-to-end:
      wide : one Linear(n_inputs -> 2) straight from the augmented features
             to the targets — the geostrophic/linear backbone (now including
             the quadratic regime features, so a second-order flow model).
             Initialized from the closed-form ridge solution and trained
             with a 10x smaller LR and NO weight decay (decaying physics
             coefficients toward zero is the wrong prior), so it adapts
             slowly and stays a stable, time-robust anchor — linear maps
             generalize across the temporal train->test shift better than a
             deep net's co-adapted features, which is the late-test weakness.
