...
def _featurize(X, mean, scale, n_cells):
    ...
    spatial std, and explicit per-feature spatial derivatives up to second
    order.
    ...
    neighborhood. Ingredients:
      ...
      * per-feature central differences (n, 2F): gx = (right - left)/2 and
        ...
        anonymous columns;
      * per-feature SECOND derivatives (n, 2F), the new block: the 5-point
        Laplacian (right + left + up + down - 4*center) and the mixed
        cross-derivative ((ne - nw - se + sw)/4). Curvature is the signature
        of eddy cores — geostrophic relative vorticity is proportional to
        the Laplacian of SSH — and the mixed derivative completes the
        Hessian the strain-rate tensor needs. First differences alone can
        say "there is a gradient here" but not "the gradient turns", which
        is exactly what separates a front's flank from an eddy center. The
        mirror imputation is benign for these operators: a hole filled with
        2*center - mirror contributes exactly zero to that axis of the
        Laplacian, so swath-edge rows get conservatively damped curvature
        rather than amplified noise, and the missingness block tells the
        net when to trust it less.
    """
    ...
    # neighbors at +/-k, diagonals at +/-k +/- 1. Orientation/sign
    # conventions don't matter to the net — only that the directions are
    # consistent and orthogonal.
    ...
    if k >= 3 and k * k == n_cells:
        ...
        lap = (Zf[:, :, ci + 1] + Zf[:, :, ci - 1]
               + Zf[:, :, ci + k] + Zf[:, :, ci - k]
               - 4.0 * Zf[:, :, ci]).astype(np.float32)
        gxy = (0.25 * (Zf[:, :, ci + k + 1] - Zf[:, :, ci + k - 1]
                       - Zf[:, :, ci - k + 1] + Zf[:, :, ci - k - 1])).astype(np.float32)
        blocks += [gx, gy, lap, gxy]
