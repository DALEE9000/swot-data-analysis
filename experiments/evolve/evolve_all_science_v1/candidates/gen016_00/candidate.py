...
def _featurize(X, mean, scale, n_cells):
    ...
    neighborhood. Ingredients:
      ...
      * per-feature SECOND-order operators (n, 3F), new this generation:
        the 5-point Laplacian (left + right + up + down - 4*center) and the
        two diagonal central differences (NE - SW)/(2*sqrt(2)) and
        (NW - SE)/(2*sqrt(2)). The Laplacian of SSH is proportional to
        geostrophic relative vorticity (zeta = g/f * lap(eta)) — the eddy
        signal itself — and the diagonal gradients complete the local
        deformation/strain tensor that axis-aligned differences cannot
        resolve; fast fronts and eddy rims are exactly where these are
        large. Because the mirror imputation is Taylor-LINEAR, filled cells
        satisfy filled + mirror - 2*center = 0, so at swath edges the
        second differences collapse toward zero — a conservative "no
        measured curvature" default rather than amplified edge noise — and
        the missingness block tells the net when that zero is imputed
