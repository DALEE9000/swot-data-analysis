...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # THE ONE CHANGE: a THREE-member ensemble — conv, mlp, and a second
    # conv under a distinct seed. The parent's GPU staging made epochs
    # cheap, but it still spent the whole budget on two members, so the
    # freed wall-clock only bought epochs past each member's early-stop
    # point. A third member converts that surplus into variance reduction
    # instead: averaging a decorrelated third prediction cancels noise
    # concentrated in the highest-variance bins (edge-stencil, fast
    # regime), the parent's worst. Member order keeps the two proven
    # architectures first, so even if the third slice is squeezed the
    # original pair trains fully; per-member deadlines, distinct seeds
    # (init + data order), and the 15% val guard below make the worst case
    # degenerate to the parent's two-member average rather than averaging
    # in an undertrained net.
    archs = ["conv", "mlp", "conv"]
    M_ens = 3
    members = []
