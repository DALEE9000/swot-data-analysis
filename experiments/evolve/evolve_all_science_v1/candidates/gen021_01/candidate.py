...
class HeteroMLP(nn.Module):
    ...
    Shared trunk -> two heads:
      ...
      logvar : per-row, per-component log aleatoric variance, used to weight
               the training loss AND — new this generation — to precision-
               weight the ensemble combination at prediction time. Hard/noisy
               rows (swath-edge stencils, fast fronts/eddies) get high
               predicted variance and therefore small gradients, so they stop
               distorting the fit on the well-conditioned majority of rows.
    """
...
def _combine(mus, lvs, precision_weighted):
    """Combine member predictions for one batch.

    mus/lvs: lists of (b, 2) tensors, one per member.
    Plain mean, or per-row/per-component precision weighting
    w_m = exp(-logvar_m) (clamped to the training range, so weight ratios
    across members are bounded by exp(9)) — the inverse-variance-optimal
    combination of independent Gaussian estimates. On easy bulk rows the
    calibrated variances are near-equal and this reduces to the plain mean;
    it diverges from it exactly on the edge-stencil and fast-front rows
    where member disagreement is largest.
    """
    mu_s = torch.stack(mus)
    if not precision_weighted or len(mus) == 1:
        return mu_s.mean(0)
    w_s = torch.exp(-torch.stack(lvs).clamp(-7.0, 2.0))
    return (mu_s * w_s).sum(0) / w_s.sum(0).clamp(min=1e-12)
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # Combination-rule selection (the one new mechanism this generation).
    # Score plain averaging vs per-row precision weighting on the temporal
    # validation tail. Both members absorbed the tail during the recency
    # fine-tune, so the absolute MSEs are optimistic — but identically so
    # for both rules, and only their ORDER matters here. If precision
    # weighting does not beat the plain mean on the most test-like data we
    # have, the test-time combination degenerates bit-for-bit to the parent.
    use_prec = False
    if len(keep) > 1:
        bs_pred = 65536
        with torch.no_grad():
            se_plain, se_prec = 0.0, 0.0
            for i in range(0, len(X_v), bs_pred):
                xb = X_v[i:i + bs_pred]
                yb, mb = Y_v[i:i + bs_pred], M_v[i:i + bs_pred]
                mus, lvs = [], []
                for net in keep:
                    mu, lv = net(xb)
                    mus.append(mu)
                    lvs.append(lv)
                p_plain = _combine(mus, lvs, False)
                p_prec = _combine(mus, lvs, True)
                se_plain += ((p_plain - yb) * mb).pow(2).sum().item()
                se_prec += ((p_prec - yb) * mb).pow(2).sum().item()
        use_prec = se_prec < se_plain

    # Predict on the test set in batches with the selected combination rule.
    ...
    with torch.no_grad():
        for i in range(0, len(X_test), 65536):
            ...
            for net in keep:
                ...
                mus.append(mu)
                lvs.append(lv)
            out[i:i + 65536] = _combine(mus, lvs, use_prec).cpu().numpy()
