...
class HeteroMLP(nn.Module):
    ...
    Shared trunk -> two heads:
      ...
      logvar : per-row, per-component log aleatoric variance, used to
               weight the training loss AND, at test time, to weight each
               ensemble member's vote. Hard/noisy rows (swath-edge stencils,
    ...
    Capacity (changed this generation): trunk widened from (256, 256, 128)
    to (512, 512, 256). With ~3.8M training rows and ~150 engineered inputs
    the old net had only ~140k parameters and the fast-tail response surface
    saturates a narrow trunk; per-step wall time here is dominated by CPU
    batch gather + host-to-device transfer, not the tiny matmuls, so ~4x
    FLOPs costs little wall clock. Depth, activation, dropout, and both
    heads are unchanged so the lineage's tuned LR/schedule still applies.
    """

    def __init__(self, n_inputs, hidden=(512, 512, 256), dropout=0.1,
                 logvar_init=-3.5):
        super().__init__()
        layers = []
        d = n_inputs
...
class _EMA:
    ...
    of a ~500k-parameter net plus one lerp per optimizer step.
...
def _train_member(member_seed, deadline, device, max_epochs,
    ...
    net = HeteroMLP(X_t.shape[1], hidden=(512, 512, 256), dropout=0.1).to(device)
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # their learned variance surfaces — and combining their mu predictions
    ...
    # per-member wall time (conservative: it amortizes the one-off
    # featurization cost over the members), so an extra either trains
    # near-fully or is never started, and the val-loss guard below still
    # drops any member the deadline truncated. With no slack this loop never
    # runs and the ensemble is exactly the two-member parent's.
    ...
    # Uncertainty-weighted ensemble aggregation. Per-member logvar
    # calibration on the temporal-val split: each member's mean predicted
    # logvar per component. Test-time vote weights use the DEVIATION from
    # this baseline rather than the raw logvar, so a systematically
    # overconfident member cannot dominate every row — only rows where a
    # member is unusually confident or unusually uncertain relative to its
    # own norm shift the average. On typical rows all deviations are near
    # zero and the combination reduces to the proven uniform mean; on
    # edge-stencil and fast-front rows — where members disagree most and
    # their variance heads are regime-conditioned by the explicit flow
    # features — the vote shifts toward whichever member is relatively more
    # confident there.
    calib = []
    with torch.no_grad():
        for net in keep:
            lvs = []
            for i in range(0, len(X_v), 8192):
                _, lv = net(X_v[i:i + 8192])
                lvs.append(lv)
            calib.append(torch.cat(lvs).mean(dim=0, keepdim=True))

    # Temper (tau=2) and clamp deviations to +/-1 after tempering so the
    # weight ratio between any two members stays within ~e^2 — enough to
    # matter on the noisy tail, bounded enough that a miscalibrated head
    # cannot collapse the ensemble to one member.
    TAU = 2.0
    ...
    with torch.no_grad():
        for i in range(0, len(X_test), 65536):
            ...
            mus, ws = [], []
            for net, c in zip(keep, calib):
                mu, lv = net(xb)
                dev = ((lv.clamp(-7.0, 2.0) - c) / TAU).clamp(-1.0, 1.0)
                mus.append(mu)
                ws.append(torch.exp(-dev))
            W = torch.stack(ws)                      # (M, batch, 2)
            Wn = W / W.sum(dim=0, keepdim=True).clamp(min=1e-8)
            out[i:i + 65536] = (torch.stack(mus) * Wn).sum(dim=0).cpu().numpy()
