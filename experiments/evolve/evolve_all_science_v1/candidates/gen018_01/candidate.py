...
def _ridge_fit(Xs, Ys, mask, rng, n_sub=1_000_000, lam_rel=1e-3):
    ...
    quadratic regime features out of this fit is the point of this
    generation — a linear model over clipped quadratics is neither physics
    nor robust under temporal shift (gen018_00's failure mode).
...
class WideDeep(nn.Module):
    ...
    Two additive paths trained jointly end-to-end:
      ...
      deep : the lineage's trunk (LayerNorm/SiLU/Dropout MLP) over the FULL
             ...
             flow), now with explicit regime coordinates for exactly the
             fast rows where the deficit lives.
      logvar : per-row, per-component log aleatoric variance from the trunk
             — also regime-conditioned now — used only to weight the loss.
    """
...
def _train_member(member_seed, deadline, device, max_epochs,
    ...
    (linear columns only) before training. Model selection (raw vs
    weight-EMA, unweighted temporal-val masked MSE of mu) and the recency
    fine-tune are the lineage's proven procedure, unchanged.
