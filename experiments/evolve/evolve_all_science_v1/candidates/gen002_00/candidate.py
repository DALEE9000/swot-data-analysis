...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # most test-like — 10% of the training window, and the model above never
    # trained on it. After model selection, absorb it with a short low-LR pass
    # over the FULL window (tail included). Low LR + few epochs keeps the
    # selected solution basin intact while adapting to the freshest ocean
    # state; there is no validation signal during this phase by construction,
    # so it is deliberately conservative and budget-checked in-loop.
    if time.time() - t0 < time_budget_s:
