...
def _featurize(X, mean, scale, n_cells):
    ...
    Layout matters downstream: the attention net slices the FIRST F*C
    columns as the standardized/imputed stencil values (its cell tokens)
    and the NEXT C columns as the per-cell missing fraction, which serves
    double duty as a token feature and as the attention key-padding mask;
    everything after the image block (mask included) feeds the dense path.
    The fallback MLP consumes the whole vector flat.
    """
...
class StencilAttnNet(nn.Module):
    """WILDCARD CORE: masked transformer over stencil-cell tokens.

    Each of the C stencil cells becomes one token: its F standardized
    (imputed) feature values concatenated with its missing fraction, linearly
    embedded to d_model, plus a learned positional embedding per cell. A
    learned CLS token is prepended and two tiny pre-norm TransformerEncoder
    layers run over the (C+1)-token sequence.

    The structural novelty is the key-padding mask: cells whose features are
    entirely missing (stencil padding at swath edges) are EXCLUDED from
    attention, so at edge rows the CLS summary aggregates over only the cells
    that truly exist — every prior core (conv, MLP, MoE) instead consumed
    mirror-imputed values at those positions. This targets the edge-stencil
    deficit, the parent's largest relative gap. The center cell is never
    masked (it is always real for a scored row), so the sequence can never be
    fully padded.

    The CLS output is fused with the lineage's proven dense path over the
    engineered scalars (mask block onward), then the standard mu/logvar
    heads. Works for any C > 1 — attention does not need a square stencil.
    """

    def __init__(self, d_total, n_feat, n_cells, d_model=64, n_layers=2,
                 dropout=0.1, logvar_init=-3.5):
        super().__init__()
        self.F = n_feat
        self.C = n_cells
        self.img_dim = n_feat * n_cells
        self.tok = nn.Linear(n_feat + 1, d_model)
        self.pos = nn.Parameter(torch.empty(1, n_cells, d_model))
        ...
        self.trunk = nn.Sequential(
            nn.Linear(d_model + 128, 128), nn.LayerNorm(128), nn.SiLU(),
            nn.Dropout(dropout),
        )
    ...
    def forward(self, x):
        b = x.shape[0]
        # (b, F, C) -> (b, C, F): one token per cell.
        img = x[:, :self.img_dim].view(b, self.F, self.C).transpose(1, 2)
        msk = x[:, self.img_dim:self.img_dim + self.C]  # (b, C) miss frac
        tokens = self.tok(torch.cat([img, msk.unsqueeze(-1)], dim=-1)) + self.pos
        seq = torch.cat([self.cls.expand(b, -1, -1), tokens], dim=1)
        pad = msk >= 0.999  # fully-missing cells drop out of attention
        pad = pad.clone()
        pad[:, self.C // 2] = False  # center cell always attends
        pad = torch.cat(
            [torch.zeros(b, 1, dtype=torch.bool, device=x.device), pad], dim=1)
        zc = self.enc(seq, src_key_padding_mask=pad)[:, 0]
...
class HeteroMLP(nn.Module):
    """Lineage MLP core, kept ONLY as the degenerate-layout fallback
    (n_cells == 1, where there is no stencil to attend over)."""
...
def _make_net(d_total, n_feat, n_cells, device, arch):
    """arch is "attn" (the wildcard core) or "mlp". A degenerate 1-cell
    layout has nothing to attend over, so it falls back to the MLP —
    never a crash. Attention works for any C > 1, square or not."""
    if arch == "attn" and n_cells > 1:
...
def _masked_mse(pred, target, mask, comp_w):
    ...
    raw-vs-EMA selection, and the member guard all optimize the metric
    directly instead of pooled MSE."""
...
def _train_member(member_seed, arch, deadline, device, max_epochs,
    ...
    """Train one ensemble member with the recency- and component-weighted
    anchored heteroscedastic NLL, the budget-aware cosine LR anneal, EMA,
    raw-vs-EMA selection on component-weighted temporal-val masked MSE,
    patience-15 backstop, and the recency fine-tune — all unchanged from
    the reference. A grad-norm clip (5.0) is added since attention layers
    are spikier early in training than the conv/MLP cores were."""
    torch.manual_seed(member_seed)
    rng = np.random.default_rng(member_seed)

    net = _make_net(X_t.shape[1], n_feat, n_cells, device, arch)
    lr0, lr_floor = 1e-3, 3e-5
    ...
    t_start = time.time()
    total_T = None  # cosine horizon, estimated after epoch 1
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        # Budget-aware cosine anneal: size the horizon from the measured
        # first-epoch cost (train + val, since both recur), then set the LR
        # for the NEXT epoch. Floor of 3 epochs keeps a degenerate estimate
        # from annealing instantly; cap at max_epochs so the schedule ends
        # exactly when the epoch loop can.
        if total_T is None:
            ep_time = max(time.time() - t_start, 1e-6)
            left = max(0.0, deadline - time.time())
            total_T = min(max_epochs, epoch + max(3, int(left / ep_time)))
        frac = min(1.0, epoch / max(total_T, 1))
        new_lr = lr_floor + 0.5 * (lr0 - lr_floor) * (1.0 + math.cos(math.pi * frac))
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # Per-component inverse-variance weights (unchanged). Score =
    # mean_c(1 - MSE_c/Var_c): weighting each component's squared error by
    # 1/Var_c (of the ORIGINAL targets — the additive ridge baseline
    # preserves MSE identity, and R^2 is computed on the final predictions)
    # makes both the training loss and every validation-based selection
    # decision optimize mean R^2 rather than pooled MSE. Mean-1 normalized
    # so the tuned loss scale is unchanged; clipped to [0.5, 2] relative so
    # a pathological variance ratio cannot destabilize training.
    ...
    # early stopping optimizes forward-in-time generalization — what the
    # temporal test holdout measures.
    ...
    # Two-member SEED-diverse ensemble of the wildcard attention core, with
    # per-member wall-clock slices (slice structure unchanged from the
    # reference). Both members are StencilAttnNet so the run measures the
    # new core cleanly — diluting it with the old MLP would confound the
    # wildcard's information value. Distinct seeds per member so data order
    # and init both differ; each member's cosine schedule is sized to its
    # own slice, so both finish annealed.
    archs = ["attn", "attn"]
    M_ens = 2
    members = []
    ...
    # degenerates to a single attention net instead of averaging in an
    # undertrained member.
