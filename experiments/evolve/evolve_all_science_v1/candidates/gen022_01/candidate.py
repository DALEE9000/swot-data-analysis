...
def _featurize(X, mean, scale, n_cells):
    ...
    Nonlinear flow-regime block: the fast-regime deficit lives in fronts and
    eddies, whose signature is NONLINEAR in the derivatives — magnitudes and
    products that a SiLU MLP spends capacity approximating and does so poorly
    far from the data bulk (exactly the fast tail). So we hand them over
    explicitly, computed from the standardized, imputed stencil:
      * |grad f| = sqrt(gx^2 + gy^2) per feature (n, F) — front intensity;
      * when F == 8 (the pooled layout [mdt, ssha, ugos, vgos, ugosa,
        vgosa, era5_u, era5_v]): center-cell flow speeds |u_g|, |u_ga|,
        |u_wind| (3), relative vorticity zeta = dv/dx - du/dy, normal strain
        sn = du/dx - dv/dy, shear strain ss = dv/dx + du/dy, and the
        Okubo-Weiss parameter W = sn^2 + ss^2 - zeta^2 (4) — the classic
        discriminant between eddy cores (W<0, rotation-dominated) and
        strain-dominated fronts (W>0); axis-orientation conventions cancel
        in W. Plus geostrophic advection u_g * gx_f + v_g * gy_f of every
        feature (n, F) — the leading nonlinear momentum/tracer term, i.e.
        the ageostrophic tendency the linear features cannot express.
    Quadratic terms are clipped to +/-16 (inputs already sigma-scaled and
    fills clamped to +/-4) so rare outliers do not blow up the first Linear
    layer; magnitudes are naturally bounded the same way. These features
    also give the heteroscedastic logvar head direct regime conditioning,
    sharpening the loss weighting on exactly the fast rows.
    """
...
def _augment_edges(X_raw, n_cells, rng):
    """Swath-truncation augmentation (new this generation).

    The edge-stencil bins are the model's weakest by far (r2_u 0.29 vs 0.47
    full-stencil), and real edge rows are both scarcer and noisier than
    interior rows, so the net learns the imputed-input regime poorly. This
    synthesizes supervised edge examples from clean ones: given raw rows
    with FULL stencils, NaN out one randomly chosen stencil side — top or
    bottom row, or left or right column, across ALL features, exactly the
    geometry of a swath-edge truncation — and let the standard featurization
    path apply its mirror imputation. The target for the augmented row is
    the source row's true (u, v), so the model gets dense, clean supervision
    for the imputation regime it must handle at the swath edges.

    Returns the truncated RAW copies (caller featurizes them with the same
    mean/scale so train/test transforms stay identical).
    """
    n, d = X_raw.shape
    F = d // n_cells
    k = int(round(math.sqrt(n_cells)))
    ...
    cell_sets = [
        np.arange(k),                          # top row of the k x k window
        np.arange(n_cells - k, n_cells),       # bottom row
        np.arange(0, n_cells, k),              # left column
        np.arange(k - 1, n_cells, k),          # right column
    ]
    for s, cells in enumerate(cell_sets):
        rows = np.nonzero(side == s)[0]
        if len(rows) == 0:
            continue
        # feature-major layout: column of (feature f, cell c) is f*n_cells + c
        cols = (np.arange(F)[:, None] * n_cells + cells[None, :]).ravel()
        Xa[np.ix_(rows, cols)] = np.nan
    return Xa
...
def _masked_nll_anchored(mu, logvar, target, mask, row_w):
    ...
    scale the optimizer and LR schedule were tuned on. Augmented edge rows
    carry half their source row's weight so the real data distribution
    stays dominant.
    """
...
class _EMA:
    ...
    lineage's prediction-space multi-seed ensemble and costs one extra copy
...
def _train_member(member_seed, deadline, device, max_epochs,
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        # being optimized. Validation contains only REAL rows — augmented
        # edge rows train but never validate, so selection still measures
        # the true forward-in-time target.
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # ---- swath-truncation augmentation (new this generation) ----
    # Synthesize edge-stencil training rows from clean full-stencil ones:
    # take 25% of the full-stencil rows in the TRAIN window (never the val
    # tail), NaN out one random stencil side in the RAW inputs, and push the
    # copies through the identical featurization (same mean/scale), so the
    # mirror-imputation regime gets dense supervision with clean targets.
    # Augmented rows inherit the source row's target/mask/recency weight at
    # HALF weight (real distribution stays dominant) and never enter
    # validation. k >= 3 guard: with no spatial window there is no edge
    # regime to teach and the module degenerates exactly to the parent.
    k = int(round(math.sqrt(n_cells)))
    aug_parts = None
    if k >= 3 and k * k == n_cells:
        n_tr = n - n_val
        full_rows = np.nonzero(~np.isnan(X_train[:n_tr]).any(axis=1))[0]
        rng_aug = np.random.default_rng(seed + 777)
        n_aug = int(0.25 * len(full_rows))
        if n_aug > 0:
            pick = np.sort(rng_aug.choice(full_rows, size=n_aug, replace=False))
            Xa_raw = _augment_edges(X_train[pick], n_cells, rng_aug)
            Xa = _featurize(Xa_raw, mean, scale, n_cells)
            del Xa_raw
            aug_parts = (Xa, Ys[pick], mask[pick],
                         (0.5 * w_all[pick]).astype(np.float32))

    Xt_np = Xs[train_idx]
    Yt_np, Mt_np, Wt_np = Ys[train_idx], mask[train_idx], w_all[train_idx]
    if aug_parts is not None:
        Xa, Ya, Ma, Wa = aug_parts
        Xt_np = np.concatenate([Xt_np, Xa])
        Yt_np = np.concatenate([Yt_np, Ya])
        Mt_np = np.concatenate([Mt_np, Ma])
        Wt_np = np.concatenate([Wt_np, Wa])
        del Xa, aug_parts

    X_t = torch.from_numpy(Xt_np)
    Y_t = torch.from_numpy(Yt_np)
    M_t = torch.from_numpy(Mt_np)
    W_t = torch.from_numpy(Wt_np)
    ...
    # Adaptive-size deep ensemble. The first two members keep the lineage's
    # proven per-member wall-clock slices (member m must finish by
    # 0.95*(m+1)/2 of the budget, leaving 5% for test prediction).
    # Independently seeded nets make decorrelated errors — including in
    # their learned variance surfaces — and averaging their mu predictions
    # cancels seed-dependent variance; the gain concentrates on edge-stencil
    # and fast-front rows where single-net outputs are noisiest.
    M_MIN, M_MAX = 2, 4
    hard_end = t0 + time_budget_s * 0.95
    members = []
    for m in range(M_MIN):
        deadline = t0 + time_budget_s * 0.95 * (m + 1) / M_MIN
        if time.time() > deadline:
            break
    ...
    # Opportunistic extra members: early stopping usually converges each
    # scheduled member well before its slice deadline, so a sizable fraction
    # of the soft budget goes unused. Spend it on more independent seeds —
    # the most reliable variance-reduction knob available. An extra member
    # starts only if the time remaining exceeds 0.7x the observed mean
    # per-member wall time so far; that estimate is conservative (it
    # amortizes the one-off featurization cost over the members), so an
    # extra either trains near-fully or is never started, and the val-loss
    # guard below still drops any member the deadline truncated. With no
    # slack this loop never runs; the ~25% longer augmented epochs are
    # absorbed here as fewer extra members, not as truncated ones.
    while M_MIN <= len(members) < M_MAX:
        per_member = (time.time() - t0) / len(members)
        if time.time() + 0.7 * per_member > hard_end:
            break
        net, best_val = _train_member(
            seed + 101 * len(members), hard_end, device, max_epochs,
            X_t, Y_t, M_t, W_t, X_v, Y_v, M_v,
            X_v_cpu, Y_v_cpu, M_v_cpu, W_v_cpu)
        members.append((net, best_val))
