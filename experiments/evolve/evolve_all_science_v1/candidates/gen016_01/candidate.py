...
def _featurize(X, mean, scale, n_cells):
    ...
    neighborhood. Ingredients:
      ...
      * per-feature central differences (n, 2F): gx = (right - left)/2 and
        ...
        dynamics behind the fast-regime deficit. Handing them to the MLP as
        first-class inputs means the first Linear layer can form those
...
def _masked_nll_anchored(mu, logvar, target, mask, row_w, comp_w):
    ...
    comp_w is the component weight (shape (2,), mean 1): 1/Var(target_c)
    normalized. The scored metric is the MEAN of per-component R^2 =
    1 - mean(mse_c / var_c), i.e. inverse-variance-weighted MSE — but a raw
    squared-error sum weights v (~2x the variance of u) with ~2/3 of the
    anchor gradient, starving u, the consistently weaker component. Folding
    comp_w into both the weight and the normalizer aligns the anchor's
    gradient allocation with the metric without changing the loss scale the
    optimizer and LR schedule were tuned on.
    """
    logvar = logvar.clamp(-7.0, 2.0)
    err2 = (mu - target).pow(2)
    per = 0.5 * (logvar + err2 * torch.exp(-logvar)) + err2
    w = mask * row_w.unsqueeze(1) * comp_w
    return (per * w).sum() / w.sum().clamp(min=1e-8)


def _masked_mse(pred, target, mask, comp_w):
    # Inverse-variance-weighted masked MSE: monotone in the scored quantity
    # (mean per-component R^2), unlike raw MSE which lets the higher-variance
    # v component dominate model selection.
    w = mask * comp_w
    return ((pred - target).pow(2) * w).sum() / w.sum().clamp(min=1e-8)
...
def _val_mse(net, X_v, Y_v, M_v, bs, comp_w):
    ...
    with torch.no_grad():
        ...
        for i in range(0, len(X_v), bs):
            mu, _ = net(X_v[i:i + bs])
            losses.append(_masked_mse(mu, Y_v[i:i + bs], M_v[i:i + bs], comp_w).item())
...
def _train_member(member_seed, deadline, device, max_epochs,
    ...
    """Train one ensemble member with the recency-weighted, component-
    balanced anchored heteroscedastic NLL. Each epoch, BOTH the raw net and
    its weight-EMA shadow are scored on the UNWEIGHTED-by-row, inverse-
    variance-weighted-by-component temporal-val masked MSE of mu — the
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        # Model selection on the component-balanced masked MSE of mu — the
        # scored quantity — NOT on the training loss, whose value mixes in
        # the variance term and the recency weighting. Both the raw and EMA
    ...
    # same recency-weighted, component-balanced anchored loss so relative row
    # and component weighting stays consistent (the tail carries the largest
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # Per-component inverse-variance weights (mean 1). The scored metric is
    # the mean of per-component R^2 = 1 - mean(mse_c / var_c): an inverse-
    # variance-weighted MSE. Raw squared error hands the higher-variance v
    # component (~2x u's variance) roughly 2/3 of the anchor gradient and
    # control of early stopping, starving u — the consistently weaker
    # component and the cheaper source of score. These weights realign the
    # anchor, validation selection, EMA-vs-raw choice, and ensemble guard
    # with the metric; mean-1 normalization keeps the loss scale (and hence
    # the tuned LR/scheduler behavior) unchanged.
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        tvar = np.nanvar(Y_train.astype(np.float32), axis=0)
    inv = np.where(np.isfinite(tvar) & (tvar > 1e-8), 1.0 / tvar, 1.0)
    comp_w = torch.from_numpy((inv / inv.mean()).astype(np.float32)).to(device)
