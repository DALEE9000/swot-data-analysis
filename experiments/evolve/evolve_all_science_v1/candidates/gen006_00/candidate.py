...
def _featurize(X, mean, scale, n_cells):
    ...
    neighborhood. Two appended blocks:
      ...
      * per-feature stencil standard deviation (n, F), computed on the
        ...
        intensity signal for the fast-flow regime, and exactly the
        conditioning information the heteroscedastic variance head needs to
        downweight the rows it is meant to downweight. Rows with a single
        valid cell get std 0 ("no local gradient information").
    """
