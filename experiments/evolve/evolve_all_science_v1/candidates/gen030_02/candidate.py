...
class HeteroMLP(nn.Module):
    ...
    leans on — and why the NEW third member is a second MLP seed."""
...
def _make_net(d_total, n_feat, n_cells, device, arch):
    ...
    reshape would be invalid), so the pooled k=3 dataset gets one conv and
    two MLPs and degenerate layouts get all MLPs — never a crash."""
    k = int(round(math.sqrt(n_cells)))
...
def _train_member(member_seed, arch, deadline, device, max_epochs,
    ...
    Grafted from the inspiration (and required by the 3-way slice split):
    a budget-aware cosine LR anneal. ReduceLROnPlateau needs 5 stagnant
    epochs per halving, so a short wall-clock slice expires long before the
    LR decays — leaving the weights (and the EMA built from them) at a
    high-LR noise floor. Here, after the first epoch we measure the
    per-epoch cost, estimate how many epochs fit in the member's remaining
    slice, and anneal the LR from 1e-3 to a 3e-5 floor on a cosine over
    exactly that horizon (capped by max_epochs; patience-15 backstop kept).
    One refinement over the inspiration: the horizon RESERVES ~2.5 epochs
    of wall clock for the recency fine-tune, so the proven +~0.01 fine-tune
    is never starved by a schedule that consumes the whole slice (a likely
    reason the inspiration only tied its parent). Selection (raw-vs-EMA on
    component-weighted temporal-val masked MSE) is unchanged."""
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        # first-epoch cost (train + val, since both recur), minus a ~2.5-
        # epoch reservation so the recency fine-tune always gets its slice.
        # Floor of 3 epochs keeps a degenerate estimate from annealing
        # instantly; cap at max_epochs so the schedule ends exactly when
        # the epoch loop can.
        if total_T is None:
            ep_time = max(time.time() - t_start, 1e-6)
            left = max(0.0, deadline - time.time() - 2.5 * ep_time)
        ...
        if epoch >= total_T:
            break
        if time.time() > deadline - 2.5 * (time.time() - t_start) / max(epoch, 1):
            break
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # THE ONE CHANGE vs the parent: a THREE-member architecture-diverse
    # deep ensemble with per-member wall-clock slices. Member 0 is the
    # conv-wide-and-deep core, members 1 and 2 are HeteroMLPs with distinct
    # seeds (data order and init both differ). Rationale: prediction
    # averaging is the lineage's proven biggest lever, and a 3-way average
    # cancels more variance exactly where errors are noisiest (late-test,
    # edge-stencil, fast fronts); the MLP is the documented stronger-r2_u
    # architecture, so doubling it tilts the average toward the persistent
    # u deficit. Shorter slices are made viable by the grafted budget-aware
    # cosine anneal — every member finishes fully annealed no matter how
    # few epochs fit — and the guard below ejects any member the tighter
    # slice still undertrains, so the worst case degenerates to the
    # parent's 2-member (or single-member) ensemble instead of averaging
    # in junk.
    archs = ["conv", "mlp", "mlp"]
    M_ens = 3
    members = []
