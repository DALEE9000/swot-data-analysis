...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # Adaptive-size deep ensemble (grafted from the inspiration). The first
    # two members keep the lineage's proven per-member wall-clock slices
    # (member m must finish by 0.95*(m+1)/2 of the budget, leaving 5% for
    # test prediction). Independently seeded nets make decorrelated errors —
    # including in their learned variance surfaces — and combining their mu
    # predictions cancels seed-dependent variance; the gain concentrates on
    # edge-stencil and fast-front rows where single-net outputs are noisiest.
    M_MIN, M_MAX = 2, 4
    ...
    # ---- NEW: uncertainty-weighted ensemble aggregation ----
    # Per-member logvar calibration on the temporal-val split: each member's
    # mean predicted logvar per component. Test-time vote weights use the
    # DEVIATION from this baseline rather than the raw logvar, so a
    # systematically overconfident member cannot dominate every row — only
    # rows where a member is unusually confident or unusually uncertain
    # relative to its own norm shift the average. On typical rows all
    # deviations are near zero and the combination reduces to the proven
    # uniform mean; on edge-stencil and fast-front rows — where members
    # disagree most and their variance heads are regime-conditioned by the
    # explicit flow features — the vote shifts toward whichever member is
    # relatively more confident there.
    calib = []
    with torch.no_grad():
        for net in keep:
            lvs = []
