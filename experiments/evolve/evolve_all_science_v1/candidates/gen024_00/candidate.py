...
def _featurize(X, mean, scale, n_cells):
    ...
    order), and the nonlinear flow-regime features.
    ...
    Proven plumbing (unchanged from the lineage):
      ...
      * when F == 8 (pooled layout [mdt, ssha, ugos, vgos, ugosa, vgosa,
        era5_u, era5_v]): center flow speeds, vorticity/strain/Okubo-Weiss
        (n, 7) and geostrophic advection of every feature (n, F), quadratics
        clipped to +/-16.

    Output layout (the CNN net relies on this ordering):
      [raw standardized+filled stencil (F*C) | per-cell miss (C) | scalars].
    The raw block and the miss block are reshaped into (F+1)-channel k x k
    images inside the network; everything after them feeds the MLP branch.
    """
...
class StencilCNNHetero(nn.Module):
    """Two-branch joint (u, v) predictor with a heteroscedastic head.

    CNN branch (the wildcard core): the raw standardized+imputed stencil is
    reshaped feature-major into an F-channel k x k image and concatenated
    with the per-cell missingness mask as one extra channel. Two 3x3
    convolutions (padding=1, so any k >= 1 works) learn translation-
    equivariant local operators — gradients, Laplacians, and cross-feature
    combinations the lineage has been hand-coding one at a time — and,
    crucially, condition on the mask channel, so at swath edges the net can
    learn one-sided difference operators instead of trusting mirror-imputed
    cells. The full 64 x k x k activation map is flattened (no pooling:
    position within the stencil is physical, not nuisance) and projected to
    a 128-dim embedding.

    Scalar branch: the engineered scalars (missingness handled above; spatial
    std, explicit derivatives, flow-regime block) through one Linear+LN+SiLU
    to 128 dims — these remain useful as high-precision priors the small CNN
    would need many filters to replicate exactly.

    Trunk: concat(128+128) -> 256 -> 128 -> mu (scored) and logvar (loss
    weighting only), initialized as in the lineage so early training behaves
    like scaled MSE. With the ridge residual baseline, mu now predicts the
    RESIDUAL of the clipped linear map, not the raw velocity — the additive
    baseline means the net's capacity concentrates on the nonlinear
    corrections while the linear geostrophic backbone extrapolates into the
    fast tail on its own.
    """

    def __init__(self, F, k, d_scalar, dropout=0.1, logvar_init=-3.5):
        super().__init__()
        self.F, self.k = F, k
        self.raw = F * k * k
        self.cells = k * k
        self.conv = nn.Sequential(
            nn.Conv2d(F + 1, 64, 3, padding=1), nn.SiLU(),
            nn.Conv2d(64, 64, 3, padding=1), nn.SiLU(),
        )
        conv_dim = 64 * k * k
        self.conv_proj = nn.Sequential(
            nn.LayerNorm(conv_dim), nn.Linear(conv_dim, 128), nn.SiLU())
        self.scalar = nn.Sequential(
            nn.Linear(d_scalar, 128), nn.LayerNorm(128), nn.SiLU(),
            nn.Dropout(dropout))
        ...
        # early training behaves like plain (scaled) MSE. Residuals of the
        # ridge baseline have slightly smaller variance than the raw targets,
        # but the same init is within the head's one-step adjustment range.
        nn.init.zeros_(self.logvar.weight)
        nn.init.constant_(self.logvar.bias, logvar_init)

    def forward(self, x):
        B = x.shape[0]
        raw = x[:, :self.raw].view(B, self.F, self.k, self.k)
        miss = x[:, self.raw:self.raw + self.cells].view(B, 1, self.k, self.k)
        img = torch.cat([raw, miss], dim=1)
        c = self.conv_proj(self.conv(img).flatten(1))
        s = self.scalar(x[:, self.raw + self.cells:])
...
def _train_member(member_seed, deadline, device, max_epochs, make_net,
    ...
    scored quantity (targets are ridge residuals, and the baseline is
    additive per row, so residual val MSE == final-prediction val MSE) —
    and model selection keeps the better of the two; the LR schedule still
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    d_scalar = Xs.shape[1] - F * n_cells - n_cells
    mask = np.isfinite(Y_train).astype(np.float32)
    Ys = np.nan_to_num(Y_train.astype(np.float32), nan=0.0)
    n = len(Xs)

    def make_net():
        return StencilCNNHetero(F, k, d_scalar, dropout=0.1)

    # ---- Linear baseline + residual learning (grafted from gen023_02's
    # lineage, where it is a proven component) ----
    # Ridge from the full engineered block to (u, v) on a seeded subsample;
    # the CNN fits the residual. The linear map extrapolates the dominant
    # geostrophic signal into the fast tail (this parent's worst bin:
    # rmse 0.2303 fast vs 0.0704 slow) where bounded activations saturate;
    # baseline clipped to +/-3 m/s so a wild extrapolation can never blow up
    # a prediction. Additive per-row baseline means residual val MSE ==
    # final-prediction val MSE, so all selection still optimizes the scored
    # quantity.
    ...
    for m in range(M_ens):
        ...
        net, best_val = _train_member(
            seed + 101 * m, deadline, device, max_epochs, make_net,
            X_t, Y_t, M_t, W_t, X_v, Y_v, M_v,
            X_v_cpu, Y_v_cpu, M_v_cpu, W_v_cpu)
        members.append((net, best_val))

    # Guard: drop members starved by a tight budget (val loss >15% worse than
    # the best member) so the worst case degenerates to a single CNN model
    ...
    # Predict on the test set in batches: ridge baseline + averaged residual
    # mu over accepted members.
