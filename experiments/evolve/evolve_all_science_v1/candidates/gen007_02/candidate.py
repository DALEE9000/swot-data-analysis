...
class HeteroMLP(nn.Module):
    ...
    Shared trunk -> two heads:
      ...
      logvar : per-row, per-component log aleatoric variance, used only to
               ...
               reduced — but with beta-NLL no longer vanishing — gradients.
    """
...
def _masked_beta_nll(mu, logvar, target, mask, beta=0.5):
    """beta-NLL (Seitzer et al. 2022): per-component Gaussian NLL scaled by
    stop-grad(sigma^(2*beta)).

    Plain NLL's mu-gradient scales as 1/sigma^2, so rows the variance head
    marks as hard — fast fronts/eddies and padded edge stencils, exactly the
    weakest diagnostic bins — get their training signal crushed by up to
    ~exp(5.5)x across the clamp range. The sigma^(2*beta) factor (beta=0.5)
    makes the effective mu-gradient scale as 1/sigma: still downweighted
    relative to easy rows, no longer starved. beta=0 recovers NLL, beta=1
    recovers MSE. Weighted-mean normalization keeps the loss scale (and thus
    the LR schedule's operating point) comparable to the parent's plain NLL,
    instead of shrinking it by the mean weight.
    """
    logvar = logvar.clamp(-7.0, 2.0)
    per = 0.5 * (logvar + (mu - target).pow(2) * torch.exp(-logvar))
    w = torch.exp(beta * logvar).detach() * mask
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for i in range(0, n_train, bs):
            ...
            optimizer.zero_grad()
            mu, lv = net(xb)
            loss = _masked_beta_nll(mu, lv, yb, mb)
            loss.backward()
            optimizer.step()
    ...
    # same beta-NLL so noisy rows stay downweighted without being starved.
    if time.time() - t0 < time_budget_s:
    ...
    # Predict on the test set in batches; only mu is returned.
