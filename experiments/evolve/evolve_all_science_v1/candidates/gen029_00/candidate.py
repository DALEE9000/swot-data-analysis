...
class StencilCNNHetero(nn.Module):
    """Per-row stencil CNN with heteroscedastic heads (unchanged from the
    parent).
    ...
    mu / logvar heads (logvar only weights the training loss).

    Fallback: if the stencil is degenerate (k < 3), conv is skipped and the
    trunk consumes the full flat vector — the module then degenerates to a
    plain MLP.
    """
...
def _train_member(member_seed, deadline, device, max_epochs, F, k,
    ...
    Returns (net, best_val). NOTE: with the ridge baseline subtracted
    upstream, targets here are residuals; because the baseline is additive
    per row, residual val MSE == final-prediction val MSE, so every
    selection decision below still optimizes the scored quantity."""
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # ---- THE ONE CHANGE: linear baseline + residual learning (grafted
    # from the top lineage, where it is proven). Ridge from the full
    # engineered block to (u, v) on a seeded subsample; the nets fit the
    # residual. Motivation: the parent's worst regime bin is fast flow
    # (rmse 0.2352 vs 0.0743 slow) — the bounded SiLU trunk saturates in
    # the fast tail while the dominant geostrophic signal there is largely
    # linear in the features; an unbounded linear map extrapolates it.
    # Baseline clipped to +/-3 m/s so a wild extrapolation cannot blow up
