...
def _ema_update(ema, net, decay):
    # Polyak averaging of the weight trajectory. With decay 0.999 the
    # averaging horizon is ~1000 optimizer steps (~1 epoch here), long enough
    # to smooth SGD noise but short enough to track the LR schedule.
    with torch.no_grad():
...
def _train_member(member_seed, deadline, device, max_epochs,
    ...
    """Train one ensemble member with the heteroscedastic NLL. Weight EMA runs
    alongside training; each epoch BOTH the raw and the EMA weights are scored
    on the temporal-val masked MSE of mu and the better one becomes the
    selection candidate, so EMA can only match or improve the parent's model
    selection. Early stop on that val score, then the proven recency
    fine-tune (its final weights replaced by the fine-tune-trajectory EMA,
    since that stage has no validation to pick an iterate). Returns
    (net, best_val)."""
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        # Score raw AND EMA weights; keep the better as this epoch's candidate.
        val_raw = _val_mse(net, X_v, Y_v, M_v, bs)
        raw_state = copy.deepcopy(net.state_dict())
        net.load_state_dict(ema)
        val_ema = _val_mse(net, X_v, Y_v, M_v, bs)
        if val_ema <= val_raw:
            val_loss = val_ema
            cand_state = {k: v.detach().clone() for k, v in ema.items()}
        else:
            val_loss = val_raw
            cand_state = raw_state
        net.load_state_dict(raw_state)  # training always resumes from raw
    ...
    # the same heteroscedastic NLL so noisy rows stay downweighted. This stage
    # has no validation set, so instead of keeping its arbitrary final
    # iterate we keep the EMA of its trajectory (anchored at the selected
    # weights), which smooths late-batch noise.
    if time.time() < deadline and np.isfinite(best_val):
        ...
        for _ in range(2):
            ...
            for bi, i in enumerate(range(0, n_all, bs)):
                ...
                loss.backward()
                ft_opt.step()
                _ema_update(ft_ema, net, ema_decay)
                if bi % 100 == 0 and time.time() > deadline:
                    out_of_time = True
                    break
            if out_of_time:
                break
        net.load_state_dict(ft_ema)
