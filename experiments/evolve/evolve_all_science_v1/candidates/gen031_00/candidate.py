...
def _train_member(member_seed, arch, deadline, device, max_epochs,
    ...
    THE ONE CHANGE vs the parent lives in the caller: the train/val
    matrices may now arrive already resident on the GPU in float16 (see
    train_and_predict), so the inner loop indexes on-device and casts each
    batch to float32 there — no host fancy-indexing, no PCIe transfer per
    batch. All `.to(device)` / `.float()` calls below are no-ops when the
    tensors are already device-resident fp32 (the CPU fallback path), so
    the training math is identical in both modes; only wall-clock per
    epoch changes, which lets the plateau schedule and patience-15 early
    stop actually operate inside a member's slice instead of being clipped
    by the wall clock. The fine-tune gathers train/val batches from the
    two tensors directly rather than materializing a full concatenated
    ...
    # the same set of rows per batch as the parent's concatenated pass,
    # merely permuted within the batch — SGD-identical, no 1.2 GB copy.
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # THE ONE CHANGE: stage the full featurized train matrix on the GPU in
    # float16 (~3.4M x ~160 cols -> ~1.1 GB, well inside 4 GB alongside the
    # ~0.12 GB fp16 val block, the small nets, and optimizer state). The
    # parent's epoch time was dominated by host-side fancy-indexing of the
    # 2+ GB fp32 matrix and per-batch PCIe transfer, so members spent their
    # whole wall-clock slice on few, high-LR epochs. On-device indexing +
    # per-batch fp32 cast removes that bottleneck entirely; fp16 rounding
    # (~1e-3 relative on standardized, clipped features) is far below data
    # noise. On any CUDA OOM we fall back to the parent's exact CPU
    # pipeline, so the worst case is the parent, not a crash.
    if device.type == "cuda":
        try:
            ...
            M_t = M_t.cpu()
            W_t = W_t.cpu()
            X_v = X_v.cpu().to(device)  # parent behavior: fp32 val on device
        Y_v = Y_v.to(device)
        M_v = M_v.to(device)
        W_v = W_v.to(device)
