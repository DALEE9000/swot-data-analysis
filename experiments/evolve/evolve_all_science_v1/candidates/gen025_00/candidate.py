...
def _featurize(X, mean, scale, n_cells):
    ...
    the image block (mask included) feeds the dense path.
    """
...
class ConvWideDeep(nn.Module):
    """Predictive core: conv path over the raw stencil image + dense path
    over the engineered scalars, joined into one trunk (unchanged from the
    parent).

    Conv path: the first F*C input columns are the standardized, mirror-
    imputed stencil, feature-major — reshaped to an (F, k, k) image. The
    per-cell missing fraction (the C columns right after the image block)
    is appended as an explicit (1, k, k) MASK channel, so filters see
    jointly WHERE the data is real and WHAT it says: the net can learn
    edge-aware local operators (one-sided differences, mask-gated
    smoothing) that fixed central differences — which silently read
    imputed mirror values at swath edges — cannot express. Two padded 3x3
    convolutions keep the k x k support; with k=3 the receptive field of
    the second layer already covers the whole window (~20k conv params).

    Dense path: everything from the mask block onward (missingness, spatial
    std, hand derivatives, regime features) through a small MLP — the
    proven engineered features are kept as a shortcut so the conv path only
    has to learn what they miss.

    Heads: same mu/logvar pair as the lineage; mu is the residual from the
    ridge baseline, logvar weights the anchored NLL.
    """
...
class HeteroMLP(nn.Module):
    """Fallback core (lineage MLP) used only if the stencil is not a square
    k x k window; the pooled dataset always is, so this is a safety net."""
...
def _masked_nll_anchored(mu, logvar, target, mask, row_w):
    ...
    scaled by a per-row weight. The lineage weight was pure recency; this
    candidate folds in a missingness upweight (see train_and_predict) so
    edge rows — whose learned logvar would otherwise shrink their NLL
    gradient toward zero — keep pulling on the optimizer. The anchor term
    scales with the same weight, so the gradient floor rises on edge rows
    exactly where the heteroscedastic head suppresses it."""
...
def _train_member(member_seed, deadline, device, max_epochs,
    ...
    """Train one ensemble member with the weighted anchored heteroscedastic
    NLL; selection over raw-vs-EMA on UNWEIGHTED temporal-val masked MSE
    (the scored quantity — the edge upweight biases only the fit, never the
    selection); then the proven recency fine-tune ending on a short-horizon
    EMA. Returns (net, best_val)."""
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # the edge-stencil signal the new loss upweight keys on.
    ...
    # Row weights = recency ramp x NEW missingness upweight.
    ...
    #     get barely upweighted). Rationale: the heteroscedastic head learns
    #     high logvar on imputed edge rows, which scales their NLL gradient
    #     down — the net systematically underfits exactly the bin where the
    #     parent is weakest (r2_u 0.287 edge vs 0.471 full). Sibling
    #     evidence says simulating edges by masking full rows hurts; this
    #     instead spends more gradient on the REAL edge rows, no
    #     distribution shift introduced. Joint mean-1 renormalization keeps
    #     the tuned loss scale unchanged.
    ...
    # than the best) so the worst case degenerates to a single conv net.
