...
class HeteroMLP(nn.Module):
    """Lineage MLP core, kept as a first-class ensemble member. Its inductive
    bias (flat fully-connected over the whole engineered vector) differs
    from the conv core's (local translation-shared filters over the raw
    stencil), so its errors decorrelate from the conv member's far more than
    a second seed of the same architecture would — that decorrelation is
    what the blended prediction harvests."""
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # conv-wide-and-deep core, member 1 is the lineage HeteroMLP. Distinct
    ...
    # ---- NEW: per-component validation-fitted blend weights ----
    # The two architectures have asymmetric skills (MLP stronger on u, conv
    # stronger on v in the lineage), so a uniform average is suboptimal for
    # both components. Fit the scalar convex blend weight per component by
    # closed-form least squares on the temporal validation split: for
    # w*p0 + (1-w)*p1 vs residual target y, the minimizer is
    # w = <p0-p1, y-p1> / ||p0-p1||^2 over valid entries. Only 2 scalars
    # are fitted, so overfitting val (which the recency fine-tune briefly
    # saw at low LR) is negligible; clipping to [0.15, 0.85] guarantees
    # both members always contribute, preserving the ensemble's
    # variance-cancellation insurance. Falls back to a uniform average for
    # any member count other than two.
    n_keep = len(keep)
    w_blend = np.full((n_keep, 2), 1.0 / max(n_keep, 1), dtype=np.float32)
    if n_keep == 2:
        vp = []
        with torch.no_grad():
            for net in keep:
                ...
                for i in range(0, len(X_v), 8192):
                    mu, _ = net(X_v[i:i + 8192])
                    preds.append(mu.cpu().numpy())
                vp.append(np.concatenate(preds).astype(np.float32))
        y_v = Ys[val_idx]
        m_v = mask[val_idx]
        for c in range(2):
            valid = m_v[:, c] > 0
            if valid.sum() > 100:
                d = vp[0][valid, c] - vp[1][valid, c]
                r = y_v[valid, c] - vp[1][valid, c]
                denom = float((d.astype(np.float64) ** 2).sum())
                if denom > 1e-8:
                    w = float((d.astype(np.float64) * r.astype(np.float64)).sum() / denom)
                    w = float(np.clip(w, 0.15, 0.85))
                    w_blend[0, c] = w
                    w_blend[1, c] = 1.0 - w

    # Predict: ridge baseline + per-component blend of residual mu over
    # accepted members.
    wb_t = torch.from_numpy(w_blend).to(device)
