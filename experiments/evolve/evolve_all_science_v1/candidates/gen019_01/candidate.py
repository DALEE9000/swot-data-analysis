...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # Opportunistic extra members (new): early stopping usually converges
    # each scheduled member well before its slice deadline, so a sizable
    # fraction of the soft budget goes unused. Spend it on more independent
    # seeds — the most reliable variance-reduction knob available. An extra
    # member starts only if the time remaining exceeds 0.7x the observed
    # mean per-member wall time so far; that estimate is conservative (it
