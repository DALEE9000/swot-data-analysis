...
def _featurize(X, mean, scale, n_cells):
    ...
    order), and — new this generation — NONLINEAR flow-regime features.
    ...
    New: the fast-regime deficit lives in fronts and eddies, whose signature
    is NONLINEAR in the derivatives — magnitudes and products that a
    SiLU MLP spends capacity approximating and does so poorly far from the
    data bulk (exactly the fast tail). So we hand them over explicitly,
    ...
    if k >= 2 and k * k == n_cells:
        ...
        if k >= 3:
            ...
            blocks += [lap, gp, gq]
        # ---- nonlinear flow-regime block (new) ----
