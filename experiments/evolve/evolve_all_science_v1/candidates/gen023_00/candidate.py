...
class ConvWideDeep(nn.Module):
    """Predictive core (unchanged from parent): conv path over the raw
    stencil image + mask channel, dense path over the engineered scalars,
    joined into one trunk with mu/logvar heads. The mask channel is what
    makes the new edge-truncation augmentation learnable: filters see
    jointly WHERE the data is real and WHAT it says, so they can form
    mask-gated one-sided operators at synthetic and real edges alike."""
...
def _train_member(member_seed, deadline, device, max_epochs,
    ...
    heteroscedastic NLL; selection over raw-vs-EMA on unweighted temporal-
    val masked MSE; then the proven recency fine-tune ending on a
    short-horizon EMA. Identical to the lineage; the training tensors now
    include the edge-truncation-augmented rows. Returns (net, best_val)."""
    ...
    # most-recent 10% with a short low-LR pass over the full window
    # (augmented rows included, carrying their source rows' recency
    # weights), ending on a short-horizon EMA of the fine-tune trajectory.
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # generalization — what the temporal test holdout measures. Validation
    # rows are ORIGINAL only; augmented rows never enter validation.
    ...
    # ---- Structured edge-truncation augmentation (the one new mechanism) --
    # The edge-stencil bins are the widest diagnostic gap (r2_u 0.2869 vs
    # 0.4707 full). Real swath edges truncate whole rows/columns of the
    # k x k window, not random cells — so synthesize exactly that: for 15%
    # of training-window rows, blank one full side of the stencil in the
    # RAW input (NaN), then re-run the complete featurization so the mask
    # channel, spatial std, derivatives, regime block, AND the ridge
    # baseline are all self-consistently degraded — the augmented row is
    # indistinguishable from a genuine edge row with a known-good target.
    # Same target + degraded input = denoising-style supervision: the conv
    # path (which sees the mask channel) learns mask-gated one-sided
    # operators from abundant synthetic edges instead of only scarce, noisy
    # real ones. Augmented rows carry their source rows' recency weights and
    # targets; validation stays original-only, so model selection still
    # optimizes the scored quantity. Cost is a one-time featurize pass over
    # ~15% extra rows plus ~15% longer epochs, absorbed by the existing
    # deadline logic.
    X_t_np = Xs[train_idx]
    Y_t_np = Ys[train_idx]
    M_t_np = mask[train_idx]
    W_t_np = w_all[train_idx]
    k_st = int(round(math.sqrt(n_cells)))
    if k_st >= 2 and k_st * k_st == n_cells:
        n_tr = len(train_idx)
        n_aug = int(0.15 * n_tr)
        # train_idx is the prefix [0, n - n_val), so these index X_train too.
        aug_src = rng0.choice(n_tr, size=n_aug, replace=False)
        Xa = X_train[aug_src].copy()
        sides = rng0.integers(0, 4, size=n_aug)
        for s in range(4):
            rs = np.where(sides == s)[0]
            if len(rs) == 0:
                continue
            if s == 0:
                ...
                cells = list(range(n_cells - k_st, n_cells))        # bottom row
            cols = [f * n_cells + c for f in range(n_feat) for c in cells]
            Xa[np.ix_(rs, cols)] = np.nan
        Xa_f = _featurize(Xa, mean, scale, n_cells)
        del Xa
        base_a = _baseline(Xa_f)
        Ma = mask[aug_src]
        Ya = np.where(Ma > 0,
                      np.nan_to_num(Y_train[aug_src].astype(np.float32),
                                    nan=0.0) - base_a,
                      0.0).astype(np.float32)
        del base_a
        X_t_np = np.concatenate([X_t_np, Xa_f])
        del Xa_f
        Y_t_np = np.concatenate([Y_t_np, Ya])
        M_t_np = np.concatenate([M_t_np, Ma])
        W_t_np = np.concatenate([W_t_np, w_all[aug_src]])

    X_t = torch.from_numpy(X_t_np)
    Y_t = torch.from_numpy(Y_t_np)
    M_t = torch.from_numpy(M_t_np)
    W_t = torch.from_numpy(W_t_np)
