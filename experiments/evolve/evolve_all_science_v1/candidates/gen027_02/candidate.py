...
def _featurize(X, mean, scale, n_cells):
    ...
    consumes the whole vector flat. The time coordinate (the one change vs
    the parent) is appended OUTSIDE this function, after all engineered
    blocks, so it lands in the dense path and never disturbs the image
    slice.
    """
...
class ConvWideDeep(nn.Module):
    ...
    the proven engineered features kept as a shortcut. The appended time
    coordinate feeds this path, so the dense MLP can condition the learned
    residual mapping on where in the mission window a row sits.
    ...
    def __init__(self, d_total, n_feat, n_cells, dropout=0.1,
        ...
        dd = d_total - self.img_dim  # mask block + engineered scalars + time
...
def _masked_nll_anchored(mu, logvar, target, mask, row_w, comp_w):
    ...
    weight (both inherited from the parent). The score is mean per-component
...
def _train_member(member_seed, arch, deadline, device, max_epochs,
    ...
    # on a short-horizon EMA of the fine-tune trajectory. With the time
    # coordinate this pass is also what teaches the nets the pos ~ 1.0
    # regime the clamped test coordinate rides.
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # ---- Time coordinate (THE one change vs the parent) ----
    # Rows are time-ordered, so row position is a clean proxy for time in
    # the mission window. Append it, mapped to [-1, 1], as one final column
    # AFTER all engineered blocks (image slice untouched; feeds the dense
    # path and the flat MLP). Rationale: late-test u (0.391) is the
    # parent's worst temporal bin while late v (0.510) beats early v — the
    # target mapping demonstrably shifts over the window, and the recency
    # loss ramp can only re-weight gradient toward late rows; it cannot let
    # a single static mapping VARY with time. An explicit smooth time input
    # can (drift/seasonal conditioning), and the two mechanisms compose.
    # Test rows are strictly later than the window, so they get the clamp
    # value +1.0 — the latest regime the net has seen; the temporal val
    # split sits at pos ~ [0.8, 1.0] so every selection decision already
    # measures the near-future regime, and the recency fine-tune trains
    # pos up to 1.0 so test is not an extrapolation. One smooth scalar has
    # minimal capacity for time-memorization; if drift is not learnable
    # its weights go to ~0 and this degenerates to the parent.
    n = len(Xs)
    pos_t = np.linspace(-1.0, 1.0, n, dtype=np.float32)[:, None]
    Xs = np.hstack([Xs, pos_t])
    del pos_t

    mask = np.isfinite(Y_train).astype(np.float32)
    Ys = np.nan_to_num(Y_train.astype(np.float32), nan=0.0)

    # Per-component inverse-variance weights (inherited from the parent).
    ...
    # still optimizes the scored quantity. The time column is part of the
    # ridge input too, so the baseline absorbs any LINEAR drift in the
    # targets (clamped at pos = 1 for test, i.e. end-of-window trend, never
    # an unbounded extrapolation).
    ...
    # Test rows are later than the whole training window, so their time
    # coordinate is the clamp value +1.0 (latest learned regime).
    ...
    with torch.no_grad():
        for i in range(0, len(X_test), 65536):
            ...
            Xf = np.hstack([Xf, np.ones((len(Xf), 1), dtype=np.float32)])
            base = _baseline(Xf)
