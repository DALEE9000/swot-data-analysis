...
def _featurize(X, mean, scale, n_cells):
    ...
    neighborhood. Two appended blocks:
      * per-cell missing fraction (n, C) — encodes swath-edge geometry
        compactly, letting the net distinguish padded stencil cells from a
        genuinely mean-valued ocean (edge rows are the widest R^2 gap in the
        parent's diagnostics);
      * per-feature stencil standard deviation (n, F), computed on the
        standardized values over VALID cells only — an explicit local
        gradient / front-intensity signal aimed at the fast-flow regime.
        Rows with a single valid cell get std 0 ("no local gradient info").
    """
