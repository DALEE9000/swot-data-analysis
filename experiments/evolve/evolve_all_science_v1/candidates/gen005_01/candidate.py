...
def _masked_mse(pred, target, mask, comp_w):
    # Per-component inverse-variance weights make this a masked mean of
    # variance-normalized squared errors: minimizing it minimizes
    # (2 - R^2_u - R^2_v) up to constants — the actual scored quantity —
    # instead of letting the higher-variance component dominate gradients.
    wm = mask * comp_w
    diff = (pred - target).pow(2) * wm
    return diff.sum() / wm.sum().clamp(min=1e-8)
...
def _train_member(member_seed, deadline, device, max_epochs, comp_w,
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for i in range(0, n_train, bs):
            ...
            optimizer.zero_grad()
            loss = _masked_mse(net(xb), yb, mb, comp_w)
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            ...
            for i in range(0, len(X_v), bs):
                val_losses.append(
                    _masked_mse(net(X_v[i:i + bs]), Y_v[i:i + bs], M_v[i:i + bs], comp_w).item())
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # Inverse-variance component weights, normalized to sum to 2 so the loss
    # scale (and hence LR / weight-decay balance) matches the parent. Since
    # the score is mean(R^2_u, R^2_v), each component's MSE should count
    # relative to its own variance; the raw sum under-weights the lower-
    # variance u channel, which is exactly the persistently weaker one.
    var_u = float(np.nanvar(Y_train[:, 0]))
    var_v = float(np.nanvar(Y_train[:, 1]))
    w = np.array([1.0 / max(var_u, 1e-8), 1.0 / max(var_v, 1e-8)], dtype=np.float32)
    w *= 2.0 / w.sum()
    comp_w = torch.from_numpy(w).to(device)
