...
def _featurize(X, mean, scale, n_cells):
    ...
    spatial std + signed directional gradients.
    ...
    neighborhood. Four ingredients:
      ...
      * per-feature SIGNED directional gradients (n, 2F): the stencil is
        reshaped to a (k, k) patch and dx = mean(last column) - mean(first
        column), dy = mean(last row) - mean(first row), NaN-aware over valid
        cells only and computed BEFORE imputation so padded cells never
        fabricate gradients. The std block above is an unsigned magnitude —
        "a front is here" — while shear/strain of the geostrophic field
        (which drives the ageostrophic secondary circulation at fronts and
        eddies, the fast-regime rows with the worst RMSE) is a signed,
        oriented quantity, and orientation is what separates the u response
        from the v response. A one-sided-missing edge patch degrades to a
        half-width estimate or 0, which the missingness block flags;
      * gradient-preserving imputation: a missing stencil cell at offset
        ...
        local gradient) continuation of the field. Extrapolated fills are
    ...
    with warnings.catch_warnings():
        # nanstd/nanmean emit RuntimeWarning on all-NaN slices; those entries
        # are handled by the nan_to_num calls below.
        warnings.simplefilter("ignore")
        spat = np.nanstd(Z, axis=2).astype(np.float32)
        k = int(round(math.sqrt(n_cells)))
        if k >= 3 and k * k == n_cells:
            # Row-major (k, k) patch: axis 2 = row (along-track), axis 3 = col.
            G = Z.reshape(n, F, k, k)
            dx = (np.nanmean(G[:, :, :, k - 1], axis=2)
                  - np.nanmean(G[:, :, :, 0], axis=2)).astype(np.float32)
            dy = (np.nanmean(G[:, :, k - 1, :], axis=2)
                  - np.nanmean(G[:, :, 0, :], axis=2)).astype(np.float32)
        else:
            dx = np.zeros((n, F), dtype=np.float32)
            dy = np.zeros((n, F), dtype=np.float32)
    spat = np.nan_to_num(spat, nan=0.0, posinf=0.0, neginf=0.0, copy=False)
    dx = np.nan_to_num(dx, nan=0.0, posinf=0.0, neginf=0.0, copy=False)
    dy = np.nan_to_num(dy, nan=0.0, posinf=0.0, neginf=0.0, copy=False)
    # Fill AFTER the std/gradient blocks (which must see true valid cells).
    # Snapshot the point-reflected cells before any in-place filling so late
    # cells never read an already-imputed mirror as if it were data.
    center = Z[:, :, n_cells // 2]
    ...
    return np.hstack([Xs, miss, spat, dx, dy])
