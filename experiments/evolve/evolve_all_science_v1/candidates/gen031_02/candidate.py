...
def _masked_mse(pred, target, mask, comp_w):
    ...
    raw-vs-EMA selection, snapshot selection, and the member guard all
...
def _train_member(member_seed, arch, deadline, device, max_epochs,
    ...
    recency- and component-weighted anchored heteroscedastic NLL. Returns
    (nets, best_val) — a LIST of snapshot nets sharing this member's slice.

    THE ONE CHANGE vs the parent: the schedule + within-member snapshot
    ensembling. The parent's throughput fix (on-device fp16 data) freed
    many epochs per slice but spent them on ReduceLROnPlateau + one final
    net, and the score did not move — epochs are no longer the limiter,
    estimator variance is. Here, after epoch 1 the member measures its
    epoch cost, sizes a cosine horizon to ~85% of its remaining slice, and
    — when the horizon is long enough (>= 14 epochs) — splits it into TWO
    cosine cycles (lr 1e-3 -> 3e-5, warm restart between). At each
    annealed cycle end it snapshots the better of raw/EMA on the
    component-weighted temporal-val MSE. Each snapshot then gets the
    proven recency fine-tune (best-val snapshot first; later ones only if
    slice time remains), and ALL snapshots enter the prediction average.
    Different cosine basins give decorrelated errors at zero extra
    wall-clock slice cost — the diversity gen030_02's third member tried
    to buy by starving every member's training. Short slices degrade to
    one cycle == the proven single budget-aware cosine (gen028_02).
    Patience-15 backstop, raw-vs-EMA selection, and the fine-tune math are
    unchanged; the fp16 on-device fast path is inherited from the parent."""
    ...
    cycle_len = None
    snap_list = []   # (val, epoch, state_dict) at annealed cycle ends
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        # Size the horizon from the measured first-epoch cost (train + val,
        # both recur), reserving ~15% of the slice for the fine-tune stage.
        # Two cycles only when the horizon can fund two real anneals;
        # otherwise this is exactly the proven single budget-aware cosine.
        if total_T is None:
            ep_time = max(time.time() - t_start, 1e-6)
            left = max(0.0, deadline - time.time())
            total_T = min(max_epochs, epoch + max(3, int(0.85 * left / ep_time)))
            n_cycles = 2 if total_T >= 14 else 1
            cycle_len = max(3, int(math.ceil(total_T / n_cycles)))

        # Snapshot at each annealed cycle end (and at the horizon's end, in
        # case the last cycle is truncated): the better of raw/EMA there.
        if (epoch % cycle_len == 0) or (epoch >= total_T):
            src = net if val_raw <= val_ema else ema.net
            snap_list.append((val_loss, epoch, copy.deepcopy(src.state_dict())))

        if epoch >= total_T:
            break

        # Cosine-with-restart LR for the NEXT epoch: pos 0 (new cycle)
        # restores lr0; within a cycle it anneals to the 3e-5 floor.
        pos = epoch % cycle_len
        frac = min(1.0, pos / cycle_len)
    ...
    # Candidate snapshots: annealed cycle ends plus the global best (which
    # may predate a restart). Keep the 2 lowest-val distinct epochs so the
    # average never dilutes below the parent's per-member quality.
    snap_list.append((best_val, best_epoch, best_state))
    snap_list.sort(key=lambda t: t[0])
    chosen, seen = [], set()
    for v, e, s in snap_list:
        if e in seen or not np.isfinite(v):
            continue
        seen.add(e)
        chosen.append((v, s))
        if len(chosen) == 2:
    ...
    # on a short-horizon EMA of the fine-tune trajectory. Virtual concat of
    # train/val tensors (no 1.2 GB copy), unchanged from the parent.
    def _finetune(ft_net):
        n_t = len(X_t)
        n_all = n_t + len(X_v)
        ft_opt = torch.optim.AdamW(ft_net.parameters(), lr=1e-4, weight_decay=1e-4)
        ft_ema = _EMA(ft_net, decay=0.998)
        out_of_time = False
        ft_net.train()
        for _ in range(2):
            ...
            for bi, i in enumerate(range(0, n_all, bs)):
                ...
                loss.backward()
                ft_opt.step()
                ft_ema.update(ft_net)
                if bi % 100 == 0 and time.time() > deadline:
                    out_of_time = True
                    break
            if out_of_time:
                break
        ft_net.load_state_dict(ft_ema.net.state_dict())

    # Fine-tune each kept snapshot, best first; a snapshot whose turn finds
    # the slice exhausted joins the average as-is (still a valid annealed
    # model — never worse than dropping it to a single-net member).
    nets = []
    for v, state in chosen:
        m_net = _make_net(X_t.shape[1], n_feat, n_cells, device, arch)
        m_net.load_state_dict(state)
        if time.time() < deadline and np.isfinite(best_val):
            _finetune(m_net)
        m_net.eval()
        nets.append(m_net)
    return nets, best_val
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # early stopping and snapshot selection optimize forward-in-time
    ...
    # Inherited from the parent: stage the full featurized train matrix on
    # the GPU in float16 (~3.4M x ~160 cols -> ~1.1 GB, well inside 4 GB
    # alongside the ~0.12 GB fp16 val block, the small nets, and optimizer
    # state). On-device indexing + per-batch fp32 cast removes the host
    ...
    # OOM we fall back to the lineage's exact CPU pipeline.
    if device.type == "cuda":
        try:
    ...
    # wall-clock slices (unchanged): member 0 is the conv-wide-and-deep
    # core, member 1 is the lineage HeteroMLP. Each member now contributes
    # up to two cosine-cycle snapshots (see _train_member), so the average
    # spans both architectures AND basins without shrinking any member's
    # training slice.
    archs = ["conv", "mlp"]
    M_ens = 2
    members = []
    for m in range(M_ens):
        ...
        if time.time() > deadline:
            break
        nets, best_val = _train_member(
            seed + 101 * m, archs[m % len(archs)], deadline, device, max_epochs,
            n_feat, n_cells, comp_w,
            X_t, Y_t, M_t, W_t, X_v, Y_v, M_v, W_v)
        members.append((nets, best_val))
    ...
    # degenerates to the single-conv lineage instead of averaging in an
    # undertrained or weaker-arch net.
    finite = [(nets, v) for nets, v in members if np.isfinite(v)]
    if finite:
        v_best = min(v for _, v in finite)
        keep = [net for nets, v in finite if v <= v_best * 1.15 for net in nets]
    else:
        keep = list(members[0][0])

    # Predict: ridge baseline + averaged residual mu over accepted snapshot
    # nets from all accepted members.
