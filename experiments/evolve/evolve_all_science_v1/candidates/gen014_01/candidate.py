...
def _masked_nll_anchored(mu, logvar, target, mask, row_w):
    ...
    mask carries per-component inverse-variance weights (0 where the
    component is invalid): the harness scores mean(R^2(u), R^2(v)), which
    normalizes each component by its own variance and weights them equally,
    while a raw-m/s loss hands the higher-variance component more gradient.
    With the weighted mask, the optimized quantity is proportional to
    mean(1 - R^2) — the fitness itself. Weights are normalized to sum 2 so
    the loss scale (and tuned LR/scheduler behavior) is unchanged.
    """
...
def _masked_mse(pred, target, mask):
    ...
    return diff.sum() / mask.sum().clamp(min=1e-8)
...
def _train_member(member_seed, deadline, device, max_epochs,
    ...
    heteroscedastic NLL (early stop on UNWEIGHTED-by-recency temporal-val
    masked MSE of mu under the component-normalized mask — a proxy for the
    scored mean R^2 — then the proven recency fine-tune) within its
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        # Model selection and LR scheduling on masked MSE of mu under the
        # component-normalized mask — i.e. a direct proxy for the scored
        # mean R^2 — NOT on the training loss, whose value mixes in the
        # variance term and the recency weighting. Keeping validation free
        # of recency weights means the recency emphasis cannot game early
        # stopping; keeping the component weights means selection ranks
        # models by the same u/v balance the harness scores.
        net.eval()
        with torch.no_grad():
            val_losses = []
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # Component-normalized loss weights: the score is mean(R^2(u), R^2(v)) —
    # each component's error normalized by its OWN variance, equal weight —
    # but MSE in raw m/s hands the higher-variance component proportionally
    # more gradient and more of the shared trunk's capacity (the likely
    # mechanism behind the lineage-wide u deficit). Folding inverse-variance
    # weights into the validity mask makes every masked loss in the module
    # (training NLL + anchor, fine-tune, validation selection) proportional
    # to mean(1 - R^2) over valid entries — the fitness itself. Normalized
    # to sum 2 (mean 1 per component) so the overall loss scale, and hence
    # the tuned LR/scheduler/logvar-init behavior, is unchanged.
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        comp_var = np.array([np.nanvar(Y_train[:, 0]), np.nanvar(Y_train[:, 1])],
                            dtype=np.float32)
    comp_var = np.where(np.isfinite(comp_var) & (comp_var > 1e-8), comp_var, 1.0)
    comp_w = 1.0 / comp_var
    comp_w *= 2.0 / comp_w.sum()
    mask = mask * comp_w[None, :].astype(np.float32)
    ...
    # is strictly later than every training row and the parent's late-test
    # r2_u degrades, so the training distribution drifts within the window; a
    # mild geometric emphasis keeps the early window as a physics anchor while
    # biasing the fit toward the most test-like conditions.
