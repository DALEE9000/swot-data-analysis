...
class ConvWideDeep(nn.Module):
    ...
    scalars, joined into one trunk.

    THE ONE CHANGE (shared with the MLP member): ~2x capacity. The parent's
    GPU-resident staging removed the data bottleneck, and the extra epochs
    it bought moved the score by only +0.0001 — the members are no longer
    epoch-starved, they are parameter-starved on 3.8M rows. Conv channels
    32/48 -> 48/64, both path projections 128 -> 160, dense widened to 384,
    trunk widened to 192 on the 320-dim concat. Richer mask-aware filters
    also give the edge-stencil rows (the worst diagnostic bin) more
    expressive local operators than the fixed central differences.

    Conv path: first F*C input columns reshaped to an (F, k, k) image with
    the per-cell missing fraction appended as an explicit mask channel.
    Two padded 3x3 convolutions; with k=3 the second layer's receptive
    field covers the whole window.
    ...
    def __init__(self, d_total, n_feat, n_cells, dropout=0.1,
        ...
        self.conv_proj = nn.Sequential(
            nn.Linear(ch2 * n_cells, 160), nn.LayerNorm(160), nn.SiLU(),
            nn.Dropout(dropout),
        )
        dd = d_total - self.img_dim  # mask block + all engineered scalars
        self.dense = nn.Sequential(
            nn.Linear(dd, 384), nn.LayerNorm(384), nn.SiLU(), nn.Dropout(dropout),
            nn.Linear(384, 160), nn.LayerNorm(160), nn.SiLU(), nn.Dropout(dropout),
        )
        self.trunk = nn.Sequential(
            nn.Linear(320, 192), nn.LayerNorm(192), nn.SiLU(), nn.Dropout(dropout),
        )
        self.mu = nn.Linear(192, 2)
...
class HeteroMLP(nn.Module):
    ...
    leans on. Widened 256,256,128 -> 384,384,192 as part of the single
    capacity change (see ConvWideDeep docstring)."""

    def __init__(self, n_inputs, hidden=(384, 384, 192), dropout=0.1,
                 logvar_init=-3.5):
        super().__init__()
        layers = []
        d = n_inputs
...
def _make_net(d_total, n_feat, n_cells, device, arch):
    ...
    return HeteroMLP(d_total, hidden=(384, 384, 192), dropout=0.1).to(device)
...
def _train_member(member_seed, arch, deadline, device, max_epochs,
    ...
    Data plumbing unchanged from the parent: train/val matrices may arrive
    already resident on the GPU in float16 (see train_and_predict), so the
    inner loop indexes on-device and casts each batch to float32 there —
    no host fancy-indexing, no PCIe transfer per batch. All `.to(device)` /
    `.float()` calls below are no-ops when the tensors are already
    device-resident fp32 (the CPU fallback path). The fine-tune gathers
    train/val batches from the two tensors directly rather than
    materializing a full concatenated copy, avoiding a ~2x memory spike on
    the 4 GB card."""
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # GPU staging (unchanged from the parent): full featurized train matrix
    # on the GPU in float16 (~3.4M x ~160 cols -> ~1.1 GB, well inside 4 GB
    # alongside the ~0.12 GB fp16 val block, the widened nets (~0.5M params
    # total, a few MB with optimizer state), and activations). On-device
    # indexing + per-batch fp32 cast removes the host-side bottleneck; fp16
    # rounding (~1e-3 relative on standardized, clipped features) is far
    # below data noise. On any CUDA OOM we fall back to the exact CPU
    # pipeline, so the worst case is slow, not a crash.
    if device.type == "cuda":
        try:
    ...
    # degenerates to a single member instead of averaging in an
