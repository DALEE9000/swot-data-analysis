...
def _featurize(X, mean, scale, n_cells):
    ...
    The MoE core consumes the whole vector flat; the gating network sees the
    same vector, so the regime block (speeds, vorticity, strain, OW) is
    directly available as a routing signal.
    """
...
class MoENet(nn.Module):
    """Gated mixture-of-experts over flow regimes — the wildcard core.

    A shared encoder maps the full engineered vector to a 256-d
    representation; K=4 expert sub-networks (each a 2-layer 128-d MLP)
    transform it independently, and a small gating MLP on the RAW input
    vector produces dense softmax mixture weights that blend the expert
    outputs. Because the input contains the explicit regime block (speeds,
    vorticity, strains, Okubo-Weiss), the gate can partition rows by
    dynamical regime and let each expert fit a regime-conditional
    residual map — the fast-front minority stops sharing capacity (and
    gradient direction) with the slow-regime majority. Dense soft gating,
    not top-k: differentiable everywhere, stable at this scale, and if the
    gate collapses to one expert the model degrades to the lineage MLP
    rather than failing. A mild temperature (logits / 1.5) keeps mixtures
    soft early so all experts receive gradient before specializing.

    Heads: mu (ridge-residual predictions) and logvar (anchored-NLL
    weighting) on the mixed representation, as in the whole lineage.
    """
    ...
    def __init__(self, d_total, n_experts=4, dropout=0.1, logvar_init=-3.5):
        ...
        self.gate = nn.Sequential(
            nn.Linear(d_total, 64), nn.SiLU(), nn.Linear(64, n_experts),
        )
    ...
    def forward(self, x):
        h = self.encoder(x)
        g = torch.softmax(self.gate(x) / 1.5, dim=1)          # (b, K)
        e = torch.stack([ex(h) for ex in self.experts], dim=1)  # (b, K, 128)
        z = (g.unsqueeze(-1) * e).sum(dim=1)
...
def _masked_nll_anchored(mu, logvar, target, mask, row_w, comp_w):
    ...
    weight (unchanged from the lineage). comp_w = (1/Var_u, 1/Var_v) mean-1
    normalized makes the loss's exchange rate between u and v errors match
    the metric's. The anchor keeps a gradient floor on variance-downweighted
    rows."""
...
def _masked_mse(pred, target, mask, comp_w):
    ...
    component R^2 deficit — the scored quantity — so early stopping and
    raw-vs-EMA selection optimize the metric directly."""
...
def _train_moe(member_seed, deadline, device, max_epochs, comp_w,
               X_t, Y_t, M_t, W_t, X_v, Y_v, M_v,
               X_v_cpu, Y_v_cpu, M_v_cpu, W_v_cpu):
    """Train the single MoE core on the full wall-clock budget with the
    lineage's proven optimization recipe: recency- and component-weighted
    anchored heteroscedastic NLL, EMA shadow, budget-aware cosine anneal
    (LR 1e-3 -> 3e-5 over the measured epoch horizon), patience-15
    backstop, raw-vs-EMA selection on component-weighted temporal-val
    masked MSE, and the recency fine-tune."""
    torch.manual_seed(member_seed)
    rng = np.random.default_rng(member_seed)

    net = MoENet(X_t.shape[1], n_experts=4, dropout=0.1).to(device)
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        # first-epoch cost, then set the LR for the NEXT epoch. Floor of 3
        # epochs keeps a degenerate estimate from annealing instantly; cap
        # at max_epochs so the schedule ends exactly when the loop can.
        if total_T is None:
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # Single MoE core on the full budget — the wildcard's structural bet:
    # regime specialization via gating buys more than architecture-diverse
    # averaging at the same cost.
    deadline = t0 + time_budget_s * 0.95
    net = _train_moe(seed + 101, deadline, device, max_epochs, comp_w,
                     X_t, Y_t, M_t, W_t, X_v, Y_v, M_v,
                     X_v_cpu, Y_v_cpu, M_v_cpu, W_v_cpu)
