...
class HeteroMLP(nn.Module):
    """Lineage MLP core. In the parent it was only a fallback for non-square
    stencils; here it is a first-class ensemble member again. Its inductive
    ...
    what the prediction average harvests."""
...
def _train_member(member_seed, arch, deadline, device, max_epochs,
    ...
    ending on a short-horizon EMA. Identical to the parent except the net
    architecture is a parameter. Returns (net, best_val)."""
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # Two-member ARCHITECTURE-diverse deep ensemble with the parent's
    # per-member wall-clock slices: member 0 is the conv-wide-and-deep core
    # (the parent's best single model), member 1 is the lineage HeteroMLP
    # (gen022_02's core, near-equal fitness, different error structure —
    # notably stronger r2_u). Different inductive biases decorrelate errors
    # far more than different seeds of one architecture, so the same-cost
    # average cancels more variance, concentrated on the noisy edge-stencil
    # and fast-front rows. Distinct seeds per member are kept so data order
    # and init both differ.
    archs = ["conv", "mlp"]
    M_ens = 2
    members = []
