...
class HeteroMLP(nn.Module):
    ...
    def __init__(self, n_inputs, hidden=(256, 256, 128), dropout=0.1,
        ...
        # Targets are standardized to unit variance per component, so the
        # marginal log-variance is ~0. Starting there keeps the NLL well
        # scaled from step 1 and early training behaves like plain MSE on
        # standardized targets — which is exactly the metric-aligned loss.
...
def _masked_nll_anchored(mu, logvar, target, mask):
    ...
    Targets arrive standardized to unit variance per component, so one unit
    of squared error is worth the same amount of R^2 for u as for v — the
    scored metric averages the two R^2 values with equal weight, and this
    loss now shares that weighting instead of letting the higher-variance
    component dominate the gradients.

    The NLL alone lets the variance head shrink gradients on hard rows by
    exp(-logvar) — which trades edge/fast-regime accuracy for the easy
    majority. The scored metric weights all rows equally, so that trade is a
    training/metric mismatch. The MSE anchor restores a gradient floor on
    downweighted rows while the variance head still handles calibration and
    relative weighting. In standardized units the residual precision
    exp(-logvar) settles near 1/(1 - R^2) ~ 1.8, so an anchor coefficient of
    0.1 keeps the same ~6% gradient-floor ratio the raw-unit parent had
    (where the anchor was 1 vs precision ~33... x0.06 ~ 2/33).
    """
    logvar = logvar.clamp(-7.0, 2.0)
    err2 = (mu - target).pow(2)
    per = 0.5 * (logvar + err2 * torch.exp(-logvar)) + 0.1 * err2
...
def _train_member(member_seed, deadline, device, max_epochs,
    ...
    """Train one ensemble member with the anchored heteroscedastic NLL (early
    stop on temporal-val masked MSE of mu, then the proven recency fine-tune)
    within its wall-clock slice. Returns (net, best_val)."""
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        # Model selection and LR scheduling on masked MSE of mu — computed on
        # STANDARDIZED targets, so both components count equally, matching
        # the equal-weight mean-R^2 the harness scores — NOT on the training
        # loss, whose value mixes in the variance term.
        net.eval()
        with torch.no_grad():
            val_losses = []
    ...
    # same anchored loss so relative row weighting stays consistent.
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # Per-component TARGET standardization. The harness scores the unweighted
    # mean of R^2(u) and R^2(v); R^2 normalizes each component by its own
    # variance, but raw-m/s MSE weights components by variance, so the
    # higher-variance component soaks up gradient, LR scheduling, and model
    # selection while the weaker one (u, persistently ~0.06 R^2 behind) is
    # under-fit. Training, validating, and fine-tuning on unit-variance
    # targets makes the optimization objective proportional to the scored
    # metric; predictions are mapped back to m/s at the end. Means are left
    # untouched (near 0 in these data) so a NaN->0 masked fill stays neutral.
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        y_std = np.nanstd(Y_train, axis=0)
    y_std = np.where(np.isfinite(y_std) & (y_std > 1e-6), y_std, 1.0).astype(np.float32)
    Ys = np.nan_to_num(Y_train.astype(np.float32) / y_std, nan=0.0)
    ...
    # then un-standardize back to m/s.
    ...
    with torch.no_grad():
        for i in range(0, len(X_test), 65536):
            ...
            for net in keep:
                mu, _ = net(xb)
                acc += mu
            out[i:i + 65536] = ((acc / len(keep)) * y_std_t).cpu().numpy()
