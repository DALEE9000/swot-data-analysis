...
def _masked_nll_anchored(mu, logvar, target, mask, row_w, comp_w):
    ...
    scaled by a per-row weight (recency ramp x edge-missingness upweight,
    the parent's mechanism: edge rows' learned logvar would otherwise
    shrink their NLL gradient toward zero, systematically underfitting the
    weakest bin) AND a per-component inverse-variance weight (the grafted
    inspiration mechanism). The score is mean per-component R^2 =
    mean_c(1 - MSE_c/Var_c), so a unit of squared error on the
    lower-variance component (u) buys more R^2 than the same error on v;
    comp_w = (1/Var_u, 1/Var_v) mean-1 normalized makes the loss's
    exchange rate between u and v errors match the metric's. The two
    factors are orthogonal (row vs component axis) and simply multiply.
    The anchor keeps a gradient floor on variance-downweighted rows."""
...
def _masked_mse(pred, target, mask, comp_w):
    ...
    directly. Note: NO row weight here — the edge upweight biases only the
    fit, never the selection (the parent's argument, kept), while comp_w
    belongs in selection because it IS part of the metric."""
...
def _train_member(member_seed, arch, deadline, device, max_epochs,
    ...
    row-weighted (recency x edge) + component-weighted anchored
    heteroscedastic NLL.

    THE ONE CHANGE vs the parent: the LR schedule. The parent used
    ReduceLROnPlateau (halve after 5 stagnant epochs), but training is
    wall-clock-bound — a member's slice usually expires long before the
    plateau logic has annealed the LR, so the final weights (and the EMA
    built from them) sit at a high-LR noise floor. Here, after the first
    epoch we measure the per-epoch cost, estimate how many epochs fit in
    the member's remaining slice, and anneal the LR from 1e-3 to a 3e-5
    floor on a cosine over exactly that horizon (capped by max_epochs).
    Every member therefore finishes its slice fully annealed, which
    sharpens convergence uniformly and — combined with the EMA — lands in
    flatter minima that transfer better across the temporal shift (the
    late-test u bin, the parent's worst, has the most to gain: terminal LR
    jitter costs the lower-variance component the most R^2). Selection
    (raw-vs-EMA on component-weighted temporal-val masked MSE), patience-15
    backstop, and the recency fine-tune are unchanged."""
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # Per-row missing fraction BEFORE featurize consumes the NaNs: this is
    # the edge-stencil signal the loss upweight keys on.
    ...
    # Per-component inverse-variance weights (the parent's grafted
    # mechanism, unchanged). Score = mean_c(1 - MSE_c/Var_c): weighting
    # each component's squared error by 1/Var_c (of the ORIGINAL targets —
    # the additive ridge baseline preserves MSE identity, and R^2 is
    # computed on the final predictions) makes both the training loss and
    # every validation-based selection decision optimize mean R^2 rather
    # than pooled MSE, which implicitly favors the higher-variance
    # component (v, the one already ahead). Mean-1 normalized so the tuned
    # loss scale is unchanged; clipped to [0.5, 2] relative so a
    ...
    # Row weights = recency ramp x edge-missingness upweight (the parent's
    # mechanism, unchanged):
    #   * Recency: latest rows weigh 3x the earliest (exponential ramp).
    #   * Edge: rows with missing stencil cells get up to +1.5x extra pull
    #     (1 + 1.5 * missing fraction, continuous so barely-truncated rows
    #     get barely upweighted). Both members' heteroscedastic heads learn
    #     high logvar on imputed edge rows, scaling their NLL gradient
    #     down — systematic underfit of exactly the weakest bin. This
    #     spends more gradient on the REAL edge rows, introducing no
    #     distribution shift. Joint mean-1 renormalization keeps the tuned
    ...
    # seeds per member so data order and init both differ. Each member's
    # cosine schedule is sized to its own slice, so both finish annealed.
    archs = ["conv", "mlp"]
    M_ens = 2
    members = []
