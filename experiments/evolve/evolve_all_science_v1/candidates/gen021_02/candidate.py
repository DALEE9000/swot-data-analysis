...
def _train_member(member_seed, deadline, device, max_epochs,
    ...
    X_t / X_v_cpu arrive as float16 host tensors (throughput optimization —
    see train_and_predict); each minibatch is cast to float32 AFTER the
    device transfer so the net always computes in full precision.
    ...
    if time.time() < deadline and np.isfinite(best_val):
        X_f = torch.cat([X_t, X_v_cpu])  # both float16 on host
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # Throughput optimization (new this generation): the training feature
    # matrix lives on host RAM in FLOAT16. For a ~100k-parameter MLP the
    # per-step cost is dominated by the CPU fancy-index gather and the
    # host->device copy of the ~150-wide float rows, both linear in bytes —
    # halving the storage width roughly halves step time. Precision is not
    # a concern for the INPUTS: every column is sigma-standardized and all
    # engineered terms are clipped to +/-16, so float16's ~1e-3 relative
    # error is far below the feature noise floor. Each minibatch is cast
    # back to float32 on the GPU before the forward pass, so weights,
    # activations, loss, and the optimizer are untouched — optimization
    # dynamics are bit-for-bit the parent's up to input rounding. Targets,
    # masks, and weights stay float32 (they are cheap and enter the loss
    # directly). The freed wall clock is what lets the adaptive ensemble
    # below actually reach members 3-4 (the parent measured 873s for 2
    # members — no slack ever materialized), converting the speedup into
    # the one mechanism this plateau still rewards: seed-variance
    # reduction. The GPU-resident validation set stays float32.
    X_t = torch.from_numpy(Xs[train_idx]).to(torch.float16)
    Y_t, M_t = (torch.from_numpy(a[train_idx]) for a in (Ys, mask))
    W_t = torch.from_numpy(w_all[train_idx])
    # Keep CPU copies of the validation tail for the post-selection fine-tune
    # (float16 to match X_t for the torch.cat in the fine-tune pass).
    X_v_cpu = torch.from_numpy(Xs[val_idx]).to(torch.float16)
    Y_v_cpu = torch.from_numpy(Ys[val_idx])
    M_v_cpu = torch.from_numpy(mask[val_idx])
    W_v_cpu = torch.from_numpy(w_all[val_idx])
    X_v = X_v_cpu.to(device=device, dtype=torch.float32)
    ...
    # Opportunistic extra members: early stopping converges each scheduled
    # member well before its slice deadline (especially with the float16
    # throughput gain above), so a sizable fraction of the soft budget goes
    # unused. Spend it on more independent seeds — the most reliable
    # variance-reduction knob available. An extra member starts only if the
    # time remaining exceeds 0.7x the observed mean per-member wall time so
    # far; that estimate is conservative (it amortizes the one-off
    ...
    # drops any member the deadline truncated. With no slack this loop
    # never runs and the module degenerates exactly to the two-member
    # parent.
    ...
    # Test featurization stays float32 end to end — no rounding on the
    # scored path.
