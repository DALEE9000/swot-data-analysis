...
def _featurize(X, mean, scale, n_cells):
    """Center-fill NaN stencil cells, standardize, append missingness flags.
    ...
    neighborhood. NaN padding cells at swath edges are filled with that
    row's CENTER-cell value of the same feature: the padded neighborhood
    becomes a locally plausible, zero-gradient field instead of a jump to
    the global feature mean, which previously fabricated spatial gradients
    exactly where the stencil model looks for flow structure. The per-cell
    missingness fractions are appended so the network can still distinguish
    replicated padding from a genuinely uniform ocean state; interior rows
    get all-zero flags and identical features to before.
    """
    X = X.astype(np.float32, copy=True)
    n, d = X.shape
    F = d // n_cells
    Xr = X.reshape(n, F, n_cells)          # view into X
    nanmask = np.isnan(Xr)
    miss = nanmask.mean(axis=1).astype(np.float32)
    center = Xr[:, :, n_cells // 2]
    Xr[nanmask] = np.broadcast_to(center[:, :, None], Xr.shape)[nanmask]
    Xs = (X - mean) / scale
    # Rows whose center cell is itself NaN fall back to the feature mean.
    Xs = np.nan_to_num(Xs, nan=0.0, posinf=0.0, neginf=0.0, copy=False)
    return np.hstack([Xs, miss])


def _edge_patterns(n_cells):
    """Boolean masks of realistic swath-edge shapes on the k x k stencil.

    A swath edge truncates a whole side of the neighborhood (or two adjacent
    sides at a corner), never a random scatter of cells, so the augmentation
    draws from the 4 side + 4 corner patterns. The center cell is never in
    any pattern.
    """
    k = int(round(math.sqrt(n_cells)))
    if k * k != n_cells or k < 2:
        return None
    r = np.arange(n_cells) // k
    c = np.arange(n_cells) % k
    left, right, top, bot = (c == 0), (c == k - 1), (r == 0), (r == k - 1)
    pats = [left, right, top, bot,
            top | left, top | right, bot | left, bot | right]
    return np.stack(pats).astype(np.float32)


def _augment_edges(xb, patterns, F, C, p=0.25):
    """Simulate swath edges on a fraction of the batch, on-device.

    Mirrors _featurize's edge convention exactly: masked cells take the row's
    (standardized) center value of the same feature, and the per-cell
    missingness flags are raised to 1. Cell columns of one feature share
    near-identical standardization stats, so overwriting in standardized
    space is equivalent to the raw-space center-fill to within noise.
    """
    B = xb.shape[0]
    sel = (torch.rand(B, device=xb.device) < p).float()
    pid = torch.randint(patterns.shape[0], (B,), device=xb.device)
    m = patterns[pid] * sel[:, None]                      # (B, C), 1 = masked
    xf = xb[:, :F * C].view(B, F, C)
    center = xf[:, :, C // 2].unsqueeze(2)
    xf = xf * (1.0 - m[:, None, :]) + center * m[:, None, :]
    flags = torch.maximum(xb[:, F * C:], m)
    return torch.cat([xf.reshape(B, F * C), flags], dim=1)
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # matrix so valid-data moments are not skewed by replicated padding.
    ...
    pat_np = _edge_patterns(n_cells)
    patterns = torch.from_numpy(pat_np).to(device) if pat_np is not None else None
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for i in range(0, n_train, bs):
            ...
            if patterns is not None:
                xb = _augment_edges(xb, patterns, n_feats, n_cells)
            optimizer.zero_grad()
    ...
    # and budget-checked in-loop. The same edge augmentation applies here so
    # the fine-tune sees the identical input distribution as main training.
    if time.time() - t0 < time_budget_s:
