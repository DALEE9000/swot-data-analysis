...
def _make_edge_augment(X_raw, rng, n_cells, frac=0.25):
    """Swath-edge dropout augmentation (new this generation).

    The edge-stencil bins are the widest diagnostic gap (r2_u 0.2876 vs
    0.4632 full stencil), and real edge rows can never teach the net how
    much information their truncation destroyed — their targets are only
    ever paired with degraded inputs. So we manufacture that supervision:
    take rows whose stencil is FULLY valid, NaN out one randomly chosen
    side of the k x k window at depth 1 (the geometry of swath-edge
    truncation: a whole row or column of neighbors falls off the swath),
    and add the masked copy as an extra training row with the ORIGINAL
    high-quality target. Downstream featurization treats the mask exactly
    like a real edge — nonzero missing fraction, stencil std over the
    surviving cells, mirror fill degrading central differences to
    one-sided ones — so the net sees abundant (edge-like input, clean
    target) pairs and learns the degraded-input mapping instead of
    treating edge patterns as out-of-distribution noise.

    Only fully-valid rows are candidates (masking an already-truncated
    stencil would create unrealistic double-edge patterns). Returns
    (source_indices, masked_raw_copy) or None when there is no stencil
    or no candidates.
    """
    k = int(round(math.sqrt(n_cells)))
    if k < 2 or k * k != n_cells:
        return None
    cand = np.flatnonzero(np.isfinite(X_raw).all(axis=1))
    m = min(int(len(X_raw) * frac), len(cand))
    if m == 0:
        return None
    idx = rng.choice(cand, size=m, replace=False)
    idx.sort()  # keep time order so recency weights stay meaningful
    Xa = X_raw[idx].copy()
    F = Xa.shape[1] // n_cells
    Z = Xa.reshape(m, F, n_cells)
    cols = np.arange(n_cells) % k
    rows = np.arange(n_cells) // k
    side_masks = [cols == 0, cols == k - 1, rows == 0, rows == k - 1]
    side = rng.integers(0, 4, size=m)
    for s in range(4):
        rsel = np.flatnonzero(side == s)
        if rsel.size:
            Z[np.ix_(rsel, np.arange(F), np.flatnonzero(side_masks[s]))] = np.nan
    return idx, Xa
...
def _masked_nll_anchored(mu, logvar, target, mask, row_w):
    ...
    scale the optimizer and LR schedule were tuned on. Augmented edge-
    dropout rows arrive with 0.5x their source's weight, folded into the
    same mechanism.
    """
...
def _train_member(member_seed, deadline, device, max_epochs,
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        # being optimized. The validation set contains NO augmented rows —
        # selection stays on the real data distribution.
...
def train_and_predict(X_train, Y_train, X_test, params):
    ...
    # ---- swath-edge dropout augmentation (the one new mechanism) ----
    # Manufacture (edge-like input, clean target) pairs from fully-valid
    # rows of the TRAINING slice only (validation stays untouched real
    # data). Augmented rows get 0.5x their source's recency weight so they
    # regularize the edge mapping without outvoting real observations; the
    # loss normalizer divides by the batch weight sum, so the overall loss
    # scale the LR schedule was tuned on is preserved.
    rng_aug = np.random.default_rng(seed + 777)
    aug = _make_edge_augment(X_train[:n - n_val], rng_aug, n_cells, frac=0.25)
    if aug is not None:
        a_idx, Xa_raw = aug
        Xa = _featurize(Xa_raw, mean, scale, n_cells)
        del Xa_raw
        X_t = torch.cat([X_t, torch.from_numpy(Xa)])
        Y_t = torch.cat([Y_t, torch.from_numpy(Ys[a_idx])])
        M_t = torch.cat([M_t, torch.from_numpy(mask[a_idx])])
        W_t = torch.cat([W_t, torch.from_numpy(0.5 * w_all[a_idx])])
        del Xa
