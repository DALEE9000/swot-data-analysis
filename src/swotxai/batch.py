...
# Live per-job progress: (run_id, step, frac, msg). Receives every step event
# including the structured "train_epoch" stream from the ANN backend.
JobProgressCb = Callable[[str, str, float, str], None]


@dataclass
class JobSpec:
    run_id: str
    model: str                 = "rf"  # "rf" or "ann"
    features: list[str]        = field(default_factory=lambda: list(DEFAULT_FEATURES))
    stencil_k: int             = 3
    random_state: int          = 42
    # RF hyperparameters (ignored when model == "ann")
    rf_n_estimators: int       = 50
    rf_max_depth: int          = 15
    # ANN hyperparameters (ignored when model == "rf")
    ann_hidden_layers: list[int] = field(default_factory=lambda: [256, 256, 128])
    ann_activation: str        = "silu"
...
def derive_base_run_id(batch_cfg: BatchConfig) -> str:
    ...
    key = {
        ...
        "era5_pkl_path":  batch_cfg.era5_pkl_path,
...
def _shared_config_kwargs(batch_cfg: BatchConfig) -> dict:
    return dict(
        swot_path        = batch_cfg.swot_path,
        hfr_path         = batch_cfg.hfr_path,
        era5_path        = batch_cfg.era5_path,
...
def _build_job_config(batch_cfg: BatchConfig, job: JobSpec, rf_n_jobs: int) -> SWOTConfig:
    """Overlay job-specific model + hyperparams onto the shared base config."""
    return SWOTConfig(
        **_shared_config_kwargs(batch_cfg),
        # Per-job fields
        run_id            = job.run_id,
        model             = job.model,
        ...
        rf_n_estimators   = job.rf_n_estimators,
        rf_max_depth      = job.rf_max_depth,
        rf_n_jobs         = rf_n_jobs,
        ann_hidden_layers = job.ann_hidden_layers,
        ann_activation    = job.ann_activation,
        ann_dropout       = job.ann_dropout,
        ann_lr            = job.ann_lr,
        ann_max_epochs    = job.ann_max_epochs,
...
def run_batch(
    ...
    use_cache: bool = True,
    job_progress_cb: JobProgressCb | None = None,
) -> dict[str, dict]:
    """
    ...
    job_progress_cb : JobProgressCb | None
        Called as (run_id, step, frac, msg) for every step event of every
        job, including the ANN backend's structured "train_epoch" stream —
        lets UIs show live per-job training progress.
    use_cache : bool

    Returns
    -------
    dict mapping run_id → per-job results dict (keys: flattened, model_u,
    model_v, metrics, swot_dict, hfr_dict, animation_paths).
    """
    ...
    rf_n_jobs = max(1, (os.cpu_count() or 1) // batch_cfg.max_workers)
    ...
    def _make_job_cb(run_id: str) -> ProgressCb:
        def cb(step: str, frac: float, msg: str) -> None:
            if step != "train_epoch":  # structured event for live monitors, not a log line
                with log_lock:
                    job_logs[run_id].append(f"[{step}] {msg}")
            if job_progress_cb:
                job_progress_cb(run_id, step, frac, msg)
        return cb

    def _run_job(job: JobSpec) -> dict:
        job_cfg = _build_job_config(batch_cfg, job, rf_n_jobs)
        if job_status_cb:
